...
export function ReporterFab({ unreadCount, onOpen }: ReporterFabProps) {
  ...
  return (
    <div className="everframe-root everframe-fab-wrap">
      <button type="button" className="everframe-fab" aria-label={label} data-testid="reporter-fab" onClick={onOpen}>
        <MessageSquare size={20} aria-hidden="true" />
        {unreadCount > 0 ? <span className="everframe-fab-dot" aria-hidden="true" /> : null}
      </button>
    </div>
  );
}
