...
//
// Copied from packages/sdk-react/__tests__/_setup.ts when the framework-free
// machinery moved here. The jest-dom import came back with the reporter UI:
// the `__tests__/reporter-ui/**` specs render React and assert with jest-dom
// matchers. This is TEST-ONLY — `src/index.ts` still has no path to React
// (see its header); only `src/ui.ts` does.
