 ...
 * native payloads as `Object[]`; each Everframe event has a narrower bridge
 * contract, so the unchecked native boundary is centralized here instead of
 * repeated at every listener.
 */
export type EverframeNativeEventEmitter = {
 ...
 * dedicated `RCTEventEmitter` subclass (`EverframeEventEmitter`); Android
 * uses the default `RCTDeviceEventEmitter` which `NativeEventEmitter`
 * routes to when constructed with no argument.
 */
export function getEmitter(): EverframeNativeEventEmitter {
  if (_emitter !== null) return _emitter as unknown as EverframeNativeEventEmitter;
  if (Platform.OS === 'ios') {
    // `NativeModules.EverframeEventEmitter` is registered by
    // `EverframeEventEmitter.swift`'s `RCT_EXPORT_MODULE`. If the host hasn't
    ...
    const mod = NativeModules.EverframeEventEmitter;
    if (mod === undefined && __DEV__) {
      // eslint-disable-next-line no-console
      console.warn(
        '[everframe] EverframeEventEmitter native module not found — companion events will not fire. Did the host run `pod install`?',
      );
    }
  ...
  return _emitter as unknown as EverframeNativeEventEmitter;
}
