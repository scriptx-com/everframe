...
//
// @traceitx/react-native public barrel.
//
// Architecture (Phase 06, D-05/D-07 flip 2026-05-11):
// - <TraceItXProvider config={...}> owns configure-on-mount and the React
...
//   `useTraceItX().open()` for every other trigger; the call resolves when
//   the user submits or cancels the native modal.
// - <TraceItXSensitive> + useTraceItXSensitiveRef forward to
//   NativeTraceItX.registerSensitiveRect so the native modal redacts
...
export { TraceItXProvider, useTraceItX } from './TraceItXProvider.js';
export type { TraceItXProviderProps } from './TraceItXProvider.js';

// Sensitive wrapper + hook.
export { TraceItXSensitive, useTraceItXSensitiveRef } from './Sensitive.js';
...
export type { TraceItXContextValue } from './contextSeam.js';
export type { TraceItXIntegration } from './integrations/types.js';
...
// nav stack. See TXScreen.tsx header for per-stack recipes.
...
export type { VitalsPlayerEventType } from '@traceitx/protocol';

// Bridge-level types still useful at the JS boundary.
export type { ConfigOpts, Rect, Spec, TXUserSpec } from './NativeTraceItX.js';

// Phone-companion bridge (Plan 06.2-11). Namespace import mirrors the
// `TraceItX.shared.companion` / `TraceItX.companion` shape on iOS / Android:
//
//   import { companion, useCompanion } from '@traceitx/react-native';
...
export type { CaptureExceptionOptions, ReportDraft, DeviceMetadata } from '@traceitx/sdk-core';
/**
