...
//   NOT the named `EverframeUserSpec` alias, even though `EverframeUserSpec` is flat like
...
//   like `ConfigOpts`/`Rect`/`EverframeUserSpec` resolves to — always emits a
...
//   a reduced spec: `setUser(user?: EverframeUserSpec)` generates
//   `- (void)setUser:(JS::NativeEverframe::EverframeUserSpec &)user;` (required,
...
//   there. `EverframeUserSpec` stays exported as the JS-FACING type (runtime.ts /
 ...
 * the richer UI-tree `Rect` from @everframe/sdk-core in a spec position.
 */
export type Rect = {
  x: number;
  y: number;
...
/**
 * Configure-time options for the native Everframe runtime. Flat-object only.
...
export type ConfigOpts = {
   ...
   * matching `EverframeConfig.installIdentifierEnabled`'s own `true` default
...
export interface Spec extends TurboModule {
   ...
   * below exactly): called by the JS-side `everframe.extra.resolveRequested`
   * listener once it has — IF a resolver is currently registered — invoked
   * it, budgeted the result exactly like `@everframe/sdk-core`'s
   ...
   * (`Everframe.shared.addBreadcrumb` on iOS / `Everframe.addBreadcrumb` on
  ...
  // `Everframe.shared.companion` state + pairUrl signals to JS via the
  ...
  //   • `everframe.companion.state`            → 'unpaired' | 'paired' | 'report_in_progress' | 'phone_disconnected'
  ...
  //   • `everframe.companion.resolvedName`     → string | null (naming spec 2026-08-24)
  //   • `everframe.companion.attachChallenge`  → { code, requestedByName, ttlMs } | null (spec 2026-08-19)
  startCompanion(): void;
  stopCompanion(): void;
  /**
   ...
   *
   * Typed `UnsafeObject`, NOT `EverframeUserSpec` — a named struct alias like
   * `EverframeUserSpec` always generates as a REQUIRED ObjC reference regardless of
   * its `?`, which would silently break "no argument = clear" (see the file
   * header's D-decision note for the verified codegen output).
   * `EverframeUserSpec` is the richer type the JS facade (`runtime.ts` /
...
export default TurboModuleRegistry.getEnforcing<Spec>("Everframe");
