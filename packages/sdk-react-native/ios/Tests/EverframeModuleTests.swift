...
//
// These tests instantiate `EverframeBridge` directly (Swift static-method facade
...
final class EverframeModuleTests: XCTestCase {
    ...
    func testCaptureNowDictShape() {
        ...
        DispatchQueue.main.async {
            EverframeBridge.captureNow { dict, err in
    ...
    func testCaptureNowDoesNotLeakBinary() {
        let exp = expectation(description: "captureNow completion")
        DispatchQueue.main.async {
            EverframeBridge.captureNow { dict, _ in
                guard let dict = dict else {
                    XCTFail("captureNow returned nil dict")
                    exp.fulfill()
                    return
    ...
    func testSubmitEnqueuesToOutbox() {
        ...
        let outboxBefore = EverframeBridge._outboxCountForTesting()
        let exp = expectation(description: "submit completion")
        EverframeBridge.submitEnvelope(envelope) { result, err in
        ...
        wait(for: [exp], timeout: 5.0)
        let outboxAfter = EverframeBridge._outboxCountForTesting()
