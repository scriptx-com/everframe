...
//   TXReporterPresenter owns the entire report UX (capture + annotate + redact +
...
//
//     1. configure(opts)            → start TraceItX.shared with TraceItXConfig
//                                     + install TXReporterPresenter resolver
//     2. openReporter()             → await TraceItX.shared.report.open()
//                                     resolves with { status, reportId? }
//     3. registerSensitiveRect(tag) → SensitiveRectRegistry.mark(view)
//
// Strict invariants:
//   • NO new public method is added to `TraceItX.shared`.
//   • Every method guards on `TraceItX.shared.captureGate`.
...
import UIKit
import TraceItXKit
import TraceItXReporterUI    // tvOS slice present per Package.swift:12 (TV-03 of Phase 06.1)

@objc public final class TraceItXBridge: NSObject {

    private static let log = OSLog(subsystem: "com.traceitx.rn", category: "bridge")

    // One-shot resolver install flag. `TXReporterPresenter.installResolver()`
    ...
    @objc public static func configure(
        ...
        _ = endpoint
        os_log("[traceitx] configure appId=%{public}@", log: log, type: .info, appId as String)
        // Polarity flips exactly once, here: JS sends the veto
        // (`networkBodiesDisabled`), native stores the permission.
        var cfg = TraceItXConfig(appId: appId as String)
        ...
        // Codex round-6, H1 — IDEMPOTENT configure, twin of `TraceItXModule.configure`'s gate on
        // Android, and decided against the INSTALLED config rather than a cache of the last
        // options this bridge started with.
        //
        // `TraceItX.shared.start(...)` SUPERSEDES the running SDK: the outgoing controller's
        ...
        // `TraceItX.shared.start(configB)` between two bridge configures of A reproduces it with
        ...
        //
        // Codex round-7, I3 — the comparison is the WHOLE config (`TraceItXConfig` is now
        ...
        let unchanged = TraceItX.shared.captureGate
            && TraceItX.shared.currentConfig == cfg
            && companionAttachPinUi == newAttachPinUi
        if unchanged {
            os_log("[traceitx] configure: SDK already running this exact config — skipping start()",
                   log: log, type: .debug)
        } else {
            try TraceItX.shared.start(config: cfg)
        ...
        // funnel through TraceItX.shared.report.open() →
    ...
    /// host set no theme — maps to `nil` options so `TraceItXConfig.theme`
    ...
    /// Wires `TXReporterPresenter` as the resolver for `TraceItX.shared.report
    ...
    /// itself is `#if !os(tvOS)`-gated in TraceItXReporterUI. Mirror that
    ...
    private static func installPresenterResolverIfNeeded() {
        ...
        DispatchQueue.main.async {
            ...
            os_log("[traceitx] installed TXReporterPresenter resolver", log: log, type: .info)
        }
        #else
        os_log("[traceitx] tvOS — TXReporterPresenter not installed; reporting routes through phone companion", log: log, type: .info)
        #endif
    }
    ...
    /// `TraceItX.__pendingExtraResolveHook` closure (D1 fix — stale-teardown
    ...
    /// Installs `awaitJsExtraResolve` onto `TraceItX.__pendingExtraResolveHook`
    /// (see that property's doc comment on TraceItX.swift). Called from
    /// EVERY `configure()`, UNCONDITIONALLY — mirrors Android's
    /// `TraceItXModule.init` block installing its own `extraResolveHook`
    ...
    /// `TraceItXBridge` has no per-instance construction point of its own —
    ...
    ///
    /// Returns the fresh generation so `TraceItXModule.mm`'s `-configure:`
    ...
    private static func installExtraResolveHook() -> Int {
        ...
        TraceItX.__pendingExtraResolveHook = { await awaitJsExtraResolve() }
        return generation
    }
    ...
    /// `TraceItXModule.mm`'s `-configure:` calls this immediately after
    ...
    /// `NativeTraceItX.ts`'s `Spec.setExtraResolverActive`. `nonisolated
    ...
    /// `NativeTraceItX.ts`'s `Spec.setExtraResolverActive`.
    ...
    /// Installed onto `TraceItX.__pendingExtraResolveHook`. No-ops
    ...
    /// exactly: send `TraceItXEventEmitter.sendExtraResolveRequested`, then
    ...
    private static func awaitJsExtraResolve() async {
        ...
        await withCheckedContinuation { (cont: CheckedContinuation<Void, Never>) in
            ...
            // uses.
            TraceItXEventEmitter.sendExtraResolveRequested(correlationId)
    ...
    /// JS handshake ack: called once the `traceitx.extra.resolveRequested`
    ...
    /// `TraceItXModule.mm`'s own header comment), but `runtime.ts`'s
    ...
    /// instance's teardown unconditionally nil'd `TraceItX
    ...
    /// Android's `TraceItX.__pendingExtraResolveHook === extraResolveHook`
    /// reference check (`TraceItXModule.kt:570-574`). Android gets that
    /// identity for free because each module INSTANCE owns a distinct
    /// closure object it can compare by reference; `TraceItXBridge` is
    ...
    @objc public static func resetExtraResolverForTeardown(_ generation: Int) {
        ...
        guard isCurrentInstallation else {
            os_log("[traceitx] resetExtraResolverForTeardown: stale instance (generation %ld) — a newer configure() is already live; leaving it untouched",
        ...
        TraceItX.__pendingExtraResolveHook = nil
    ...
    /// `TXReporterPresenter.installResolver()` itself makes — and WAIT for
    ...
    /// `TXReporterPresenter.installResolver()` also wires the full modal
    ...
    /// header + `RelayWSClient`'s), and only `TXReporterPresenter` (the full
    ...
    /// Routes through `TraceItX.shared.report.open()` so any host-installed
    ...
    @objc public static func openReporter(
        ...
        os_log("[traceitx] openReporter.enter", log: log, type: .info)
        guard TraceItX.shared.captureGate else {
            completion(nil, NSError(
                domain: "TraceItXBridge", code: 1100,
                userInfo: [NSLocalizedDescriptionKey: "TraceItX not started (captureGate=false). Call configure() first."]
            ))
            return
        }
    ...
    public static func markSensitive(_ view: NSObject) {
        ...
        guard let uiView = view as? UIView else {
            os_log("[traceitx] markSensitive: expected UIView, got %{public}@; ignoring",
                   log: log, type: .error, String(describing: type(of: view)))
            return
        }
        MainActor.assumeIsolated {
            guard TraceItX.shared.captureGate else { return }
            os_log("[traceitx] markSensitive view=%{public}@", log: log, type: .info, uiView)
    ...
    /// `traceitx-react-native` (distinct from `CrashReporter.captureFacts`'s
    /// `"traceitx-ios"` default) so envelopes are attributable to the RN SDK.
    @objc public static func reportCrash(_ crashJson: NSString) -> Bool {
        CrashReporter.captureFacts(json: crashJson as String, sdkName: "traceitx-react-native")
    }
    ...
    @objc public static func captureHandledException(_ crashJson: NSString) -> Bool {
        CrashReporter.captureHandledFacts(json: crashJson as String, sdkName: "traceitx-react-native")
    }

    // MARK: - setExtra

    /// Forward an opaque string from JS into TraceItX.shared.setExtra.
    /// Empty string clears the pending extra.
    @objc public static func setExtra(_ value: NSString) {
        let s = value as String
        if s.isEmpty {
            TraceItX.shared.clearExtra()
        } else {
            TraceItX.shared.setExtra(s)
        }
    }
    ...
    /// `TraceItX.shared.addBreadcrumb(message:kind:level:data:)` (Task 5) —
    ...
    /// Forward a host-declared user from JS into `TraceItX.shared.setUser`.
    ...
    /// keeps the Task 9 email-only `TXUser` shape (nullable `id`) usable.
    ///
    ...
    /// `TraceItX.setUser` was reached, and `setUser({ id: 12345 })` therefore
    ...
    /// a partial (or empty) user and `TraceItX.shared.setUser` is ALWAYS
    ...
    @objc public static func setUser(_ user: NSDictionary?) {
        ...
        TraceItX.shared.setUser(TXUser(
    ...
    /// `TraceItX.shared.recordScreen(_:data:)` — NO validation/coercion
    ...
    // `TraceItXModule` instance, because React Native does not guarantee one module
    ...
    public static func markSensitiveViewOrWarn(_ view: NSObject?) {
        ...
        guard let v = view else {
            os_log("[traceitx] markSensitive: view not found for tag (registry no-op)",
                   log: log, type: .info)
            return
        }
        markSensitive(v)
    ...
    //   • Combine subscriptions on `TraceItX.shared.companion.$state`,
    //     `.$pairUrl`, `.$code` and `.$attachedUserName` that forward into
    //     `TraceItXEventEmitter`.
    //
    ...
    // both `TraceItX.shared.start` semantics and the RelayWSClient init/teardown
    // contract.
    ...
    /// `TraceItX.shared.companion` signals to the JS event emitter. Idempotent:
    ...
    @objc public static func startCompanion(_ endpoint: NSString) {
        ...
        if companionClient != nil {
            ...
            os_log("[traceitx] startCompanion ignored — already running; call stopCompanion() first",
                   log: log, type: .info)
            return
        }
        os_log("[traceitx] startCompanion (endpoint arg ignored)",
               log: log, type: .info)
        ...
        // `TXReporterPresenter.installResolver()` always installs
        ...
        TraceItX.shared.companion.__setBuiltinPinUiSuppressed(companionAttachPinUi != .builtin)
        ...
        let client = RelayWSClient(
            companion: TraceItX.shared.companion,
            ...
            companionDeviceId: TraceItX.shared.currentConfig?.companionDeviceId,
            ...
            companionBadge: CompanionBadgeOptions(
                enabled: TraceItX.shared.currentConfig?.companionBadgeEnabled ?? true,
                position: Self.parseCompanionBadgePosition(TraceItX.shared.currentConfig?.companionBadgePosition)
            )
        )
    ...
    /// when there isn't a usable one. Read straight off `TraceItX.shared`'s
    ...
    ///
    /// Returns nil when `TraceItX.start` never ran, and when the host
    ...
    private static func configuredSdkKey() -> String? {
        guard let appId = TraceItX.shared.currentConfig?.appId,
    ...
    /// `traceitx.companion.reportRequested` listener is ready for capture to
    ...
    /// because the underlying CompanionAPI singleton on `TraceItX.shared`
    ...
    @objc public static func stopCompanion() {
        os_log("[traceitx] stopCompanion", log: log, type: .info)
        companionLock.lock()
    ...
    /// Install Combine sinks on `TraceItX.shared.companion.$state`,
    /// `.$pairUrl`, `.$code` and `.$attachedUserName` that forward to
    /// `TraceItXEventEmitter`. Idempotent — the
    ...
    private static func installCompanionSubscriptionsIfNeeded() {
        ...
        companion.$state
            .sink { state in
                ...
                // TraceItXModule.kt.
                TraceItXEventEmitter.sendState(Self.rnString(for: state))
            }
            .store(in: &bag)
        companion.$pairUrl
            .sink { url in
                TraceItXEventEmitter.sendPairUrl(url)
            }
            .store(in: &bag)

        // Companion discovery (spec 2026-08-07). `CompanionState.swift` also
        // posts `.traceItXCompanionCodeChange` /
        // `.traceItXCompanionAttachedUserNameChange` for UIKit/ObjC hosts, but
        ...
        companion.$code
            .sink { code in
                TraceItXEventEmitter.sendCode(code)
            }
            .store(in: &bag)
        companion.$attachedUserName
            .sink { name in
                TraceItXEventEmitter.sendAttachedUserName(name)
            }
            .store(in: &bag)
        ...
        companion.$resolvedName
            .sink { name in
                TraceItXEventEmitter.sendResolvedName(name)
            }
            .store(in: &bag)
        ...
        companion.$attachChallenge
            .sink { challenge in
                TraceItXEventEmitter.sendAttachChallenge(challenge.map {
        ...
        .sink { note in
            ...
            TraceItXEventEmitter.sendReportRequested(corrId)
        }
        .store(in: &bag)
        ...
        companionLock.unlock()
        os_log("[traceitx] installed companion Combine subscriptions",
               log: log, type: .info)
    }

    /// Map `CompanionState` to the snake_case payload string the JS facade
    /// expects (`'unpaired' | 'paired' | 'report_in_progress' |
    /// 'phone_disconnected'`). MUST match `TraceItXModule.toRnString()` on
    ...
    @objc public static func _resetCompanionForTesting() {
        ...
        companionLock.unlock()
        TraceItX.shared.companion.__setBuiltinPinUiSuppressed(false)
    }
}
