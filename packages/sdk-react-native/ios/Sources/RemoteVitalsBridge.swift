...
//
// WHY THIS EXISTS. The five vitals shims used to hang off `EverframeBridge` as
...
//
// LIFETIME: one `RemoteVitalsBridge` per `EverframeModule` instance, created at
// module init and released with it. `reset()` runs from `-[EverframeModule
...
// bundle. A bare `Everframe.shared` restart does NOT reach here: the module
...
@objc public final class RemoteVitalsBridge: NSObject {
    ...
    private let remotePlayers = RemotePlayerRegistry(
        register: { integration, name in Everframe.shared.trackPlayer(integration, name: name) },
        sessionTrack: { name, data in Everframe.shared.trackVitals(name, data: data, player: nil) },
        captureSourceQuery: { Everframe.shared.currentConfig?.vitals.captureSourceQuery ?? false })
    ...
    /// Called from `-[EverframeModule invalidate]`. Idempotent.
