...
import com.traceitx.capture.video.VideoPrivacyAdapter
...
internal object RnVideoPrivacyAdapter : VideoPrivacyAdapter {
    override fun classify(view: View): Classification = try {
        if (view.getTag(com.facebook.react.R.id.view_tag_native_id) == "traceitx-sensitive")
...
internal class RnSensitiveRegistration(
    ...
    @Synchronized fun register(tag: Double) {
        if (destroyed) return
        if (token == null) token = TraceItX.__beginSensitiveRegistration()
                ...
                synchronized(this) {
                    ...
                    try {
                        ...
                        if (view == null) uncertain = true else {
                            ...
                            if (view.getTag(com.traceitx.R.id.tx_sensitive) != true) uncertain = true
                        }
