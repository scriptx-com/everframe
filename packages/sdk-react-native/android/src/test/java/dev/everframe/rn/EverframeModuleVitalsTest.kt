...
//
// EverframeModuleVitalsTest — Session Vitals (spec 2026-09-06), Android bridge
...
//      `Everframe.start(...)` again — a start SUPERSEDES the running SDK and detaches every
...
//
// Lives beside `EverframeModuleConfigureTest.kt` (and not in
// `EverframeModuleTest.kt`) for the reason that file's header spells out:
// `EverframeModuleTest.kt` is excluded from `compileDebugUnitTestKotlin` in
...
import dev.everframe.vitals.RemotePlayerRegistry
...
class EverframeModuleVitalsTest {
    ...
    @Before
    fun setUp() {
        // Mirrors EverframeModuleConfigureTest.kt exactly: RN's concrete
        ...
        Everframe.kill() // known-closed baseline before each test
    }
     ...
     * everframe-core and invisible from this Gradle module.)
     */
    @Test
    fun `invalidate closes this instance's registry and the reloaded bundle re-mints tokens on a new one`() {
        ...
        val reloaded = EverframeModule(reactContext)
     ...
     *
     * WHAT COUNTS AS THE OBSERVABLE START. `Everframe.startEpochChanged`/`currentStartEpoch`
     * would be the direct signal, but both are `internal` to `everframe-core` and therefore
     * invisible from this Gradle module. `Everframe.currentConfig` is public and is set to the
     * exact `EverframeConfig` INSTANCE each `start()` was handed, so object identity is a
     ...
     * `EverframeConfig` against the installed one, so this holds by construction.)
     */
    @Test
     ...
     * React instance, or two overlapping configures interleaving) starts `Everframe` with a
     ...
     * `Everframe.currentConfig` cannot drift: it IS the installed state.
     *
    ...
    fun `a start under the bridge is noticed - an unchanged configure restarts the SDK`() {
        ...
        val other = EverframeConfig(appId = "tx_other_key", sdkKey = "tx_other_key", release = "9.9.9")
     ...
     * rather than in `EverframeConfig`, so config equality alone is blind to it changing — and a
    ...
    private fun registryOf(m: EverframeModule): RemotePlayerRegistry =
        EverframeModule::class.java
