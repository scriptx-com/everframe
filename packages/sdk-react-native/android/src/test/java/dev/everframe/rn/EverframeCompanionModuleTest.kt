...
//   4. Pushing a `CompanionState` value into the `Everframe.companion.state`
...
// loop — that lives in `RelayWSClientTest.kt` in :everframe-core (Plan
...
import dev.everframe.companion.Companion
...
@Config(sdk = [33])
class EverframeCompanionModuleTest {
    ...
    fun setUp() {
        ...
        // `internal` to :everframe-core and unreachable from this separate
    ...
    /**
     * Contract 4: state changes on `Everframe.companion.state` reach the
    ...
    fun captureProvider_withNoForegroundActivity_returnsEmptyAssembled() {
        ...
        val payload = provider!!.invoke("corr_no_activity", Everframe.__replayFreeze())
    ...
    fun addListener_andRemoveListeners_areNoOps() {
        module.addListener("everframe.companion.state")
        module.addListener("everframe.companion.pairUrl")
