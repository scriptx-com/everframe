...
// used to have TWO independent owners: `TraceItX` (this SDK's core facade,
// `:traceitx-core`) and this module's own `companionClient` field. A hybrid
...
// `TraceItXModule`). The pure bookkeeping semantics of the two seams
// themselves, plus `TraceItX`'s own `startCompanionInternal`'s use of them,
// are covered from the OTHER side, in `:traceitx-core`'s own
// `TraceItXCompanionOwnershipTest.kt` — that module cannot reach
// `TraceItXModule` (dependency runs the other way), so the two halves cannot
// live in one file.
//
...
import com.traceitx.companion.CompanionBadge
...
class TraceItXCompanionOwnershipTest {
    ...
    /** The stand-in for a native host's own `TraceItX.startCompanion()` client —
    ...
    fun tearDown() {
        ...
        nativeFacadeClient?.let { TraceItX.__unregisterCompanionClient(it) }
    ...
    fun receivedReportOwnsRnStashBeforeStopAndRestart() = kotlinx.coroutines.runBlocking {
        ...
        TraceItX.start(activity, com.traceitx.config.TraceItXConfig(appId = "original", sdkKey = "txx_live_test1234567890"))
        val capture = TraceItX.__replayFreeze()
        val session = TraceItX.captureSessionSnapshot()
        TraceItXModule::class.java.getDeclaredMethod("installCompanionBridgeProviders").apply { isAccessible = true }.invoke(module)
        val client = RelayWSClient(client = OkHttpClient())
        assertTrue(TraceItX.__registerCompanionClient(client))
        TraceItXModule::class.java.getDeclaredField("companionClient").apply { isAccessible = true }.set(module, client)
        ...
        val screenshot = com.traceitx.capture.ScreenshotCapture.CaptureResult(bitmap, 2, 2, bytes)
        val stashType = TraceItXModule::class.java.declaredClasses.single { it.simpleName == "CompanionCapture" }
        ...
        val stashes = TraceItXModule::class.java.getDeclaredField("companionCaptureStash").apply { isAccessible = true }.get(module) as MutableMap<String, Any>
        stashes["original"] = stash
        val submit = kotlinx.serialization.json.Json { ignoreUnknownKeys = true }.decodeFromString(
            com.traceitx.protocol.generated.ReportSubmit.serializer(),
        ...
        val prepared = com.traceitx.companion.CompanionCaptureBridge.__submitProvider!!(submit, bytes, emptyList(), "original-attribution", capture, session)
        assertTrue("RN must transfer the stash synchronously", stashes.isEmpty())
        module.stopCompanion()
        capture.cancel()
        TraceItX.start(activity, com.traceitx.config.TraceItXConfig(appId = "replacement", sdkKey = "txx_live_replacement123456"))
        ...
        com.traceitx.companion.CompanionSubmissionComposer.__submitterFactoryForTesting = { cfg, outbox ->
            com.traceitx.transport.ReportSubmitter(cfg, outbox, com.traceitx.transport.MultipartUploader(OkHttpClient()), server.url("/").toString())
        }
        try {
            val result = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Default) { prepared() }
            assertTrue("$result", result is com.traceitx.companion.CompanionCaptureBridge.SubmitResult.Ok)
            ...
            assertFalse(body.contains("traceitx-video-v1"))
            ...
            com.traceitx.companion.CompanionSubmissionComposer.__submitterFactoryForTesting = null
            capture.cancel(); TraceItX.kill(); server.shutdown()
        }
    }

    /** Claims the shared slot exactly like a native host's own
     *  `TraceItX.startCompanion()` would have, without any real socket I/O. */
    private fun registerNativeFacadeClient(): RelayWSClient {
        val client = RelayWSClient(client = OkHttpClient())
        assertTrue("test setup: the stand-in native client must win the slot", TraceItX.__registerCompanionClient(client))
    ...
    fun stopCompanion_afterNativeFacadeStart_doesNotTouchTheNativeFacadesClientOrProviders() {
        ...
        assertFalse(
            ...
            TraceItX.__registerCompanionClient(RelayWSClient(client = OkHttpClient())),
        )
        ...
        TraceItX.__unregisterCompanionClient(native)
        val replacement = RelayWSClient(client = OkHttpClient())
        assertTrue(
            "releasing the native facade's own client must now succeed",
            TraceItX.__registerCompanionClient(replacement),
        )
        TraceItX.__unregisterCompanionClient(replacement)
    }
}

/**
 * Minimal `CatalystInstance` stand-in — see [TraceItXCompanionOwnershipTest.setUp]'s
 ...
 *  `TraceItXCompanionModuleTest.kt` for why that's out of scope). */
