...
//
// Robolectric unit tests for TraceItXModule — the Android half of the RN
...
//      envelope into the existing JSONLOutbox at `cacheDir/com.traceitx/`.
//
// NOTE ON CODEGEN AVAILABILITY:
//   The abstract base class `com.facebook.fbreact.specs.NativeTraceItXSpec` is
//   produced by the `com.facebook.react` Gradle codegen plugin from
//   `packages/sdk-react-native/src/NativeTraceItX.ts` — it only materialises
//   inside a consuming RN host app's build. For library-side JVM unit tests we
//   therefore exercise `TraceItXModule` through its concrete bridge surface
...
class TraceItXModuleTest {
    ...
    fun setUp() {
        ...
        // no longer exist on TraceItXModule; onNewIntent/Promise.reject
