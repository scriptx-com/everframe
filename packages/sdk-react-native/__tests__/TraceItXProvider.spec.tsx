...
describe("TraceItXProvider — slim runtime (D-05/D-07 flip)", () => {
  beforeEach(() => {
  ...
  it("uses legacy asynchronous configuration only when configureSync is absent", () => {
    ...
    Object.defineProperty(NativeTraceItX, "configureSync", {
      configurable: true,
      value: undefined,
    });
    try {
      ...
      Object.defineProperty(NativeTraceItX, "configureSync", descriptor);
    }
  });
  ...
  it("open() delegates to NativeTraceItX.openReporter and returns its result", async () => {
  ...
  it("sensitive.register forwards to NativeTraceItX.registerSensitiveRect", () => {
    const rt = createRuntime({ apiKey: "k" });
    rt.mount();
  ...
  it("double-mount throws TraceItXNotMountedError (single-instance enforcement)", () => {
  ...
  describe("addBreadcrumb (Task 14)", () => {
    ...
    it("rt.addBreadcrumb forwards to NativeTraceItX.addBreadcrumb with positional args in order", () => {
      const rt = createRuntime({ apiKey: "k" });
      rt.mount();
      rt.addBreadcrumb({
