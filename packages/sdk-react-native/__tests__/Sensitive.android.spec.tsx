...
vi.mock('../src/TraceItXProvider.js', () => ({ useTraceItX: () => ({ sensitive: { register: state.register } }) }));
vi.mock('react-native', () => ({ Platform: { OS: 'android' }, View: 'View', requireNativeComponent: (name: string) => name }));
import { TraceItXSensitive } from '../src/Sensitive.js';
...
describe('Android sensitive mount contract', () => {
  it('uses one constructor-marked native host and preserves caller props before layout', () => {
    ...
    const result = TraceItXSensitive({ nativeID: 'caller', collapsable: true, style, accessible: true, accessibilityLabel: 'private group', onLayout, children: child });
    expect(result.type).toBe('TraceItXSensitiveView');
