...
class ReporterVideoBudgetTest {
    @Test fun fiveShotsOmitOwnedVideoButFourShotsKeepIt() = runBlocking {
        for (shotCount in listOf(5, 4)) {
            ...
            TraceItX.start(activity, TraceItXConfig("a", "txx_live_test1234567890", capture = CaptureConfig(logs = false)))
            ...
            try {
                ...
                assertEquals(shotCount == 4, env.toString().contains("traceitx-video-v1"))
                ...
                TraceItX.kill(); server.shutdown(); root.deleteRecursively()
            }
        }
    }
}
