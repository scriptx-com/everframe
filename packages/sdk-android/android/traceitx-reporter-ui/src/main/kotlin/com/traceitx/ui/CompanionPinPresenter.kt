...
//
// `:traceitx-reporter-ui` declares no `res/values/strings.xml` (checked —
...
internal object CompanionPinPresenter {
    ...
    fun install() {
        if (installed) return
        installed = true
        TraceItX.__attachPinUiInstalled = true
        scope.launch {
            com.traceitx.companion.Companion.attachChallenge.collect { challenge ->
                if (challenge == null) {
    ...
    fun onActivityResumed() {
        val challenge = com.traceitx.companion.Companion.attachChallenge.value
    ...
    private fun present(challenge: AttachChallengeInfo, deadlineElapsedRealtime: Long) {
        ...
        if (TraceItX.__attachPinUiSuppressed) {
            dismiss()
            return
        }
        dismiss()
        ...
        val content = LinearLayout(activity).apply {
            ...
            addView(
                TextView(activity).apply {
                    text = "Enter this code in the TraceItX dashboard"
                    textSize = 12f
                },
            )
        }
