...
//
// Android TV note: the on-device :traceitx-tv Activity was removed. Reporting
...
import com.traceitx.capture.SensitiveRectRegistry
...
internal class TXReporterPresenter(
    ...
    private val showDialog: suspend (Activity, ScreenshotCapture.CaptureResult, com.traceitx.capture.video.FrozenReportCapture, String?) -> ReportResult =
    ...
    suspend fun openReporter(activity: Activity): ReportResult {
        ...
        var submitted = false
        var frozenCapture: com.traceitx.capture.video.FrozenReportCapture? = null
        try {
        val result: ReportResult? = txGuardSuspend("presenter") {
            // DEFE-03 kill-switch
            if (!TraceItX.captureGate) return@txGuardSuspend ReportResult.Cancelled("kill_switch")
            ...
            // `TraceItX.attachReactTree` is still callable by pure-native
            // hosts, so the pending slot must still be cleared or a stale
            // attach would outlive the report that caused it.
            val (hostExtra, _) = TraceItX.consumePendingAttachments()
            ...
            val reportCapture = TraceItX.__replayFreeze()
            ...
            //     permanently anonymous — see `TraceItX.__warmIdentityToken`'s
            ...
            //    hand off to a separate :traceitx-tv Activity; that module
