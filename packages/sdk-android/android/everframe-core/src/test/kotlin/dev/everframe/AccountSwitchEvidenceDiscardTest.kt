...
// `Everframe.kt`'s `setIdentityToken` for the full account and
...
class AccountSwitchEvidenceDiscardTest {
    ...
    private fun makeOutbox(): JSONLOutbox = JSONLOutbox(File(tmp.newFolder("dev.everframe"), "outbox.jsonl"), keys = JceTestOutboxKeyProvider(), ops = JvmOutboxFileOps())
    ...
    fun `an account switch to a different verified identity does not discard the previous users evidence`() {
        ...
        Everframe.setIdentityToken(IdentityTokenSource.Token(jwt(sub = "alice", expMs = System.currentTimeMillis() + 300_000)))
        sharedBreadcrumbBuffer.add(kind = BreadcrumbKind.Tap, message = "alice-breadcrumb-marker")
        sharedLogBuffer.push(dev.everframe.capture.LogRingBuffer.Entry(
        ...
        val identityToken = runBlocking {
            resolveIdentityHeader(
                ...
                config = Everframe.currentReplayConfig(),
