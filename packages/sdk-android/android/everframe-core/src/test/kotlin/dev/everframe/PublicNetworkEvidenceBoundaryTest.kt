...
class PublicNetworkEvidenceBoundaryTest {
    ...
    private fun drive(kill: Boolean, fail: Boolean, atInsertion: Boolean = false) {
        ...
        Everframe.__replaySessionFactoryForTesting = { _, cfg, epoch, consent, _ ->
        ...
        fun start(project: String) {
            Everframe.start(context, config.copy(appId = project))
            val deadline = System.nanoTime() + TimeUnit.SECONDS.toNanos(5)
            while (Everframe._replaySession == null && System.nanoTime() < deadline) Thread.yield()
            assertNotNull(Everframe._replaySession)
            runBlocking { Everframe._replaySession!!.refreshConfigNow() }
        ...
        val client = OkHttpClient.Builder().addEverframeInterceptor().addInterceptor { chain ->
        ...
        try {
            ...
            Everframe.__resetStartTailDelayHookForTesting(); Everframe.kill()
