...
//
// Plan 5 Task 1 (hygiene carry-over) — proves `Everframe.start()` actually
...
// `Everframe.kt` — the whole Android log pipeline (and the Task-11 console
...
// iOS `Everframe.swift:167-168` (`if config.capture.logs { LogCapture.install() }`)
...
//     polls (bounded ~2s) on `Everframe._replaySession` becoming non-null,
...
//   • `@After` uninstalls LogCapture, kills Everframe, and resets the shared
...
class EverframeLogWiringTest {

    private val context: Context get() = ApplicationProvider.getApplicationContext()

    private fun configWith(logs: Boolean): EverframeConfig = EverframeConfig(
        appId = "test-app-id",
    ...
    private fun awaitHeavyInit(timeoutMs: Long = 2_000) {
        ...
        assertNotNull("heavy init did not publish its replay session", Everframe._replaySession)
    }
    ...
    fun `start with capture logs wires LogCapture and console crumbs flow end-to-end`() {
        Everframe.start(context, configWith(logs = true))
        awaitHeavyInit()
    ...
    fun `start with capture logs disabled installs nothing`() {
        Everframe.start(context, configWith(logs = false))
        awaitHeavyInit()
    ...
    fun `start tail released after kill cannot reinstall log capture`() {
        ...
        val delayedContext = object : ContextWrapper(context) {
            override fun getApplicationContext(): Context {
                ...
                if (Thread.currentThread() !== caller &&
                    Thread.currentThread().stackTrace.any { it.className == "dev.everframe.crash.CrashReporter" && it.methodName == "configure" } &&
        ...
        val previousChildren = Everframe.sdkScope.coroutineContext[Job]!!.children.toSet()
        Everframe.start(delayedContext, configWith(logs = true))
        val children = Everframe.sdkScope.coroutineContext[Job]!!.children.filterNot { it in previousChildren }.toList()
        try {
    ...
    fun `admitted old callback cannot insert logs or console crumbs after kill and start`() {
        Everframe.start(context, configWith(logs = true)); awaitHeavyInit()
        ...
        try {
            ...
            Everframe.kill()
            Everframe.start(context, configWith(logs = true).copy(appId = "project-b")); awaitHeavyInit()
    ...
    fun `signal collector reentrant start preserves the newer inline theme and capture`() {
        Everframe.start(context, configWith(logs = true)); awaitHeavyInit()
        val signal = dev.everframe.config.BrandingServerConfigSignal.flow
        signal.value = dev.everframe.config.BrandingConfigWire(watermark = false)
        val theme = dev.everframe.config.ReporterThemeOptions(accent = "#123456")
        ...
        val collector = kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.Unconfined).launch {
            signal.collect { value ->
                if (value == null && replaceOnce.compareAndSet(true, false)) {
                    Everframe.start(context, configWith(logs = true).copy(appId = "B", theme = theme))
                }
            }
        }
        try {
            Everframe.kill(); awaitHeavyInit()
            assertFalse("collector must have reentered public start", replaceOnce.get())
            org.junit.Assert.assertEquals(theme, dev.everframe.config.BrandingInlineTheme.flow.value)
