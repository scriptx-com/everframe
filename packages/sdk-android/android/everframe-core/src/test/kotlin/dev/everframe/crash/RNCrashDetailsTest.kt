...
class RNCrashDetailsTest {
    ...
    @Test fun `raw details are unread while ineligible and nested capture is refused under guard`() {
        ...
        dev.everframe.shared.SharedData.init(context)
        CrashReporter.__resetForTesting()
        dev.everframe.Everframe.__setConfigForTesting(null)
        try {
            assertFalse(capture())
            assertEquals(0, reads)
            CrashReporter.configure(context)
            val cfg = dev.everframe.config.EverframeConfig(appId = "rn-test", sdkKey = "synthetic")
            dev.everframe.Everframe.__setConfigForTesting(cfg)
            dev.everframe.Everframe.captureGate = true
            CrashReporter.sidecarFactory = { dev.everframe.outbox.CrashSidecar(java.io.File(dir, "crash-outbox.jsonl"), keys, ops) }
            assertTrue(capture())
            assertTrue(reads > 0)
            raw.optJSONObject("metadata")!!.put("n", 99)
            val entries = kotlinx.coroutines.runBlocking { dev.everframe.outbox.JSONLOutbox(java.io.File(dir, "outbox.jsonl"), keys, ops).hydrate() }
            ...
            dev.everframe.Everframe.__setConfigForTesting(cfg.copy(capture = cfg.capture.copy(crash = false)))
            val before = reads
            assertFalse(capture())
