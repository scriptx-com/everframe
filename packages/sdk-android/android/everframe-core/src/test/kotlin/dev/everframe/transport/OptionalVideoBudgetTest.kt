...
class OptionalVideoBudgetTest {
    ...
    private fun envelope(parts: List<MultipartUploader.Part>) = buildJsonObject {
        ...
        putJsonArray("attachments") { parts.forEach { part -> add(buildJsonObject {
            ...
            if (part.name == "replay") put("format", "everframe-video-v1")
        }) } }
    ...
    private suspend fun queueRoundtrip(existingSixAttachmentEntry: Boolean, byteOverflow: Boolean = false) {
        ...
        try { withServer { server ->
            ...
            val refs = parts.map { OutboxEntry.AttachmentRef(it.name, it.filename!!, it.contentType, it.data, dev.everframe.capture.video.sha256(it.data)) }
            val config = EverframeConfig("app", "original-key")
            ...
            if (existingSixAttachmentEntry) {
                ...
                val result = submitter.submit(env, "original-idem", parts.map {
                    ReportSubmitter.Attachment(it.name, it.filename!!, it.contentType, it.data, dev.everframe.capture.video.sha256(it.data))
        ...
        } } finally { Everframe.captureGate = false; root.deleteRecursively() }
    }
    ...
    private fun assertDegraded(request: RecordedRequest, count: Int, reason: String) {
        ...
        assertFalse(body.contains("video/mp4")); assertFalse(body.contains("everframe-video-v1"))
