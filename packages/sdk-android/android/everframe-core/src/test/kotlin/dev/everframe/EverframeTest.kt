...
//
// Public-API tests for `object Everframe` — start / kill / setUser bodies +
...
import dev.everframe.config.EverframeConfigError
...
class EverframeTest {
    ...
    private fun validConfig(env: Environment = Environment.production): EverframeConfig =
        EverframeConfig(
            appId = "test-app-id",
            ...
            // EverframeLogWiringTest, which has an airtight awaitHeavyInit +
            ...
            // does NOT await that coroutine before calling Everframe.kill(),
    ...
    private class RecordingPlayerIntegration : dev.everframe.vitals.PlayerIntegration {
    ...
    private fun vitalsControllerForTest() = dev.everframe.vitals.VitalsController(
        dev.everframe.vitals.VitalsController.Deps(
            localConfig = dev.everframe.config.VitalsConfig(),
            dims = dev.everframe.vitals.wire.SessionSummaryDims("android", "1", "test"),
            transport = {
                object : dev.everframe.vitals.VitalsSink {
                    override fun send(body: String) = Unit
                    override fun close() = Unit
                }
            },
            scheduler = object : dev.everframe.vitals.VitalsScheduler {
                override fun repeat(intervalMs: Long, tick: () -> Unit) = AutoCloseable { }
            },
            samplerFactory = { onSample, onTick ->
                object : dev.everframe.vitals.ResourceSampler(
    ...
    fun setUp() {
        ...
        dev.everframe.vitals.VitalsServerConfigSignal.resetForTesting()
        dev.everframe.vitals.VitalsRuntime.resetForTesting()
    ...
    fun tearDown() {
        ...
        dev.everframe.config.BrandingServerConfigSignal.resetForTesting()
        dev.everframe.config.BrandingInlineTheme.resetForTesting()
        dev.everframe.vitals.VitalsServerConfigSignal.resetForTesting()
        dev.everframe.vitals.VitalsRuntime.resetForTesting()
    }
    ...
    fun `start flips captureGate to true and stores config`() {
        ...
        assertTrue("captureGate should be true after start()", Everframe.captureGate)
        assertEquals(cfg, Everframe.currentConfig)
    }
    ...
    fun `start completes synchronously (no host-thread blocking)`() {
        ...
        val cfg = validConfig()
        val elapsed = kotlin.system.measureTimeMillis { Everframe.start(context, cfg) }
        assertTrue(
    ...
    fun `start throws BlankSdkKey on blank sdkKey`() {
        ...
        assertThrows(EverframeConfigError.BlankSdkKey::class.java) {
            Everframe.start(context, bad)
        }
    }
    ...
    fun `kill flips captureGate to false and is idempotent`() {
        ...
        Everframe.kill()
        assertFalse(Everframe.captureGate)
        assertNull(Everframe.currentConfig)
    }
    ...
    fun `kill clears the replay-config test override`() {
        ...
        assertNotNull(Everframe.__replayConfigOverrideForTesting)

        Everframe.kill()

        assertNull(
            "kill() must clear __replayConfigOverrideForTesting like every other piece of session state",
            Everframe.__replayConfigOverrideForTesting,
        )
    }
    ...
    fun `push after kill cannot repopulate the resource ring regardless of ordering with teardown`() {
        ...
        // `Everframe.captureGate` (mirroring NetworkRingBuffer's F11/F15
    ...
    fun `kill resets the network-body sampling gate so a fresh start redraws`() {
        ...
        NetworkBodyCaptureState.applyConfig(
            dev.everframe.config.NetworkBodiesConfigWire(captureBodies = true),
    ...
    fun `setUser stores user when captureGate is open`() {
        ...
        Everframe.setUser(user)
        assertEquals(user, Everframe.currentUser)
    }
    ...
    fun `start with a new configuration clears the previous session's user`() {
        Everframe.start(context, validConfig())
        Everframe.setUser(TXUser(id = "u-1", email = "test@example.com", displayName = "A"))
    ...
    fun `an identical restart also clears the user`() {
        Everframe.start(context, validConfig())
        Everframe.setUser(TXUser(id = "u-1"))

        Everframe.start(context, validConfig())
        assertNull("a same-key restart clears the user too", Everframe.currentUser)
    }
     ...
     * as `_user = null` (Everframe.kt, ~L558), so — unlike the heavy-init
    ...
    fun `start with a new configuration clears the previous session's companion-badge server override`() {
        Everframe.start(context, validConfig())
        dev.everframe.companion.CompanionBadgeServerConfigSignal.flow.value =
            dev.everframe.config.CompanionBadgeConfigWire(enabled = false, position = "top-left")
        assertNotNull(
        ...
        dev.everframe.companion.CompanionBadgeServerConfigSignal.resetForTesting()
    }
    ...
    fun `kill clears the companion-badge server override`() {
        Everframe.start(context, validConfig())
        dev.everframe.companion.CompanionBadgeServerConfigSignal.flow.value =
            dev.everframe.config.CompanionBadgeConfigWire(enabled = true, position = "top-left")
        assertNotNull(
    ...
    fun `start with a new configuration clears the previous session's branding server signal`() {
        Everframe.start(context, validConfig())
        dev.everframe.config.BrandingServerConfigSignal.flow.value =
            dev.everframe.config.BrandingConfigWire(watermark = false)
        assertNotNull(
    ...
    fun `start tears the previous session's vitals controller down and clears the vitals server signal`() {
        ...
        Everframe.start(context, validConfig().copy(sdkKey = "txx_live_other0987654321"))
    ...
    fun `a report built the instant start(B) returns carries none of project A's vitals`() {
        ...
        assertNotNull(
            ...
            dev.everframe.vitals.VitalsRuntime.currentStamp(),
        )
        ...
        assertNull(dev.everframe.vitals.VitalsRuntime.currentStamp())
        val envelope = kotlinx.serialization.json.Json.parseToJsonElement(
            String(
                dev.everframe.envelope.EnvelopeBuilder().buildEncoded(sdkVersion = "0.8.0").bytes,
                Charsets.UTF_8,
            ),
        ).jsonObject
        assertFalse(
    ...
    fun `a trackPlayer call made before start is attached by the session start installs`() {
        // Codex round-3, Critical 1, end to end. `Everframe.trackPlayer`'s KDoc
        ...
        val integration = RecordingPlayerIntegration()
        val handle = Everframe.trackPlayer(integration, "main")
    ...
    fun `a start superseded while tearing the previous session down publishes nothing`() {
        ...
        val blocking = object : dev.everframe.vitals.PlayerIntegration {
            override val library = "fake"; override val version: String? = null
            override fun attach(ctx: dev.everframe.vitals.PlayerIntegrationContext) = true
            override fun snapshot(onResult: (dev.everframe.vitals.PlayerSnapshot?) -> Boolean) { onResult(null) }
            override fun startupTimings(): dev.everframe.vitals.StartupTimings? = null
        ...
        val b = Thread { Everframe.start(context, validConfig().copy(sdkKey = "txx_live_bbbbbbbbbbbbbb")) }
        b.start()
        ...
        Everframe.start(context, validConfig().copy(sdkKey = "txx_live_cccccccccccccc"))
        assertEquals("precondition: C published its own key", "txx_live_cccccccccccccc", Everframe.currentConfig!!.sdkKey)
    ...
    fun `the vitals transport kill predicate never blocks on stateLock`() {
        ...
        val release = java.util.concurrent.CountDownLatch(1)
        val holder = Thread {
            Everframe.__withStateLockForTesting {
                held.countDown()
                release.await(5, java.util.concurrent.TimeUnit.SECONDS)
            }
        }
        ...
        try {
            // The predicate the transport actually ships (Everframe.kt), run on
            // a collector-like thread while `stateLock` is occupied.
            val answered = java.util.concurrent.CountDownLatch(1)
            val flush = Thread {
                if (Everframe.currentStartEpochVolatile() == epoch) answered.countDown()
            }
            flush.start()
            assertTrue(
            ...
            val blocked = Thread { Everframe.currentStartEpoch(); locked.countDown() }
            blocked.start()
            assertFalse(
    ...
    fun `kill clears the branding server signal and the inline theme`() {
        ...
        Everframe.start(
            context,
            validConfig().copy(theme = dev.everframe.config.ReporterThemeOptions(accent = "#336699")),
        )
        dev.everframe.config.BrandingServerConfigSignal.flow.value =
            dev.everframe.config.BrandingConfigWire(watermark = false)
        assertNotNull(
        ...
        assertNotNull(
            ...
            dev.everframe.config.BrandingInlineTheme.flow.value,
        )
    ...
    fun `start seeds the inline theme from the validated config`() {
        val theme = dev.everframe.config.ReporterThemeOptions(accent = "#336699")
        Everframe.start(context, validConfig().copy(theme = theme))
    ...
    fun `setUser is no-op when captureGate is closed`() {
        ...
        Everframe.setUser(TXUser(id = "u-1"))
        assertNull(Everframe.currentUser)
    }
    ...
    fun `SDK_VERSION is a non-blank SemVer string sourced from BuildConfig`() {
        ...
        // generates from the everframeVersion property. Verify the wiring
        // (non-blank, SemVer shape) rather than a literal value — otherwise
        // every release bump silently breaks the test.
        assertTrue(
            "SDK_VERSION must be SemVer (X.Y.Z[-SUFFIX]): got '${Everframe.SDK_VERSION}'",
            Regex("""^\d+\.\d+\.\d+(-[A-Za-z0-9.-]+)?$""").matches(Everframe.SDK_VERSION),
        )
        assertEquals(BuildConfig.SDK_VERSION, Everframe.SDK_VERSION)
    }

    @Test
    fun `resolver-indirection slots default to null`() {
        assertNull(Everframe.__reporterTriggersInstaller)
        assertNull(Everframe.__reporterTriggersTeardown)
    ...
    fun `start invokes __reporterTriggersInstaller when set`() {
        var invoked = false
        Everframe.__reporterTriggersInstaller = { _, _ -> invoked = true }
        Everframe.start(context, validConfig())
        ...
        assertNotNull(Everframe.__reporterTriggersInstaller)
        // Cleanup
        Everframe.__reporterTriggersInstaller = null
     ...
     * `EverframeLogWiringTest.awaitHeavyInit()`.
     */
