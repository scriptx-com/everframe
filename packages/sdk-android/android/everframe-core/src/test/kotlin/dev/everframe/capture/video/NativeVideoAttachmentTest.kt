...
class NativeVideoAttachmentTest {
 @Test fun normalAttachmentContainsMatchingBoundedBytesAndClosesClip()=runBlocking {
  ...
  assertEquals(Format.EverframeVideoV1,pair.envelope.format);assertEquals(AttachmentKind.SessionReplay,pair.envelope.kind)
 ...
 @Test fun realMultipartCarriesSharedVideoMetadataAndExactMedia()=runBlocking {
  val h=VideoExporterTest.Harness();val clip=h.exporter.export(h.owner,listOf(h.segment())){true}!!
  val pair=NativeVideoAttachment(h.scheduler).build(clip)!!
  val json=kotlinx.serialization.json.Json.encodeToString(dev.everframe.protocol.generated.Attachment.serializer(),pair.envelope)
  ...
  try {
   ...
   val part=pair.part
   dev.everframe.transport.MultipartUploader(okhttp3.OkHttpClient()).upload(
    endpoint=server.url("/ingest").toString(),sdkKey="test",idempotencyKey="test",
    envelopeBytes=("{\"attachments\":["+json+"]}").toByteArray(),
    attachments=listOf(dev.everframe.transport.MultipartUploader.Part(part.name,part.filename,part.data,part.contentType)))
   ...
   assertEquals("\"everframe-video-v1\"",metadata["format"].toString())
   ...
   assertEquals(pair.envelope,kotlinx.serialization.json.Json.decodeFromString(dev.everframe.protocol.generated.Attachment.serializer(),metadata.toString()))
