...
class NativeVideoSessionTest {
    ...
    @Test fun staleCaptureCannotConsumeReplacementDataAndFailureRevokesOnlyVideo() = runBlocking {
        ...
        replacement.finishConsumption(); b.teardown(); Everframe.captureGate = false
    }
    private fun networkBody(value: String) = dev.everframe.protocol.generated.NetworkBody(
    ...
    @Test fun backgroundAndIdentityChangesPreserveFrozenAuthorizationUntilConsumption() = runBlocking {
        ...
        session.teardown(); assertFalse(capture.replayAllowed()); Everframe.captureGate = false
    }
    ...
    @Test fun failedRefreshNeverUsesTtlSkipToAuthorizeStaleVideo() = runBlocking {
        ...
        capture.cancel(); session.teardown(); Everframe.captureGate = false
    }
    ...
    @Test fun slotsRejectNewOptionalCapturesAndReleaseWithoutEviction() = runBlocking {
        ...
        sessions.forEach { it.teardown() }; Everframe.captureGate = false
    }

    @Test fun sourceEpochIsCheckedInsideSnapshotLock() {
        val buffer = dev.everframe.capture.BreadcrumbRingBuffer()
        ...
        val result = java.util.concurrent.atomic.AtomicReference<List<dev.everframe.protocol.generated.Breadcrumb>?>()
    ...
    @Test fun consumedOwnersLateCancellationDeniesReplayWithoutTouchingNextCapture() = runBlocking {
        ...
        a.cancel(); a.cancel()
        assertEquals(dev.everframe.transport.ReportAuthorizationDecision(true, false), authority.evaluate())
    ...
    private fun coordinatorRevokesInsideRemux(privacy: Boolean) = runBlocking {
        ...
        } finally { release.countDown(); capture.cancel(); session.teardown(); Everframe.captureGate = false }
    }
