...
//
// JVM unit tests for EverframeInterceptor. We use OkHttp's MockWebServer for a
...
class EverframeInterceptorTest {
    ...
    private fun client(): OkHttpClient =
        OkHttpClient.Builder().addEverframeInterceptor().build()
    ...
    @Test
    fun `addEverframeInterceptor_is_chainable_and_returns_same_builder`() {
        val builder = OkHttpClient.Builder()
        val returned = builder.addEverframeInterceptor()
        assertSame("addEverframeInterceptor must return the same builder for chaining", builder, returned)
        val client = builder.build()
        val ours = client.interceptors.count { it is EverframeInterceptor }
        assertEquals(1, ours)
    }
