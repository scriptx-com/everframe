...
// defect this pins: `Everframe.captureSessionSnapshot()` reads `_config`/
...
// still regress silently is the WIRING: a future edit to `Everframe.kt`'s
...
class EverframeConfigSnapshotWiringSourceGateTest {

    /**
     * Reads `Everframe.kt` as text — no Robolectric, no `Everframe` singleton
    ...
    private fun traceItXSource(): String {
        val candidates = listOf(
            File("src/main/kotlin/dev/everframe/Everframe.kt"),
            File("everframe-core/src/main/kotlin/dev/everframe/Everframe.kt"),
            File("android/everframe-core/src/main/kotlin/dev/everframe/Everframe.kt"),
        )
        val file = candidates.firstOrNull { it.exists() }
            ?: error(
                "Everframe.kt not found relative to ${File(".").absolutePath} — tried " +
    ...
    fun `requestOutboxDrain reads config and epoch from one atomic snapshot not two separate reads`() {
        ...
        assertTrue(
            "Everframe.kt's requestOutboxDrain() no longer captures config and epoch together via " +
        ...
        assertTrue(
            "Everframe.kt no longer resolves `cfg` from captured.config — has requestOutboxDrain() " +
                "been restructured?",
        ...
        assertTrue(
            "Everframe.kt's requestOutboxDrain() no longer threads epochAtInitiation from " +
