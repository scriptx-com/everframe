...
// `.testStartClearsTheIdentityToken`) — proves `Everframe.start()` and
// `Everframe.kill()` actually clear `Everframe._identityHolder`, not merely
...
class EverframeIdentityTokenTest {
    ...
    private fun validConfig(): EverframeConfig = EverframeConfig(
        ...
        // Mirrors EverframeTest.validConfig()'s rationale: keep the detached
    ...
    fun `kill clears the identity token`() {
        ...
        val got = runBlocking { Everframe._identityHolder.get(now) }
    ...
    fun `resolveIdentityToken withholds the header when another project starts during resolution`() {
        ...
        val resultRef = AtomicReference<Everframe.IdentityResolution?>(null)
        val resolveThread = Thread {
            resultRef.set(
                runBlocking { Everframe.__resolveIdentityToken(captured.identitySubject, captured.startEpoch) },
            )
        }
        resolveThread.start()
        ...
        Everframe.start(context, validConfig().copy(sdkKey = "txx_live_projectB098765432"))
        Everframe.setIdentityToken(
