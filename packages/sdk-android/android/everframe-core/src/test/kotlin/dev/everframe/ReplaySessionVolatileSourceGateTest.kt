...
//
// `Everframe._replaySession` is written under `stateLock` (`start()`'s
...
//
// This mirrors the round-9 precedent (`EverframeConfigSnapshotWiringSourceGateTest`)
...
class ReplaySessionVolatileSourceGateTest {
    ...
    fun `_replaySession carries Volatile so currentReplayConfig's lock-free read has a publication guarantee`() {
        ...
        assertTrue(
            "Everframe.kt's _replaySession field declaration was not found as expected — has it been renamed " +
        ...
        assertTrue(
            "Everframe.kt's _replaySession is no longer annotated @Volatile immediately before its " +
