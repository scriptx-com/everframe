...
import dev.everframe.config.ReplayConfigProvider.Companion.SDK_FEATURES_HEADER_VALUE
...
class VitalsConfigWireTest {
    ...
    fun `validator rejects an out-of-range local sample rate`() {
        val bad = EverframeConfig(appId = "a", sdkKey = "k", vitals = VitalsConfig(sampleRate = 1.5))
        val err = runCatching { ConfigValidator.validate(bad) }.exceptionOrNull()
        assertTrue(err is EverframeConfigError.InvalidVitalsSampleRate)
        ConfigValidator.validate(EverframeConfig(appId = "a", sdkKey = "k", vitals = VitalsConfig(sampleRate = 0.0)))
    }
}
