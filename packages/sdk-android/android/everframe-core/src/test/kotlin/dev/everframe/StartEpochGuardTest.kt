...
// `Everframe._replaySession` synchronously (no suspension point in that
...
//
// Fixed the same way as iOS (Everframe.swift, commit 826e5f76): a
...
//
// `Everframe.__startTailDelayHookForTesting` parks the tail (via a real
...
class StartEpochGuardTest {
    ...
    private fun validConfig(): EverframeConfig = EverframeConfig(
        ...
        // Mirrors EverframeTest.validConfig()'s rationale: keep the
    ...
    fun `kill racing an in-flight start discards the stale ReplaySession install`() {
        ...
        assertFalse("kill() must leave captureGate closed", Everframe.captureGate)
    }
    ...
    fun `start after the raced kill still installs its own session normally`() {
        ...
        releaseTail.countDown()
        Thread.sleep(500)
        assertNull(Everframe._replaySession)
    ...
    fun `a start whose epoch has moved tears down nothing`() {
        Everframe.start(context, validConfig())
        val staleEpoch = Everframe.currentStartEpoch()
        ...
        assertSame(live, Everframe.takeSupersededReplay(Everframe.currentStartEpoch()))
        assertNull(Everframe._replaySession)
    }

    private fun awaitReplaySession(why: String) {
        val deadline = System.currentTimeMillis() + 5_000
        while (Everframe._replaySession == null && System.currentTimeMillis() < deadline) Thread.sleep(10)
        assertNotNull(why, Everframe._replaySession)
    }

    @Test
    fun `start does not bump killGeneration`() {
        Everframe.start(context, EverframeConfig(appId = "app_a", sdkKey = "sk_a"))
        val before = Everframe.captureSessionSnapshot().killGeneration
        Everframe.start(context, EverframeConfig(appId = "app_b", sdkKey = "sk_b"))
        assertFalse(
            "start() must not look like a revocation",
            Everframe.killGenerationChanged(before),
        )
    }
    ...
    fun `kill bumps killGeneration`() {
        ...
        val before = Everframe.captureSessionSnapshot().killGeneration
        Everframe.kill()
        assertTrue(Everframe.killGenerationChanged(before))
    }
    ...
    fun `snapshot pairs the user with the config of the same session`() {
        Everframe.start(context, EverframeConfig(appId = "app_a", sdkKey = "sk_a"))
        Everframe.setUser(TXUser(id = "u_1"))
        val snap = Everframe.captureSessionSnapshot()
