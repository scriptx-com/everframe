...
class HandledThrowableCaptureTest {
    ...
    private val config = EverframeConfig(appId = "app", sdkKey = "sk", r8MappingId = "mapping-A")
    private val startThreads = mutableListOf<Pair<Thread, java.util.concurrent.CountDownLatch>>()
    private val appContextField = Everframe::class.java.getDeclaredField("appContext").apply { isAccessible = true }
    ...
    @After fun tearDown() {
        if (startThreads.isNotEmpty()) Everframe.kill()
    ...
    @Test fun `public capture durably stores a handled nonfatal error`() {
        Everframe.captureException(IllegalStateException("handled probe", IllegalArgumentException("cause")))
    ...
    @Test fun `deliberate capture waiting on storage is rejected by kill`() {
        ...
        try {
            ...
            kill = Thread { Everframe.kill() }.also { it.start() }
            val deadline = System.nanoTime() + java.util.concurrent.TimeUnit.SECONDS.toNanos(2)
            while (Everframe.captureGate && System.nanoTime() < deadline) Thread.yield()
            assertFalse("kill must revoke capture before storage is released", Everframe.captureGate)
        } finally {
            release.countDown()
            holder.join(5_000)
            capture.join(5_000)
    ...
    @Test fun `deliberate capture refuses same-thread storage reentry`() {
        ...
        ).appendSyncAccepted(holderEntry("reentrant"), object : dev.everframe.outbox.OutboxAuthorization {
    ...
    private fun restart() {
        val entered = java.util.concurrent.CountDownLatch(1)
        val release = java.util.concurrent.CountDownLatch(1)
        Everframe.__beforeDrainLaunchForTesting = { entered.countDown(); check(release.await(30, java.util.concurrent.TimeUnit.SECONDS)) }
        val thread = Thread { Everframe.start(context, config) }
    ...
    @Test fun `refused storage leaves the same identity and final allowance available for retry`() {
        ...
        CrashReporter.sidecarFactory = { CrashSidecar(File(storageDir, "crash-outbox.jsonl"), keys,
            object : dev.everframe.outbox.OutboxFileOps by JvmOutboxFileOps() {
    ...
    @Test fun `kill then restart during a getter cannot resurrect the old attempt`() {
        val old = object : RuntimeException("old") {
            override val message: String get() { Everframe.kill(); restart(); return "revoked" }
        }
        assertFalse(CrashReporter.captureHandledThrowable(old))
        assertEquals(0, envelopes().size)
    ...
    @Test fun `every overridable accessor is protected against recursive handled capture`() {
        ...
        val error = object : RuntimeException() {
            override val message: String get() {
                messageReads++
                if (messageReads < 3) Everframe.captureException(this)
                return "outer"
            }
            override fun getStackTrace(): Array<StackTraceElement> {
                stackReads++
                if (stackReads < 3) Everframe.captureException(this)
                return emptyArray()
            }
            override val cause: Throwable? get() {
                causeReads++
                if (causeReads < 3) Everframe.captureException(this)
                return null
            }
        }
    ...
    @Test fun `original stack causes mapping and user are retained when a getter changes live context`() {
        Everframe.setUser(dev.everframe.config.TXUser(id = "original-user"))
        ...
        val outer = object : RuntimeException("outer", cause) {
            override fun getStackTrace(): Array<StackTraceElement> {
                ...
                Everframe.__setConfigForTesting(config.copy(r8MappingId = "mapping-B", sdkKey = "later-key"))
