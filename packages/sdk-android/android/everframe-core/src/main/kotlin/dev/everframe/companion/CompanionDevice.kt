...
// `packages/sdk-ios/Sources/Everframe/Companion/CompanionDevice.swift` to the
...
object CompanionDeviceId {
    ...
    /**
     * Distinct from `outbox/DeviceKey.kt`'s `everframe-device-key.enc` — this
     * id is a companion-discovery convenience, not the outbox's signing
     * identity, and the two must never collide on one file.
     */
    private const val FILE_NAME: String = "everframe-companion-device-id.enc"
