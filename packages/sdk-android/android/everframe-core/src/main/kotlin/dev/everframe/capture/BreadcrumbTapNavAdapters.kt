...
// packages/sdk-ios/Sources/Everframe/Capture/BreadcrumbTapNavAdapters.swift)
...
// omission): `androidx.fragment` is NOT a dependency of `everframe-core`
...
// Hosts that need fragment-level breadcrumbs call `Everframe.recordScreen(name)`
 ...
 * fragment-level deferred-gap decision), and `Everframe.recordScreen`
...
internal object BreadcrumbTapNavAdapters {
    ...
    /**
     * Wired from `Everframe.start(context, config)` after the kill-gate is
    ...
    fun install(context: Context, currentActivity: Activity? = null) {
        ...
        // Late-start hosts (React Native: `Everframe.start()` runs from JS long
