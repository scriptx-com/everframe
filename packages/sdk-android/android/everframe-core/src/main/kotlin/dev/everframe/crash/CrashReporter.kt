...
import dev.everframe.protocol.generated.JSBundle
...
object CrashReporter {
    ...
    private var handledAdmission = HandledThrowableAdmission { dev.everframe.Everframe.currentStartEpochVolatile() }

    @Volatile private var appContext: Context? = null

    /**
     * Context only. The config is NOT cached here any more: a second copy
     * written outside `Everframe.stateLock` is what let `start()` swap the crash
     ...
     *   own session epoch (see [dev.everframe.TXCapturedSession]).
     *
     *   Round-5 external review, finding 2 (Serious): this used to be a live
     *   `Everframe.currentUser` read down at the `buildEncoded` call — after
     ...
     *
     *   Blocking: `captureSessionSnapshot()` takes `Everframe.stateLock` for a
    ...
    private fun capture(
        ...
        try {
            ...
            val encoded = EnvelopeBuilder(EnvelopeBuilder.DefaultRedactor).buildEncoded(
                ...
                // live `Everframe.currentUser` read: by this line redaction,
            ...
            // `Everframe.resolveCapturedUser` performs internally, made
            ...
            val entry = OutboxEntry(
                    ...
                    // (`Everframe.requestOutboxDrain()` for a non-fatal RN
            ...
            val authorization = object : dev.everframe.outbox.OutboxAuthorization {
