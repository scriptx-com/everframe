...
// `Everframe.report.__resolver = { ... ReportSubmitter(config, outbox).submit(...) }`
// and calls `submitter.drainOutbox()` from Everframe.start()'s detached coroutine.
// Plan 05-05 does NOT modify Everframe.kt — single-writer invariant preserved.
//
// CRITICAL invariant (T-05-05-E mitigation):
//   Submitter's internal OkHttpClient is built WITHOUT EverframeInterceptor.
...
// still enforced by the source-grep gate (no addEverframeInterceptor reference
// in this file); making the class public does not relax that gate.
class ReportSubmitter(
    private val config: EverframeConfig,
     ...
     *   - `ReportResult.Cancelled("kill_switch")` if `Everframe.captureGate` is off
     *
     * Throws `EverframeTransportError.ServerError(status, body)` on terminal HTTP
     * (4xx other than 408/429) or `EverframeTransportError.NetworkUnavailable` on
    ...
    suspend fun submit(
        ...
        val result = try {
            ...
            if (RetryPolicy.classify(null, emptyMap(), networkFailure) == RetryPolicy.Classification.Terminal) {
                throw EverframeTransportError.NetworkUnavailable
            }
        ...
        if (result != null) {
            ...
            if (RetryPolicy.classify(result.statusCode, result.headers, null) == RetryPolicy.Classification.Terminal) {
                throw EverframeTransportError.ServerError(result.statusCode, result.responseBodyPreview)
            }
     ...
     * returning false). Safe to invoke from `Everframe.start()`'s detached
     ...
     *   resolving a token. Every caller — `Everframe.start()`'s launch drain,
     *   `Everframe.requestOutboxDrain()`'s RN non-fatal drain, and every test —
     ...
     *   `config` this submitter was constructed with (e.g. `Everframe
     *   .start()`'s own captured epoch, or a same-scope
     *   `Everframe.currentStartEpoch()` read right before constructing the
     ...
     *   former).
     * @param currentEpoch a LIVE read of `Everframe.currentStartEpoch()`,
     ...
     *   ([Everframe.__resolveIdentityToken]).
     */
    suspend fun drainOutbox(
        ...
        currentEpoch: () -> Int,
        drainSession: TXCapturedSession = Everframe.captureSessionSnapshot(),
    ...
    companion object {
        /**
         * Build an OkHttpClient that intentionally excludes EverframeInterceptor
