...
//
// Mirrors `packages/sdk-ios/Sources/Everframe/Envelope/InternalLogger.swift`.
//
// Note: We use `android.util.Log` instead of Timber to avoid forcing Timber
// onto :everframe-core's classpath (Timber is reflection-detected for log capture
...
internal object InternalLogger {
    ...
    internal fun recordSafeWrapFailure(label: String, error: Throwable) {
        ...
        try {
            android.util.Log.w("Everframe/safeWrap", "label=$label", error)
        } catch (_: Throwable) {
            System.err.println("Everframe/safeWrap label=$label error=$error")
        }
    }
