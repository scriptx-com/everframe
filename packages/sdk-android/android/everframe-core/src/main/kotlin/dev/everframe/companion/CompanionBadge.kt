...
// `Everframe.currentReplayConfig().companionBadge` — at EVERY apply/show, never
...
// `EverframeModule` because it needs no new interface on the RN bridge and
 ...
 * `EverframeModule` (the RN bridge) so `Everframe.startCompanion()` — the core
...
/**
 * Minimal current-Activity tracker for [dev.everframe.Everframe.startCompanion]
 ...
 * own provider from `EverframeModule.startCompanion()`
...
// modules in the dev.everframe library group, never host API.
@androidx.annotation.RestrictTo(androidx.annotation.RestrictTo.Scope.LIBRARY_GROUP)
object CompanionBadgeServerConfigSignal {
    val flow = kotlinx.coroutines.flow.MutableStateFlow<dev.everframe.config.CompanionBadgeConfigWire?>(null)

    /** Serialized destination mutation; guard must never acquire the facade state lock. */
    @Synchronized
    internal fun publish(value: dev.everframe.config.CompanionBadgeConfigWire?, isCurrent: () -> Boolean) {
...
class CompanionBadge(
     ...
     */
    private val serverConfig: () -> dev.everframe.config.CompanionBadgeConfigWire? =
        { dev.everframe.Everframe.currentReplayConfig().companionBadge },
    /**
    ...
    companion object {
        /**
         * Installed by the RN bridge (`EverframeModule.startCompanion`),
         ...
         * `:everframe-core` is a plain AAR that cannot resolve a foreground
         * Activity on its own.
         */
        @JvmStatic
     ...
     * `EverframeModule`. Exposed via [activityLifecycleCallbacksForTesting]
    ...
    private fun reapply() {
        apply(
            dev.everframe.companion.Companion.attachedUserName.value,
            dev.everframe.companion.Companion.resolvedName.value,
            dev.everframe.companion.Companion.code.value,
        )
    }
    ...
    init {
        combine(
            dev.everframe.companion.Companion.attachedUserName,
            dev.everframe.companion.Companion.resolvedName,
            dev.everframe.companion.Companion.code,
    ...
    private fun show(text: String) {
        ...
        activity.runOnUiThread {
            ...
            try {
                ...
                android.util.Log.w("Everframe.companion", "name badge failed to attach", t)
            }
        }
    }
