...
//
// Mirrors `packages/sdk-ios/Sources/Everframe/Capture/SensitiveRectRegistry.swift`
// (44 lines).
//
...
object SensitiveRectRegistry {
     ...
     *  `everframe.sensitive` (TX_SENSITIVE_KEY) or Compose's built-in `Password`
    ...
    private fun isSensitiveSemNode(node: Any): Boolean {
        ...
        return try {
            ...
            while (iter.hasNext()) {
                ...
                if (keyName == "everframe.sensitive" || keyName == "Password") return true
            }
            false
        } catch (_: Throwable) { false }
    }
