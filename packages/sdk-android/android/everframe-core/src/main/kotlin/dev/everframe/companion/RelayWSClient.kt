...
//
// Mirrors `packages/sdk-ios/Sources/Everframe/Companion/RelayWSClient.swift`
...
//   • `ProcessLifecycleOwner.ON_STOP` -> `dev.everframe.companion.Companion.__setState(PhoneDisconnected)`
...
import dev.everframe.protocol.generated.AttachChallenge
import dev.everframe.protocol.generated.AttachChallengeCleared
import dev.everframe.protocol.generated.CompanionName
import dev.everframe.protocol.generated.PairBonded
import dev.everframe.protocol.generated.PairCreated
import dev.everframe.protocol.generated.PairExpired
import dev.everframe.protocol.generated.PhoneDisconnected
import dev.everframe.protocol.generated.PreviewStart
import dev.everframe.protocol.generated.PreviewStop
import dev.everframe.protocol.generated.RelayMessage
import dev.everframe.protocol.generated.ReportCancelled
import dev.everframe.protocol.generated.ReportCompleted
import dev.everframe.protocol.generated.ReportFailed
import dev.everframe.protocol.generated.ReportRejected
import dev.everframe.protocol.generated.ReportRequest
import dev.everframe.protocol.generated.ReportSubmit
import dev.everframe.protocol.generated.ShotBinary
import dev.everframe.protocol.generated.ShotRequest
 ...
 * `Everframe.__attachPinUiInstalled` is set — i.e. `:everframe-reporter-ui`'s
...
class RelayWSClient(
     ...
     * release AAR points at prod, debug AAR at EVERFRAME_DEV_INGEST_URL.
    ...
    /**
     * The host's Everframe key (`EverframeConfig.sdkKey`). Supply it to make
     ...
     * `:everframe-reporter-ui`'s presenter actually installed itself.
     */
     ...
     * `Everframe.startCompanion()` wires this from `EverframeConfig` — this
    ...
    private fun installReportEndedHook() {
        dev.everframe.companion.Companion.__onReportEnded = { corrId ->
    ...
    fun stop() {
        ...
        dev.everframe.companion.Companion.__setAttachChallenge(null)
    ...
    private fun beginConnect() {
        ...
        announceScope.launch {
            try {
                ...
                dev.everframe.companion.Companion.__setCode(result.code)
                dev.everframe.companion.Companion.__setResolvedName(result.resolvedName)
     ...
     * when `:everframe-reporter-ui`'s presenter has actually installed itself
     * ([Everframe.__attachPinUiInstalled]), so an app without that module on
    ...
    private fun supportsAttachPinCapability(): Boolean = when (attachPinUi) {
        ...
        AttachPinUi.BUILTIN -> Everframe.__attachPinUiInstalled
    ...
    internal inner class RelayListener(private val generation: Long) : WebSocketListener() {
        ...
        override fun onOpen(webSocket: WebSocket, response: Response) {
            ...
            reconnectAttempt = 0
            android.util.Log.i("Everframe.companion", "ws.onOpen — code=${response.code}")
        }
        ...
        override fun onMessage(webSocket: WebSocket, text: String) {
            ...
            android.util.Log.i("Everframe.companion", "ws.onMessage parsed=${msg::class.simpleName}")
            when (msg) {
                is PairCreated -> {
                    // SECURITY: pair_token embedded in URL only — never logged separately.
                    dev.everframe.companion.Companion.__setPairUrl(buildPairUrl(msg.pairToken))
                    ...
                    dev.everframe.companion.Companion.__setAttachedUserName(null)
                    dev.everframe.companion.Companion.__setState(CompanionState.Unpaired)
                    android.util.Log.i("Everframe.companion", "pair.created → state=Unpaired, pairUrl=set")
                }
                is PairBonded -> {
                    ...
                    dev.everframe.companion.Companion.__setAttachedUserName(
                    ...
                    // visibility.
                    dev.everframe.companion.Companion.__setState(CompanionState.Paired)
                    android.util.Log.i("Everframe.companion", "pair.bonded → state=Paired, pairUrl=kept")
                }
                is PairExpired -> {
                    ...
                    dev.everframe.companion.Companion.__setAttachedUserName(null)
                    dev.everframe.companion.Companion.__setState(CompanionState.Unpaired)
                    android.util.Log.i("Everframe.companion", "pair.expired → state=Unpaired, pairUrl=kept")
                ...
                is PhoneDisconnected -> {
                    ...
                    dev.everframe.companion.Companion.__setState(CompanionState.PhoneDisconnected)
                    android.util.Log.i("Everframe.companion", "phone.disconnected → state=PhoneDisconnected")
                ...
                is PreviewStart -> {
                    android.util.Log.i("Everframe.companion", "preview.start corrId=${msg.correlationId}")
                ...
                is PreviewStop -> {
                    ...
                    android.util.Log.i("Everframe.companion", "preview.stop corrId=${msg.correlationId} reason=${msg.reason}")
                    previewSession.stopSilently()
                }
                is ShotRequest -> {
                ...
                is ReportRequest -> {
                    ...
                    // SECURITY: never log.
                    if (dev.everframe.companion.Companion.state.value == CompanionState.ReportInProgress) {
                        android.util.Log.i(
                            "Everframe.companion",
                    ...
                    // (PR-fix 7).
                    dev.everframe.companion.Companion.__beginReport(msg.correlationId)
                    android.util.Log.i("Everframe.companion", "report.request corrId=${msg.correlationId} → state=ReportInProgress")
                ...
                is ReportSubmit -> {
                    ...
                    android.util.Log.i("Everframe.companion", "report.submit corrId=${msg.correlationId}")
                ...
                is ReportCancelled -> {
                    ...
                    val ended = dev.everframe.companion.Companion.__finishReport(
                        msg.correlationId,
                    ) {
                ...
                is AttachChallenge -> {
                    ...
                    dev.everframe.companion.Companion.__setAttachChallenge(
                        AttachChallengeInfo(
                            code = msg.code,
                    ...
                    android.util.Log.i("Everframe.companion", "attach.challenge → pin surface shown")
                }
                ...
                is CompanionName -> {
                    ...
                    if (msg.name.length in 1..80) {
                        dev.everframe.companion.Companion.__setResolvedName(msg.name)
                        android.util.Log.i("Everframe.companion", "companion.name → resolvedName updated")
                    }
                }
                else -> {
                    android.util.Log.i("Everframe.companion", "ws.onMessage IGNORED (unhandled discriminator) class=${msg::class.simpleName}")
        ...
        override fun onMessage(webSocket: WebSocket, bytes: ByteString) {
            if (!isAttemptCurrent(generation)) return
            android.util.Log.i("Everframe.companion", "ws.onMessage(binary) bytes=${bytes.size}, pendingCorrId=$pendingSubmitCorrelationId")
            ...
            pendingSubmitCorrelationId = null
            if (corrId == null) {
                android.util.Log.w("Everframe.companion", "ws.onMessage(binary) DROPPED — no pending submit")
        ...
        override fun onClosing(webSocket: WebSocket, code: Int, reason: String) {
            ...
            // see [RelayListener].
            if (!isAttemptCurrent(generation)) return
            android.util.Log.i("Everframe.companion", "ws.onClosing code=$code, reason=$reason")
            ...
            when (code) {
                ...
                CLOSE_TOKEN_NOT_FOUND -> {
                    ...
                    // the pair.
                    attributionToken = null
                    dev.everframe.companion.Companion.__setPairUrl(null)
    ...
    override fun onStart(owner: LifecycleOwner) {
        ...
        // for the QR (see Everframe.companion log: two `ws.onOpen` + two
        ...
        android.util.Log.i("Everframe.companion", "onStart: reconnecting from foreground")
        if (deviceToken != null) connectWithDeviceToken() else beginConnect()
    }
