...
// call sites (LogCapture.kt, EverframeInterceptor.kt) — nothing here needs an
// "install" step for those two kinds. Lifecycle + error ARE installed (once)
// from `Everframe.start(context, config)`, after the kill-gate is set. All
 ...
 * in `EverframeInterceptor.kt` (success + IOException).
 */
internal object NetworkBreadcrumbAdapter {
     ...
     * (`EverframeInterceptor` redacts at capture time via `RedactionEngine`);
    ...
    /**
     * Dual-write call site — invoked immediately after
     * `sharedNetworkBuffer.push(entry)` at both of EverframeInterceptor.kt's
 ...
 * `object` so install-once semantics hold across repeated `Everframe.start()`
...
internal object ErrorBreadcrumbAdapter {
    ...
    fun install() {
        ...
        Thread.setDefaultUncaughtExceptionHandler(EverframeUncaughtExceptionHandler)
    }
 ...
 */
private object EverframeUncaughtExceptionHandler : Thread.UncaughtExceptionHandler {
...
internal object BreadcrumbAdapters {
    /**
     * Wired from `Everframe.start(context, config)`'s detached heavy-init
     ...
     * (`LogCapture.kt`, `EverframeInterceptor.kt`) and are gated purely by
