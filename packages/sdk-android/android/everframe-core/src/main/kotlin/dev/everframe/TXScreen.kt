...
// composes the visible screen. Feeds Everframe.recordScreen(), which shares
// the global from→to chain with the Activity-level auto-capture.
//
// This is deliberately the ONLY production @Composable in :everframe-core
...
public fun TXScreen(name: String, active: Boolean = true) {
    LaunchedEffect(name, active) {
        if (active) Everframe.recordScreen(name)
    }
}
