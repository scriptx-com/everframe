...
// — drainOutbox runs at Everframe.start() in a coroutine; no WorkManager/JobScheduler.
//
// Mirrors `packages/sdk-ios/Sources/Everframe/Transport/RetryPolicy.swift` with
...
internal object RetryPolicy {
     ...
     * `attempt` is 1-based. Throws `EverframeTransportError.RetryPolicyError` outside
    ...
    fun delay(forAttempt: Int): Long {
        if (forAttempt < 1 || forAttempt > MAX_ATTEMPTS) {
            throw EverframeTransportError.RetryPolicyError(
