...
// packages/sdk-ios/Sources/Everframe/Capture/NetworkBodyRingBuffer.swift.
//
...
// real reporter/kill/logout lifecycle in `Everframe.kt` and
...
class NetworkBodyRingBuffer internal constructor(
    ...
    companion object {
         ...
         * packages/sdk-ios/Sources/Everframe/Capture/NetworkBodyRingBuffer.swift's
         * `entryOverhead`.
         */
     ...
     * occupy while `kill()` flips [Everframe.captureGate] and `clear()`
     ...
     * `Everframe.kill()`), and only then resume and insert into a buffer
     ...
     * The production call site ([dev.everframe.okhttp.EverframeInterceptor])
     * passes a closure capturing the [NetworkBodyCaptureState] generation
     * observed at DECISION time (before the possibly-slow
     * [dev.everframe.okhttp.NetworkBodyCapture.makeEntry] work), so a remote
    ...
    fun append(entry: NetworkBody, guard: (() -> Boolean)? = null) {
        ...
        preLockHook?.invoke()
        lock.withLock {
            if (honorsKillGate && !Everframe.captureGate) return@withLock
            if (guard != null && !guard()) return@withLock
