...
//
// Public config types for Everframe. Kotlin port of
// `packages/sdk-ios/Sources/Everframe/Config/EverframeConfig.swift`.
//
...
data class EverframeConfig(
     ...
     * than a richer type. Mirrors iOS's `EverframeConfig.companionBadgeEnabled`.
     */
     ...
     * [dev.everframe.companion.CompanionBadgePosition]'s four cases
     * ('bottom-right' | 'bottom-left' | 'top-right' | 'top-left'). `null` or
     * an unrecognised value falls through to `BOTTOM_RIGHT` — see
     * `EverframeModule.parseCompanionBadgePosition`. Mirrors iOS's
     * `EverframeConfig.companionBadgePosition`.
     */
     ...
     * `EverframeConfig.installIdentifierEnabled`.
     *
...
data class CaptureConfig(
     ...
     * [dev.everframe.capture.NetworkBodyCaptureState]).
     */
...
sealed class EverframeConfigError(message: String) : Exception(message) {
    object MissingAppId : EverframeConfigError("appId is blank")
    object BlankSdkKey : EverframeConfigError("sdkKey is blank")
    object InvalidVitalsSampleRate : EverframeConfigError("vitals.sampleRate must be within 0.0..1.0")
}

// `EverframeTransportError` lives at `dev.everframe.transport.EverframeTransportError`
...
sealed class EverframeEnvelopeError(message: String) : Exception(message) {
    data class PayloadTooLarge(val bytes: Int, val limit: Int) :
        EverframeEnvelopeError("envelope $bytes bytes exceeds 25 MB cap (PIPE-03)")
}
