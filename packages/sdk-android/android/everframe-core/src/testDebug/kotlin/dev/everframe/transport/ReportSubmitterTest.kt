...
// EverframeInterceptor (no recursive capture). Verified by source-grep gate
// `grep -c 'addEverframeInterceptor' ReportSubmitter.kt == 0` AND by introspecting
...
import dev.everframe.identity.resolveIdentityHeader
...
class ReportSubmitterTest {
    ...
    fun setUp() {
        ...
        server.start()
        outboxFile = File(tmp.newFolder("dev.everframe"), "outbox.jsonl")
    ...
    fun `kill revokes permission even when resolving storage context throws`() {
        val contextField = Everframe::class.java.getDeclaredField("appContext").apply { isAccessible = true }
        val sharedField = Everframe::class.java.getDeclaredField("sharedOutbox").apply { isAccessible = true }
        val previousContext = contextField.get(Everframe)
        val previousOutbox = sharedField.get(Everframe)
        val pendingField = Everframe::class.java.getDeclaredField("pendingOutboxRevocation").apply { isAccessible = true }
        val unresolvedField = Everframe::class.java.getDeclaredField("unresolvedOutboxRevocationContext").apply { isAccessible = true }
        val previousPending = pendingField.get(Everframe)
        val previousUnresolved = unresolvedField.get(Everframe)
        val directory = tmp.newFolder()
        val keys = JceTestOutboxKeyProvider()
        val root = File(directory, "dev.everframe/outbox-v1")
        val oldStore = dev.everframe.outbox.OutboxStore(root, keys, JvmOutboxFileOps())
        val permission = object : dev.everframe.outbox.OutboxAuthorization { override fun isAllowed() = true }
        val oldToken = oldStore.enqueueSync(dev.everframe.outbox.entry("before-unresolved-kill"), permission)
        ...
        try {
            contextField.set(Everframe, context)
            sharedField.set(Everframe, null)
            val captured = Everframe.captureSessionSnapshot()
            Everframe.kill()
            assertFalse(Everframe.captureGate)
            assertTrue(Everframe.killGenerationChangedVolatile(captured.killGeneration))
            assertNull(Everframe.currentConfig)
            assertTrue(unresolvedField.get(Everframe) === context)
            ...
            assertEquals(dev.everframe.outbox.OutboxFailure.REVOKED,
                assertThrows(dev.everframe.outbox.OutboxWriteException::class.java) {
                    oldStore.enqueueSync(dev.everframe.outbox.entry("old-facade"), permission)
                }.failure)
        } finally {
            pendingField.set(Everframe, previousPending)
            unresolvedField.set(Everframe, previousUnresolved)
            contextField.set(Everframe, previousContext)
            sharedField.set(Everframe, previousOutbox)
        }
    }
    ...
    fun `SDK kill during paused drain cannot upload or resurrect accepted token`() = runBlocking {
        ...
        outbox.enqueue(dev.everframe.outbox.entry("paused"))
        ...
        entered.await()
        Everframe::class.java.getDeclaredField("sharedOutbox").apply { isAccessible = true }.set(Everframe, outbox)
        Everframe.kill()
        Everframe.captureGate = true
        val fresh = JSONLOutbox(outboxFile, keys = keys, ops = JvmOutboxFileOps())
        fresh.enqueue(dev.everframe.outbox.entry("replacement"))
    ...
    fun `SDK kill then reopened gate cannot drain durable pre-kill report`() = runBlocking {
        val keys = JceTestOutboxKeyProvider()
        outbox = JSONLOutbox(outboxFile, keys = keys, ops = JvmOutboxFileOps())
        outbox.enqueue(dev.everframe.outbox.entry("pre-kill"))
        val shared = Everframe::class.java.getDeclaredField("sharedOutbox").apply { isAccessible = true }
        shared.set(Everframe, outbox)
        Everframe.kill()
        Everframe.captureGate = true
        val reopened = JSONLOutbox(outboxFile, keys = keys, ops = JvmOutboxFileOps())
        assertEquals(0, reopened.count())
        assertEquals(dev.everframe.outbox.OutboxFailure.REVOKED,
            org.junit.Assert.assertThrows(dev.everframe.outbox.OutboxWriteException::class.java) {
                runBlocking { outbox.enqueue(dev.everframe.outbox.entry("pre-kill")) }
            }.failure)
        reopened.enqueue(dev.everframe.outbox.entry("post-kill"))
    ...
    fun `delayed drain withholds identity after endpoint alone changes`() = runBlocking {
        ...
        val oldOverride = dev.everframe.config.EndpointOverride.current
        try {
            dev.everframe.config.EndpointOverride.current = endpoint
            ...
            dev.everframe.config.EndpointOverride.current = "http://127.0.0.1:1"
    ...
    fun `503 full disk preserves accepted queue and never reports queued`() = runBlocking {
        ...
        assertTrue(result.exceptionOrNull() is dev.everframe.outbox.OutboxWriteException)
    ...
    fun `connection failure with unavailable key preserves accepted queue`() = runBlocking {
        ...
        val keys = object : dev.everframe.outbox.OutboxKeyProvider by real {
    ...
    fun `endpoint changing during HTTP cannot redirect queued report`() = runBlocking {
        val old = dev.everframe.config.EndpointOverride.current
        val original = server.url("/").toString()
        try {
            dev.everframe.config.EndpointOverride.current = original
        ...
        } finally { dev.everframe.config.EndpointOverride.current = old }
    }
    ...
    fun `optional revoke during disk admission persists only ordinary report across reopen`() = runBlocking {
        ...
        val result = makeSubmitter().submit(
            """{"reportId":"frozen","attachments":[{"partName":"replay","format":"everframe-video-v1"}]}""".toByteArray(),
    ...
    fun `submit with 400 throws ServerError and outbox stays empty`() = runBlocking {
        ...
        val ex = assertThrows(EverframeTransportError.ServerError::class.java) {
            runBlocking {
                submitter.submit(
                    envelopeBytes = "{}".toByteArray(),
    ...
    // IdentityGateTest (dev.everframe.identity) already proves
    ...
    @Test
    fun `isolated client has no EverframeInterceptor`() {
        // Build the same isolated client the submitter uses. Assert no interceptor
        // is named EverframeInterceptor (mirrors iOS class-name filter).
        ...
        for (i in all) {
            ...
            assertFalse(
                "isolated client must not contain EverframeInterceptor (recursive capture); found $name",
                name.contains("EverframeInterceptor"),
            )
        }
    }
}
