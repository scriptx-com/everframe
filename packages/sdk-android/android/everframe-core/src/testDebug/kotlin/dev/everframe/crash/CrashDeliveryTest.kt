...
class CrashDeliveryTest {
    ...
    private lateinit var crashKeys: dev.everframe.outbox.JceTestOutboxKeyProvider
    private val crashOps = dev.everframe.outbox.JvmOutboxFileOps()
    ...
    fun setUp() {
        ...
        crashKeys = dev.everframe.outbox.JceTestOutboxKeyProvider()
        ...
        // it must be installed on Everframe; `__setConfigForTesting` does that
    ...
    fun tearDown() {
        CrashReporter.__resetForTesting()
        Everframe.__setConfigForTesting(null)   // process-global; leaks across suites
        EndpointOverride.current = null   // process-global; leaks across suites
        Everframe.captureGate = false
        Everframe::class.java.getDeclaredField("sharedOutbox").apply { isAccessible = true }.set(null, null)
        server.shutdown()
    }
    ...
    fun `crash then relaunch delivers the report (definition of done)`() = runBlocking {
        ...
        dev.everframe.transport.ReportSubmitter(config, outbox).drainOutbox(
    ...
    fun `a crash captured while alice is signed in is attributed to alice on relaunch drain`() = runBlocking {
        ...
        try {
            ...
            // `Everframe.requestOutboxDrain()` or `Everframe._identityHolder` /
            // `Everframe.currentReplayConfig()` — it is `ReportSubmitter
            ...
            // drives the REAL production call (`Everframe.requestOutboxDrain()`
            // reading the REAL `_identityHolder`/`currentReplayConfig()`).
            // The OTHER production drain call site — `Everframe.start()`'s
     ...
     * drain call sites, `Everframe.start()`'s launch drain runs too early to
     ...
     * 200.
     *
     * `Everframe.requestOutboxDrain()` — the RN non-fatal path's explicit,
    ...
    fun `a non-fatal crash relaunch-drains through requestOutboxDrain once identity and config have genuinely settled`() {
        ...
        try {
            ...
            server.enqueue(MockResponse().setResponseCode(200).setBody("{}"))
            Everframe::class.java.getDeclaredField("sharedOutbox").apply { isAccessible = true }.set(null, crashOutbox)
    ...
    fun `public handled capture immediately delivers and retries the exact durable envelope without restart`() = runBlocking {
        ...
        val startThread = Thread { Everframe.start(context, config) }
        startThread.start()
        try {
            ...
            val epoch = Everframe.currentStartEpoch()
            Everframe.captureException(IllegalStateException("public handled HTTP"))
            ...
            assertEquals(entry.idempotencyKey, first.getHeader("X-Everframe-Idempotency-Key"))
            ...
            assertEquals(entry.idempotencyKey, retry.getHeader("X-Everframe-Idempotency-Key"))
            assertEquals(0, crashOutbox.count())
            assertEquals(epoch, Everframe.currentStartEpoch())
        } finally {
            Everframe.__beforeDrainLaunchForTesting = null
            Everframe.kill()
