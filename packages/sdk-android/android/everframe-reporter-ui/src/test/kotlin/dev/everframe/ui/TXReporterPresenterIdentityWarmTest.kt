...
// (`Everframe.setIdentityToken`) alone was not sufficient: once the cached
...
// SECOND call site — `Everframe.__warmIdentityToken()` at reporter-open — and
// `IdentityProviderWarmTest.kt` (`:everframe-core`) proves the underlying
...
// reachability problem. `openReporter` calls `Everframe.__warmIdentityToken()`
...
class TXReporterPresenterIdentityWarmTest {
    ...
    /** Poll `Everframe.captureUserSnapshot()` until it stamps a subject or a
     *  budget expires. */
    private fun pollForCapturedSubject(budgetMs: Long = 5_000): dev.everframe.TXCapturedUser {
