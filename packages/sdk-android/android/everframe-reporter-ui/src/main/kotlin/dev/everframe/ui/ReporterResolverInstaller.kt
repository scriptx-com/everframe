...
// `Everframe.report.__resolver` so `report.open()` resolves through
...
class ReporterResolverInstaller : Initializer<Unit> {
    ...
    override fun create(context: Context) {
        ...
        // :everframe-core ReplaySession walk via the __activitySupplier seam —
        ...
        Everframe.__activitySupplier = { ActivityRegistry.activeActivity() }

        // report.open() resolver — wraps presenter.openReporter for the
        // currently-resumed Activity.
        Everframe.report.__resolver = resolver@ {
            val activity = ActivityRegistry.activeActivity()
                ?: return@resolver dev.everframe.config.ReportResult.Cancelled("no_active_activity")
        ...
        // suppression are runtime checks (Everframe.__attachPinUiInstalled /
