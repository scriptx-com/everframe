...
import dev.everframe.capture.video.NativeVideoAttachment
import dev.everframe.transport.ReportAuthorizationFactory
...
import dev.everframe.envelope.buildAttachmentPlan
...
import dev.everframe.ui.annotation.AnnotationWireFormat
...
internal object ReporterDialog {
    ...
    private fun ReporterDialogContent(
        ...
        if (showDiscardConfirm) {
            ...
            // (dev.everframe.ui.theme) is the same provider ReporterRoot's
            // body wraps in, so this now resolves and stays live-in-sync
            // with it.
            ProvideReporterTheme {
                dev.everframe.ui.details.DiscardConfirmDialog(
    ...
    internal suspend fun submitBaked(
         ...
         * caller — never re-read from `Everframe.currentUser` in here. See the
         ...
         * captured in ([dev.everframe.TXCapturedUser]), not a bare `TXUser?`.
        ...
        // Send tap, NOT from a live `Everframe.currentConfig` read here.
        //
        ...
        return txGuardSuspend("dialog-submit") {
            ...
            val screenshotEnvelopeAttachments = mutableListOf<dev.everframe.protocol.generated.Attachment>()
            ...
            for ((index, shot) in shots.withIndex()) {
                ...
                screenshotEnvelopeAttachments.add(
                    dev.everframe.protocol.generated.Attachment(
            ...
            //     `network[]` even when LogCapture and EverframeInterceptor
            ...
            val encoded = builder.buildEncoded(
                ...
                // the Send tap, NEVER a live `Everframe.currentUser` read. Every
            ...
            // user just above. `Everframe.__resolveIdentityToken` (public,
            // unlike the `:everframe-core`-internal `_identityHolder` it
            // wraps) is the seam this cross-module call goes through — this
            // file lives in `:everframe-reporter-ui`, a SEPARATE Gradle module
            // from `:everframe-core`, mirroring why
            // `Everframe.__replayComplete(reportCapture)` is public despite the
            ...
            val identity = Everframe.__resolveIdentityToken(
