...
//
// :everframe-reporter-ui — Compose-based reporter UI for phone + tablet.
// Opt-in artifact; depends on :everframe-core.
//
// Material/Foundation/Animation pulled in HERE (allowed); Plan 01's
// Compose-isolation gate forbids them in :everframe-core only.
...
android {
    namespace = "dev.everframe.reporter.ui"
    defaultConfig {
    ...
    buildTypes {
        getByName("release") {
            // Library-level R8: matches the :everframe-core release variant.
...
dependencies {
    implementation(project(":everframe-core"))
    // ReporterDialog JSON-decodes the host-attached React tree (Phase 6
    // attachReactTree). :everframe-core has serialization.json as
...
// Maven Central publishing — see everframe-core/build.gradle.kts for the full
// rationale + signing env-var contract.
afterEvaluate {
    publishing {
        publications {
            create<MavenPublication>("release") {
                ...
                pom {
                    name.set("Everframe Android — reporter-ui")
    ...
    signing {
        ...
        if (signingKey != null && signingPassword != null) {
            ...
            logger.warn("[everframe-reporter-ui] GPG signing skipped — SIGNING_KEY / SIGNING_PASSWORD not set. Local Maven publishing still works; Central Portal bundles require signatures.")
        }
    }
}
...
//
// This is the same failure `:everframe-core` already documented for
