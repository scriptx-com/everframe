...
import com.traceitx.protocol.generated.ReportAssembledCounts
import com.traceitx.protocol.generated.ReportAssembledToggles
...
class CompanionFreezeLifecycleTest {
    ...
    fun setUp() {
        ...
        TraceItX.start(ApplicationProvider.getApplicationContext(), com.traceitx.config.TraceItXConfig(
            appId = "test", sdkKey = "txx_live_test1234567890", environment = com.traceitx.config.Environment.production))
    ...
    fun reportRequestFreezesBreadcrumbChain() {
        TraceItX.addBreadcrumb("before-report")
        var frozen: com.traceitx.capture.video.FrozenReportCapture? = null
    ...
    fun duplicateDisconnectCallbacksCannotCancelReplacementCapture() {
        ...
        while (TraceItX._replaySession == null && System.nanoTime() < deadline) Thread.sleep(5)
        org.junit.Assert.assertNotNull(TraceItX._replaySession)
        sharedBreadcrumbBuffer.clear(); TraceItX.addBreadcrumb("replacement")
