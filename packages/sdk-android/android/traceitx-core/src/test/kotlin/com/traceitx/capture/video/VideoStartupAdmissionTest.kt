...
class VideoStartupAdmissionTest {
    ...
    private fun session(epoch: Int = TraceItX.currentStartEpochVolatile(), consent: Boolean = true): ReplaySession {
    ...
    private fun artifact(session: String = UUID.randomUUID().toString(), capture: String = UUID.randomUUID().toString(),
        ...
        File(File(File(File(root, "traceitx-video"), session), capture), name).apply { parentFile!!.mkdirs(); writeText(body) }
    ...
    @Test fun failedDeletionRemainsChargedAndBlocksVideoButPreservesReportCapture() = runBlocking {
        ...
        assertTrue(created.isEmpty()); assertTrue(TraceItX.captureGate)
        val report = session.freezeOwnedCapture()
        assertNotNull(report.takeBreadcrumbs()); assertFalse(report.replayAllowed())
        assertTrue(com.traceitx.transport.ReportAuthorizationFactory.forCapture(TraceItX.captureSessionSnapshot(), report).evaluate().reportAllowed)
        report.cancel()
        val scanned = reads
    ...
    @Test fun unknownAndSymlinkStructuresStayUntouchedAndDenyNewFiles() = runBlocking {
        ...
        val video = File(root, "traceitx-video").apply { mkdirs() }
    ...
    @Test fun unknownArtifactIsPreservedAndDoesNotAuthorizeFreeBudget() = runBlocking {
        ...
        val session = session(); session.refreshConfigNow(); scheduler.drain()
        assertTrue(unknown.exists()); assertTrue(created.isEmpty()); assertTrue(TraceItX.captureGate)
    }
    ...
    private fun killTailReplacement(fromCancellation: Boolean) = runBlocking {
        ...
        TraceItX.__replaySessionFactoryForTesting = { _, _, epoch, consent, _ -> session(epoch, consent) }
        val config = TraceItXConfig(appId = "A", sdkKey = "txx_live_test1234567890", capture = CaptureConfig(logs = false))
        fun awaitSession(count: Int): ReplaySession {
            val deadline = System.nanoTime() + TimeUnit.SECONDS.toNanos(3)
            while ((sessions.size < count || TraceItX._replaySession !== sessions.last()) && System.nanoTime() < deadline) Thread.yield()
        ...
        val capturedA = TraceItX.captureSessionSnapshot()
        ...
        val startReplacement = {
                TraceItX.start(context, config.copy(appId = "B", theme = theme))
                b = awaitSession(2)
                ...
                TraceItX.addBreadcrumb(marker)
        }
        val oldJob = kotlinx.coroutines.Job(TraceItX.sdkScope.coroutineContext[kotlinx.coroutines.Job])
        try {
            if (fromCancellation) oldJob.invokeOnCompletion { startReplacement() }
            else TraceItX.__reporterTriggersTeardown = startReplacement
            TraceItX.kill()
        } finally { TraceItX.__reporterTriggersTeardown = null }
        assertSame("old kill must leave B installed", b, TraceItX._replaySession)
        ...
        assertEquals(false, com.traceitx.companion.CompanionBadgeServerConfigSignal.flow.value?.enabled)
        ...
        assertTrue(com.traceitx.capture.sharedBreadcrumbBuffer.snapshotForReport().any { it.message == marker })
    }

    @Test fun publicStartPreservesCapturedFactoryInputsAndInstallIdentifierVeto() {
        val supplied = CountDownLatch(1)
        val config = TraceItXConfig(appId = "veto", sdkKey = "txx_live_test1234567890", installIdentifierEnabled = false,
            capture = CaptureConfig(logs = false))
        val error = AtomicReference<Throwable?>()
        TraceItX.__replaySessionFactoryForTesting = { app, received, epoch, consent, identifier ->
            try {
                assertSame(context, app); assertSame(config, received)
                assertEquals(TraceItX.currentStartEpochVolatile(), epoch); assertTrue(consent)
    ...
    @Test fun realPublicStartKillAndReplacementShareDeferredCleanupWithoutResurrectingOldSession() {
        ...
        TraceItX.__replaySessionFactoryForTesting = { _, _, epoch, consent, _ -> session(epoch, consent) }
        val config = TraceItXConfig(appId = "test", sdkKey = "txx_live_test1234567890", capture = CaptureConfig(logs = false))
        TraceItX.start(context, config)
        ...
        try {
            ...
            TraceItX.kill()
            TraceItX.start(context, config.copy(sdkKey = "txx_live_test0987654321"))
        ...
        assertTrue(sessions.first().isTornDownForTesting); assertTrue(TraceItX.captureGate)
        val report = TraceItX.__replayFreeze(); assertNotNull(report.takeBreadcrumbs()); report.cancel()
    }
    ...
    @Test fun blockingFilesystemDoesNotHoldAuthorizationOrLeaseMonitorAndExportConflictOmits() = runBlocking {
        ...
        try {
            ...
            caller = Thread {
                try {
                    TraceItX.__withStateLockForTesting { session.pause(); session.resume() }
                    val lease = runCatching { VideoDirectoryLeases.acquire() }
                    lease.getOrNull()?.close()
                    assertTrue(lease.isFailure)
                    runBlocking {
                        assertNull(VideoExporter(File(root, "traceitx-video"), scheduler).export(
    ...
    @Test @Config(sdk = [24, 25]) fun oldApiPublicStartNeverInvokesVideoFilesystem() {
        ...
        val config = TraceItXConfig(appId = "old", sdkKey = "txx_live_test1234567890", capture = CaptureConfig(logs = false))
        TraceItX.__startTailDelayHookForTesting = { built.countDown() }
        TraceItX.start(context, config)
