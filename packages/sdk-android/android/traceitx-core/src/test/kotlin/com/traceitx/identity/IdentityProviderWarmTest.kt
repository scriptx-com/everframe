...
// `TraceItX.setIdentityToken` -> the detached warm -> `captureUserSnapshot()`
...
class IdentityProviderWarmTest {
    ...
    private fun pollForCapturedSubject(): com.traceitx.TXCapturedUser {
    ...
    fun `installing a provider warms the cache so the next capture stamps the subject`() {
        ...
        TraceItX.__replayConfigOverrideForTesting = enabledIdentityConfig()
    ...
    // above did NOT close this finding. `TraceItX.captureUserSnapshot()`
     ...
     * REAL capture path (`TraceItX.captureUserSnapshot()`, via
