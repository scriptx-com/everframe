    ...
    private fun drive(action: String) = runBlocking {
        ...
        try {
            ...
            when (action) {
                ...
                "kill" -> { TraceItX.kill(); TraceItX.start(activity, config.copy(appId = "b")) }
            }
            release.countDown()
            ...
            if (action == "kill") {
                ...
                assertEquals(action == "live", parsed.contains("traceitx-video-v1"))
            ...
            TraceItX.kill(); server.shutdown(); h.root.deleteRecursively()
        }
    }
}
