...
//
// Kotlin twin of packages/sdk-ios/Tests/TraceItXTests/IdentityTokenHolderTests.swift
...
// exercise `TraceItX.shared`'s start()/kill() wiring, which is a later
...
class IdentityTokenHolderTest {
    ...
    // finding. `TraceItX.captureUserSnapshot()` never calls `get()` at all —
    ...
    // so `cancelOutstandingWork()` (wired into `TraceItX.kill()`) can
    // actually reach it.
    //
