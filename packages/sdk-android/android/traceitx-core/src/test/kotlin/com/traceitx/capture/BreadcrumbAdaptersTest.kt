...
// calls) — mirrors BreadcrumbRingBufferTest / TraceItXInterceptorTest setUp.
...
class BreadcrumbAdaptersTest {
    ...
    private lateinit var crashOutbox: com.traceitx.outbox.JSONLOutbox
     ...
     * config a crash test wants exercised has to be installed on TraceItX.
     ...
     */
    private fun configureCrashReporter(config: TraceItXConfig) {
    ...
    fun setUp() {
        ...
        crashOutbox = com.traceitx.outbox.JSONLOutbox(File(dir, "outbox.jsonl"), keys, ops)
        CrashReporter.sidecarFactory = { com.traceitx.outbox.CrashSidecar(File(dir, "crash-outbox.jsonl"), keys, ops) }
        sidecarFile().delete()
    }
    ...
    fun `successful interceptor call lands a network crumb`() {
        ...
        try {
            ...
            val client = OkHttpClient.Builder().addTraceItXInterceptor().build()
    ...
    fun `handle still lands the same error crumb shape when CrashReporter is wired`() {
        configureCrashReporter(TraceItXConfig(appId = "app", sdkKey = "sk"))
        Thread.setDefaultUncaughtExceptionHandler(Thread.UncaughtExceptionHandler { _, _ -> })
        ErrorBreadcrumbAdapter.install()
    ...
    fun `sidecar is not created when CaptureConfig crash is false, but the crumb still lands`() {
        configureCrashReporter(
            TraceItXConfig(appId = "app", sdkKey = "sk", capture = CaptureConfig(crash = false)),
        )
        Thread.setDefaultUncaughtExceptionHandler(Thread.UncaughtExceptionHandler { _, _ -> })
        ErrorBreadcrumbAdapter.install()
