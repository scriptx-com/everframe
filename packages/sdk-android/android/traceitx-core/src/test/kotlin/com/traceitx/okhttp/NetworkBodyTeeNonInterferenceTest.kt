...
//
// This suite runs through a REAL OkHttp chain (addTraceItXInterceptor() +
...
class NetworkBodyTeeNonInterferenceTest {
     ...
     * `TraceItXInterceptor` immediately BEFORE the `NetworkBodyTee.attach`
     ...
     * exercise `TraceItXInterceptor`'s own catch/rethrow — `chain.proceed()`
     ...
     * (TraceItXInterceptor.kt, `bodyCtx`) — not a value a test hands it
