...
// library (testImplementation only — see :traceitx-core/build.gradle.kts).
// We verify:
...
class LogCaptureTest {
    ...
    fun `system_err_println_after_install_pushes_with_error_level`() {
        ...
        LogCapture.install()
        try {
            System.err.println("err-traceitx-line")
            val snap = sharedLogBuffer.snapshot()
            val match = snap.firstOrNull { it.message == "err-traceitx-line" }
    ...
    fun `timber_plant_after_install_routes_to_log_buffer`() {
        ...
        LogCapture.install()
        try {
            Timber.tag("UnitTest").i("timber-traceitx-msg")
            val snap = sharedLogBuffer.snapshot()
            val match = snap.firstOrNull { it.message == "timber-traceitx-msg" }
