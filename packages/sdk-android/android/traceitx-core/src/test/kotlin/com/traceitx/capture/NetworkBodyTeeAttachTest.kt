...
class NetworkBodyTeeAttachTest {
    ...
    // setUp flips `TraceItX.captureGate` and `__directForTesting` but never
    ...
    // 0, which is also how TraceItXInterceptor will capture the token at
    // decision time.
    ...
    fun killMidStreamZeroizesAndAppendsNothing() {
        ...
        TraceItX.captureGate = false  // kill lands mid-stream
    ...
    // `!TraceItX.captureGate` (honorsKillGate), so the entry is suppressed at
    ...
    fun killMidStreamZeroizesEvenIfTheGateReopensBeforeEof() {
        ...
        TraceItX.captureGate = true   // gate re-opens; an append would now succeed
