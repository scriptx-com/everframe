...
class VideoPrivacySettlementTest {
    ...
    @Before fun setup() { com.traceitx.shared.SharedData.init(context); TraceItX.captureGate = true }
    @After fun cleanup() {
    ...
    @Test fun finalNestedCloseStartsExactlyOneFreshOwnerWithoutFetchAndNeverRevivesFrozenClaim() {
        ...
        val authority = com.traceitx.transport.ReportAuthorizationFactory.forCapture(TraceItX.captureSessionSnapshot(), old)
    ...
    @Test fun localConsentLiveGateAndTeardownVetoSettlement() {
        ...
        val session = enabled(); val token = begin(); TraceItX.captureGate = false
    ...
    @Test fun nativeConstructorAndOffMainMarkerFenceRealFreshFrameAdmissionUntilDetach() {
        ...
        val session = enabled()
        try {
            val native = com.traceitx.sensitive.TXSensitiveView(activity)
            assertEquals(true, native.getTag(com.traceitx.R.id.tx_sensitive))
            root.addView(native)
            var preDraw = false
            native.viewTreeObserver.addOnPreDrawListener {
                preDraw = true; assertEquals(true, native.getTag(com.traceitx.R.id.tx_sensitive)); true
            }
            ...
            Thread { TraceItX.markSensitive(late) }.apply { start(); join(2_000); assertFalse(isAlive) }
            assertTrue(VideoPrivacyRevocation.blocked)
            scheduler.drain(); assertEquals(2, created.size)
            org.robolectric.Shadows.shadowOf(android.os.Looper.getMainLooper()).idle()
            assertEquals(true, late.getTag(com.traceitx.R.id.tx_sensitive))
    ...
    @Test fun settingsAndDurationComeFromOnePublishedDecisionDuringAncillaryPublication() {
        ...
        val locker = Thread {
            try { com.traceitx.capture.sharedBreadcrumbBuffer.__holdLockForTesting {
