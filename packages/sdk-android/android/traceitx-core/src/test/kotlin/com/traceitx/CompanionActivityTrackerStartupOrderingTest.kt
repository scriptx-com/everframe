...
// `CompanionActivityTracker` (com.traceitx.companion.CompanionBadge.kt) was
// only ever registered from `TraceItX.startCompanionInternal()`, an
...
// callbacks. A plain-native host that calls `TraceItX.start()` from
// `Application.onCreate` (the canonical integration — see `start()`'s own
// doc comment) and only calls `TraceItX.startCompanion()` later, once its
...
class CompanionActivityTrackerStartupOrderingTest {
    ...
    fun activityResumedBeforeStartCompanion_stillGetsTheBadgeOnFirstAttach() = runTest(mainDispatcher) {
        ...
        TraceItX.startCompanionInternal(
            context = context,
            config = TraceItX.currentConfig,
            okHttpClient = OkHttpClient(),
            baseUrl = server.url("/").toString(),
        )
        assertTrue("announce must finish before the first attach", socketOpened.await(5, TimeUnit.SECONDS))
        org.junit.Assert.assertEquals("LMN-421", com.traceitx.companion.Companion.code.value)
