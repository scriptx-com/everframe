...
// `MultipartBody.Builder`. Mirrors `packages/sdk-ios/Sources/TraceItX/Transport/
...
//   X-TraceItX-Idempotency-Key: <uuid>
...
//   packages/sdk-android/android/traceitx-core/src/main/kotlin/com/traceitx/okhttp/` is intentionally
...
class MultipartUploader(private val client: OkHttpClient) {
    ...
    suspend fun upload(
        ...
        repeat(2) { attempt ->
            ...
            // @traceitx/react contract; sdk-ios MultipartUploader does the
            ...
            val req = Request.Builder()
                ...
                .header("X-TraceItX-Idempotency-Key", idempotencyKey)
                .apply {
            ...
            if (BuildConfig.DEBUG) {
                Log.d("TraceItX", "POST $ingestUrl envelopeBytes=${envelopeBytes.size} parts=${attachments.size}")
            }
    ...
    internal companion object {
        ...
        internal fun preparePayload(envelope: ByteArray, parts: List<Part>, replayAllowed: Boolean): Pair<ByteArray, List<Part>> {
            ...
            val kept = metadata?.filterNot { element ->
                ...
                val video = (item["format"] as? JsonPrimitive)?.content == "traceitx-video-v1" ||
