...
import com.traceitx.BuildConfig
...
import com.traceitx.capture.ResourceSampler
...
import com.traceitx.vitals.toVitalsServerConfig
...
class ReplaySession(
    ...
    private val activitySupplier: () -> Activity? = { TraceItX.__activitySupplier?.invoke() },
    ...
    private val originatingStartEpoch: Int = TraceItX.currentStartEpochVolatile(),
    private val captureConsent: Boolean = TraceItX.captureGate,
) {
    ...
    private fun currentGenerationValid(capturedEpoch: Int) = sessionLock.withLock {
        !isTornDown.get() && epoch.get() == capturedEpoch && TraceItX.currentStartEpochVolatile() == originatingStartEpoch
    }
    ...
    fun enableIfConfigured() = TraceItX.withReportAuthorizationLock {
        ...
        initialRefreshJob = TraceItX.sdkScope.launch { refreshConfigNow() }
        refreshLoopJob = TraceItX.sdkScope.launch {
    ...
    fun teardown() {
        ...
        if (!first) return
        // Session-owned: once TraceItX._replaySession is nulled nothing can
    ...
    internal suspend fun refreshConfigNow(): Unit = refreshMutex.withLock {
        ...
        com.traceitx.companion.CompanionBadgeServerConfigSignal.publish(latest.companionBadge) { currentGenerationValid(capturedEpoch) }
        BrandingServerConfigSignal.publish(latest.branding) { currentGenerationValid(capturedEpoch) }
        com.traceitx.vitals.VitalsServerConfigSignal.publish(latest.toVitalsServerConfig()) { currentGenerationValid(capturedEpoch) }
        com.traceitx.trigger.ShakeToReportTrigger.publishRemote(latest.shakeToReport?.enabled) {
            currentGenerationValid(capturedEpoch)
        }
        if (!wasIdentityEnabled && isIdentityEnabled(latest) && currentGenerationValid(capturedEpoch)) TraceItX.__warmIdentityToken()
        ...
        NetworkBodyCaptureState.applyConfig(if (succeeded) latest.networkBodies else null,
            ...
            locallyDisabled = NetworkBodyCaptureState.locallyDisabled(TraceItX.currentConfig) ||
    ...
    private fun reconcileVideoLocked() {
        ...
        if (decision?.settings == null || !captureConsent || !TraceItX.captureGate ||
        ...
        (videoStartupAdmission ?: VideoStartupAdmission.process).request(appContext) { allowed ->
            TraceItX.withReportAuthorizationLock {
                ...
                if (!currentGenerationValid(capturedEpoch) || generation != capturedGeneration ||
                    !authorized || activeOwner != null || recorder != null || !TraceItX.captureGate ||
    ...
    fun pause() = TraceItX.withReportAuthorizationLock {
        foreground = false; recorder?.pause()
        if (state == NativeVideoRecorder.State.BUFFERING) state = NativeVideoRecorder.State.SUSPENDED
    }
    fun resume() = TraceItX.withReportAuthorizationLock {
        foreground = true; recorder?.resume()
        if (state == NativeVideoRecorder.State.SUSPENDED) state = NativeVideoRecorder.State.BUFFERING
    }

    /** Called under TraceItX's capture coordinator, which serializes snapshots with start/kill epoch changes. */
    internal fun freezeOwnedCapture(): FrozenReportCapture {
        val binding = TraceItX.withReportAuthorizationLock {
            if (isTornDown.get() || activeOwner != null || TraceItX.currentStartEpochVolatile() != originatingStartEpoch) return@withReportAuthorizationLock null
            ...
            val origin = object : ReplayCaptureOrigin {
                ...
                override fun replayAllowed(owner: VideoOwner, generation: Long) = TraceItX.withReportAuthorizationLock {
                ...
                override fun cancel(owner: VideoOwner) = TraceItX.withReportAuthorizationLock {
        ...
        return try {
            val guard = { !isTornDown.get() && TraceItX.currentStartEpochVolatile() == originatingStartEpoch }
