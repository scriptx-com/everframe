...
// `CompanionSubmissionComposer.submit`'s `TraceItX.currentConfig` reads). Those
...
// bug: `TraceItX._startEpoch`, a monotonically increasing generation counter
 ...
 * instant.
 *
 * Obtain one only from [TraceItX.captureUserSnapshot] — the constructor is
...
data class TXCapturedUser internal constructor(
     ...
     */
    fun resolve(): TXUser? = TraceItX.resolveCapturedUser(this)
}

/**
 * The whole session a crash was captured under, read in ONE [TraceItX.stateLock]
 * critical section.
 *
 ...
 *
 * Obtain one only from [TraceItX.captureSessionSnapshot]. The constructor is
...
data class TXCapturedSession internal constructor(
     ...
     */
    val config: TraceItXConfig?,
    /**
     * [TraceItX] revocation counter as of the same critical section. Monotonic
     ...
     * `:traceitx-reporter-ui`, a DIFFERENT Gradle module from the one
     * declaring [TraceItX.killGenerationChanged] — which is `internal`, and
     ...
     * [TraceItX.killGenerationChanged] itself: that function takes a bare
    ...
    val isRevoked: Boolean
        get() = TraceItX.killGenerationChanged(killGeneration)
    ...
    val isSuperseded: Boolean
        get() = TraceItX.startEpochChanged(user.startEpoch)
}
