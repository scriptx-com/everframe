...
//
// Loads packages/protocol/data/*.json (mirrored into packages/sdk-android/android/traceitx-core/src/main/assets/traceitx/
...
//
// Mirrors `packages/sdk-ios/Sources/TraceItX/SharedData/SharedData.swift`.
package com.traceitx.shared
...
object SharedData {
     ...
     * inside `TraceItX.start(context, config)` before captureGate flips.
     */
    ...
    private inline fun <T> loadJson(name: String, parse: (String) -> T): T {
        val ctx = appContext ?: error(
            "SharedData not initialized — call SharedData.init(context) before reading shared JSON. " +
                "TraceItX.start(context, config) handles this automatically."
        )
        val text = ctx.assets.open("traceitx/$name").bufferedReader().use { it.readText() }
        return parse(text)
    }
}
