...
//   • `com.traceitx.capture.ScreenshotCapture.captureBeforeReporter`
//     for PNG bytes (needs the host Activity — supplied by the
//     `__captureProvider` seam below).
//   • `com.traceitx.capture.LogRingBuffer` / `NetworkRingBuffer` counts.
//   • `com.traceitx.outbox.JSONLOutbox` for the final ship path
...
//     the RN bridge (TraceItXModule.startCompanion) using the host's
...
object CompanionCaptureBridge {
    ...
    fun onShotBinary(correlationId: String, shotId: String, bytes: ByteArray, ws: WebSocket) {
        ...
        if (pending.shots?.none { it.shotId == shotId } != false) {
            android.util.Log.w("TraceItX.companion", "shot.binary DROPPED — shot_id was not announced by the submit")
            return
        }
        ...
        submitScope.launch {
            ...
            when (result) {
                is SubmitResult.Ok -> {
                    ws.send(jsonEnvelope.encodeToString(
                        RelayMessage.serializer(),
                        com.traceitx.protocol.generated.ReportCompleted(
                ...
                is SubmitResult.Err -> {
                    android.util.Log.w("TraceItX.companion", "submit failed (reason=${result.reason})")
                    ws.send(jsonEnvelope.encodeToString(
                        RelayMessage.serializer(),
                        ReportFailed(
                            correlationId = message.correlationId,
    ...
    /**
     * PR-fix 1 — called from `TraceItXModule.stopCompanion()`. The companion
     ...
     * `TraceItXModule.stopCompanion()`. Drops both providers and any
