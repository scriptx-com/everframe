...
//
// Opt-in OkHttp Interceptor. Per RESEARCH Finding 10 + CONTEXT D-03 — TraceItX
...
// extension `OkHttpClient.Builder.addTraceItXInterceptor()` defined in
...
// has called TraceItX.kill() the interceptor short-circuits to chain.proceed
...
import com.traceitx.capture.NetworkBodyTee
import com.traceitx.capture.NetworkBreadcrumbAdapter
...
public class TraceItXInterceptor internal constructor() : Interceptor {

    override fun intercept(chain: Interceptor.Chain): Response {
        val requestEpoch = TraceItX.currentStartEpochVolatile()
        ...
        if (!TraceItX.captureGate) return chain.proceed(req)
        ...
        return try {
            ...
            // The ring buffer itself also gates on TraceItX.captureGate
            // (honorsKillGate, default true) as the authoritative backstop
            // against the check-then-act race between this line and the
            // actual push.
            if (!TraceItX.captureGate || TraceItX.currentStartEpochVolatile() != requestEpoch) return resp
            ...
            if (!TraceItX.captureGate || TraceItX.currentStartEpochVolatile() != requestEpoch) throw e
