...
//
// Wave-3 file-ownership: this plan does NOT touch TraceItX.kt. Plan 05-06
// (Wave 4) is the sole writer that wires `LogCapture.install()` from
// TraceItX.start()'s detached coroutine and `LogCapture.uninstall()` from
// TraceItX.kill().
...
internal object LogCapture {
    ...
    private fun installOwned(logs: Long, crumbs: Long) {
        ...
        txGuardVoid("LogCapture.installTimber") {
            ...
            if (ownedTimberTree == null && TimberDetector.timberOnClasspath()) {
                val tree = buildTraceItXTree(active, logs, crumbs)
     ...
     * call site of buildTraceItXTree() is gated by
     ...
     * unit tests.
     */
    private fun buildTraceItXTree(active: AtomicBoolean, logs: Long, crumbs: Long): timber.log.Timber.Tree =
    ...
    /** Map android.util.Log priority constants to TraceItX log levels. */
