...
//    `start()`.
//
//    Codex round-4, #2 — that generation is TraceItX's own `_killGeneration`
...
//    contract ([com.traceitx.TraceItX.trackPlayer]'s KDoc). Round 2 made
...
// predicate takes `TraceItX.stateLock`, so the lock order is
...
// `attach()` that calls `TraceItX.trackPlayer(...)` re-entrantly is fine on
...
internal object VitalsRuntime {
     ...
     *
     * `kill()` bumps `TraceItX._killGeneration` synchronously inside its
     ...
     * `TraceItX.stateLock` separately, so a `kill()` bumping between them
    ...
    /**
     * Codex round-3, Critical 1, re-based by round-4 #2 onto TraceItX's own
     ...
     * `TraceItX._killGeneration`, which `kill()` bumps synchronously in its
     * `stateLock` critical section.
     *
     * Reading it takes `TraceItX.stateLock` while this monitor is held — the
     * one nesting this file already performs (see [install]'s predicate).
     */
    private val DEFAULT_KILL_GENERATION: () -> Long = { com.traceitx.TraceItX.currentKillGeneration() }
    ...
    fun install(c: VitalsController, isCurrent: () -> Boolean = { true }) {
        ...
        val (old, drained) = synchronized(this) {
            ...
            // `TraceItX.stateLock` on their own, so a `kill()` can bump
     ...
     * object's monitor HELD (it reads `TraceItX.stateLock` through
     ...
     * and drops it. Called from `TraceItX.kill()`, from `TraceItX.start()`'s
     ...
     * runtime-monitor -> `TraceItX.stateLock`, identical to [install]'s
    ...
    fun shutdown(dropPending: Boolean = true, ifCurrent: () -> Boolean = { true }) {
        val (old, dropped) = synchronized(this) {
            ...
            if (dropPending) {
                ...
                //
                // Codex round-4, #2: read from TraceItX rather than bumped
     ...
     * without standing up a whole `TraceItX.start()`/`kill()` cycle. Cleared
     * by [resetForTesting].
     */
