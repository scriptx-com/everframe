...
//
// :traceitx-core — capture + envelope + transport + outbox.
//
...
// they live in :traceitx-reporter-ui.
...
plugins {
    ...
    // com.traceitx.TXScreen (screen-marker spec 2026-07-14), runtime-only,
...
android {
    ...
    defaultConfig {
        ...
        // traceitxVersion (overridable with -PtraceitxVersion=X.Y.Z). The
        ...
        // so `TraceItX.SDK_VERSION` can never drift from the Maven coordinate
        // again.
    ...
    // no string literal "TRACEITX_DEV_INGEST_URL" anywhere in release
    // bytecode). The debug variant picks up the env var at library build
    // time so local sample apps can target localhost / a LAN IP.
    buildTypes {
        getByName("debug") {
            val devUrl = System.getenv("TRACEITX_DEV_INGEST_URL")?.takeIf { it.isNotBlank() }
                ?: "http://10.0.2.2:8787"
        ...
        getByName("release") {
            ...
            //
            // A `System.getenv("TRACEITX_DEV_INGEST_URL")` fallback lived here
            ...
            buildConfigField("String", "INGEST_URL", "\"https://traceitx.com\"")
    ...
    buildFeatures {
        ...
        // @Composable (com.traceitx.TXScreen); the AGP buildFeature flag is
    ...
    testOptions {
        unitTests {
            ...
            all { test ->
                ...
                test.systemProperty("traceitxCrashDetailsCorpus", detailsCorpus.absolutePath)
                providers.gradleProperty("traceitxCrashDetailsOutput").orNull?.let { output ->
                    ...
                    test.systemProperty("traceitxCrashDetailsOutput", destination.absolutePath)
                    test.outputs.upToDateWhen { false }
                    test.outputs.cacheIf { false }
                }
                val fixture = providers.gradleProperty("traceitxR8FixtureJar").orNull
                if (fixture == null) {
                    test.exclude("**/acceptance/R8CaptureAcceptanceTest*")
                } else {
                    listOf("traceitxR8FixtureJar", "traceitxR8MappingId", "traceitxR8EnvelopeOutput", "traceitxR8AppId").forEach { name ->
...
// It class-loads `com.traceitx.companion.CompanionAnnounce` by name, and that
...
// whole class fails with `NoClassDefFoundError: com/traceitx/companion/
...
// this commit: `:traceitx-core:testReleaseUnitTest` produces 162 failures,
// every one a NoClassDefFoundError / NoSuchMethodError, because these tests
// name internal types (TraceItX's own gates and buffers, BreadcrumbAdapters,
...
// green. `:traceitx-reporter-ui` carries the identical block for the identical
// reason.
//
...
// Plan 05-02 — keep `assets/traceitx/*.json` in lockstep with `packages/protocol/data/*.json`.
...
tasks.register<Copy>("copyProtocolData") {
    ...
    into("src/main/assets/traceitx")
    include("*.json")
}
tasks.named("preBuild") { dependsOn("copyProtocolData") }

dependencies {
    api(project(":traceitx-protocol"))
    ...
    // idempotent plant + uprootAll-never-called. MockWebServer for TraceItXInterceptorTest.
...
// ---------------------------------------------------------------------------
// Maven Central publishing — coordinates: com.traceitx:core:<version>
//
// Required env vars (see PUBLISHING.md for setup):
//   * OSSRH_USERNAME / OSSRH_PASSWORD  — Sonatype Central tokens
//   * SIGNING_KEY_ID                   — last 8 hex digits of the GPG key
...
afterEvaluate {
    publishing {
        publications {
            create<MavenPublication>("release") {
                ...
                pom {
                    name.set("TraceItX Android — core")
                    description.set("Native Android SDK core for TraceItX bug reporting: capture, envelope, transport, outbox.")
        ...
        repositories {
            maven {
                ...
                // accepts legacy-style Maven deploys and auto-routes them into
                // the new Central Portal, where we finalize the release via
                // central.sonatype.com → Deployments. See PUBLISHING.md.
    ...
    signing {
        ...
        if (signingKey != null && signingPassword != null) {
            ...
            logger.warn("[traceitx-core] GPG signing skipped — SIGNING_KEY / SIGNING_PASSWORD not set. publishToMavenLocal still works; Sonatype upload will fail without signatures.")
        }
    }
}
