...
//
// Purpose:
//   1. Inject `everframe-keep.pro` (a copy of `:everframe-core/consumer-rules.pro`)
...
//
//   plugins { id("dev.everframe") version "1.2.0" }
//
// or, for project-local development:
//
//   plugins { id("dev.everframe") }   // resolved via includeBuild("../android")
...
public class EverframePlugin : Plugin<Project> {
    ...
    /**
     * Copies `/everframe-keep.pro` (bundled in the plugin JAR resources) to
     * `<projectBuildDir>/everframe-gradle-plugin/everframe-keep.pro`. Idempotent —
     * re-extracts on each apply so a refreshed plugin version is honored.
     */
    private fun extractKeepFile(project: Project): File {
        val out = File(project.layout.buildDirectory.asFile.get(), "everframe-gradle-plugin/everframe-keep.pro")
        out.parentFile.mkdirs()
        val resource = javaClass.classLoader.getResourceAsStream("everframe-keep.pro")
            ?: error("everframe-gradle-plugin: bundled everframe-keep.pro missing from plugin JAR resources.")
        resource.use { input ->
     ...
     * R8 / ProGuard preserves Everframe symbols in the customer's release APK.
     */
     ...
     * keeps `:everframe-gradle-plugin`'s runtime classpath minimal.
     */
