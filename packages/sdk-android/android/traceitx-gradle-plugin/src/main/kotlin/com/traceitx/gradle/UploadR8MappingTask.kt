...
public abstract class UploadR8MappingTask : DefaultTask() {
    ...
    @TaskAction
    internal fun upload() {
        require(!System.getenv("TRACEITX_API_TOKEN").isNullOrEmpty()) {
            "TRACEITX_API_TOKEN is required to upload an R8 mapping"
        }
        val applicationId = appId.orNull ?: error("traceitxR8.appId is required to upload an R8 mapping")
        require(runCatching { UUID.fromString(applicationId) }.isSuccess) {
            "traceitxR8.appId must be a UUID"
        }
