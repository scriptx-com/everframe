...
import com.traceitx.vitals.protocolForMime
import com.traceitx.vitals.sanitizeSource
...
class Media3Integration internal constructor(
    ...
    // `com.traceitx.envelope.InternalLogger.recordSafeWrapFailure` is `internal` to
    // :traceitx-core — Kotlin's `internal` is module-scoped, so it is invisible here
    // even though this module depends on core via `api(project(":traceitx-core"))`.
    ...
    private inline fun guard(block: () -> Unit) {
        try { block() } catch (t: Throwable) { Log.w("TraceItX", "Media3Integration listener callback failed", t) }
    }
    ...
    override fun attach(ctx: PlayerIntegrationContext): Boolean {
        ...
        val accepted = try {
            postOnPlayerThread(
                Runnable {
                    ...
                    val ok = try {
                        ...
                        Log.w("TraceItX", "Media3Integration addAnalyticsListener failed", t)
                        false
                    }
                    if (!ok) {
            ...
            Log.w("TraceItX", "Media3Integration attach seed failed", t)
            false
        }
            ...
            postOnPlayerThread(
                Runnable {
                    try {
                        ...
                        Log.w("TraceItX", "Media3Integration release observer subscription failed", t)
                    }
                },
            )
        } catch (t: Throwable) {
            Log.w("TraceItX", "Media3Integration release observer post failed", t)
            false
        }
        if (!installed) {
    ...
    override fun describe(ctx: PlayerIntegrationContext) {
        try {
            ...
            Log.w("TraceItX", "Media3Integration describe post failed", t)
        }
    }
    ...
    override fun detach(onComplete: () -> Unit) {
        val posted = try {
            ...
            Log.w("TraceItX", "Media3Integration detach post failed", t)
            false
        }
        if (!posted) {
            try {
