...
///
/// `nil` means there is NO current item. A non-nil value whose `url` is nil is an item the SDK
...
final class AVPlayerFacade: PlayerFacade, @unchecked Sendable {
    ...
    /// make a second `observeRelease` on the same `AVPlayer` — two SDK
    ...
    /// `AVPlayerItem.asset` is `@MainActor`-isolated in the Xcode 26 SDK for one
    ...
    /// ask an asset which content-key sessions it has been added to, and the SDK never sees the
