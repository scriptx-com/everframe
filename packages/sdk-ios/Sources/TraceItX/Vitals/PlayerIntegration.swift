...
public protocol PlayerIntegration: AnyObject {
    ...
    /// the caller's thread): `TraceItX.kill()` and a superseding `TraceItX.start()` run this
    ...
    /// itself takes to reach `onComplete`. That includes the SDK's own AVPlayer integration on
...
/// rollback was a plain `detach()`, which for the SDK's own AVPlayer integration also clears the
...
/// can tell the two apart, and the SDK's own is the only one that does. A customer integration
