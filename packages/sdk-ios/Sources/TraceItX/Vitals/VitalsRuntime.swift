...
// TraceItX locks are reached from inside it, and the difference matters
...
// takes TraceItX.stateLock. So the nesting to respect is runtime lock →
...
final class VitalsRuntime: @unchecked Sendable {
    ...
    /// critical section (round-5, #4): kill() bumps TraceItX's counter synchronously inside its
    ...
    /// `TraceItX._startEpoch` the published controller was BUILT FOR, written beside it in
    ...
    private var killGeneration: () -> UInt64 = { TraceItX.currentKillGeneration() }
    ...
    /// the generation still in force. Called with `lock` HELD (it takes TraceItX.stateLock
    ...
    func resetForTesting() {
        ...
        withLock { pending.removeAll(); controllerKillGen = 0; controllerStartEpoch = nil; killGeneration = { TraceItX.currentKillGeneration() } }
    }
    ...
    func __setKillGenerationForTesting(_ source: (() -> UInt64)?) {
        withLock { killGeneration = source ?? { TraceItX.currentKillGeneration() } }
    }
}
