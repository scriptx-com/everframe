...
// `NSLock` to mirror the precedent in `TraceItX.swift:33` ("@unchecked
...
/// when `TraceItXReporterUI`'s `CompanionPinPresenter` is actually linked and
/// installed (`CompanionAPI.__builtinPinUiInstalled`) — a host that links
/// only `TraceItXKit` degrades honestly to the legacy no-PIN announce rather
...
public final class RelayWSClient: NSObject, URLSessionWebSocketDelegate, @unchecked Sendable {
    ...
    internal nonisolated(unsafe) var __sendHook: ((RelayMessage) -> Void)?
    ...
    /// Non-nil exactly when the host supplied an SDK key. Its presence is what
    ...
    private func resetSubmitFraming() {
        ...
        NotificationCenter.default.post(
            name: .traceItXCompanionResetSubmitFraming, object: self)
    }
    ...
    /// Device identity (naming spec 2026-08-24) resolved ONCE, lazily, at the
    ...
    /// - Parameters:
    ///   - sdkKey: The host's TraceItX key (`TraceItXConfig.appId`). Supply it
    ...
    private func openSocket(_ url: URL, generation: UInt64) {
        ...
        // referencing it — see `isCurrentTask`. Nothing else in this SDK reads
        // or writes `taskDescription`, and these tasks belong to a session we
        // own exclusively.
        let stamp = "traceitx-relay-\(generation)"
    ...
    /// capture-bridge submit path sends it as the `X-TX-Companion-Attribution`
    ...
    /// Encode a `RelayMessage` as JSON text and send as a WS text frame.
    /// No-op if the task isn't open (silent drop — relay will be reconnected
    /// by the scheduler).
    public func send(_ msg: RelayMessage) {
    ...
    internal func handleControl(_ text: String) {
        ...
        guard let msg = try? JSONDecoder().decode(RelayMessage.self, from: data) else {
        ...
        case .reportRequest(let r):
            ...
            // mirroring Android's `is ReportRequest` branch. Read the state
            ...
            if companion?.state == .reportInProgress {
                ...
                send(.reportRejected(ReportRejected(
            ...
            NotificationCenter.default.post(
                name: .traceItXCompanionReportRequested,
                object: self,
                userInfo: ["correlation_id": r.correlationId])
        case .reportSubmit(let r):
            print("[traceitx-companion] WS recv report.submit corrId=\(r.correlationId)")
            ...
            NotificationCenter.default.post(
                name: .traceItXCompanionReportSubmit,
                object: self,
                userInfo: [
        ...
        case .reportCompleted(let r):
            ...
            NotificationCenter.default.post(
                name: .traceItXCompanionReportCompleted,
                object: self,
        ...
        case .reportFailed(let r):
            ...
            companion?.__finishReport(correlationId: r.correlationId)
            NotificationCenter.default.post(
                name: .traceItXCompanionReportFailed,
                object: self,
        ...
        case .previewStart(let msg):
            print("[traceitx-companion] preview.start corrId=\(msg.correlationId)")
        ...
        case .previewStop(let msg):
            ...
            print("[traceitx-companion] preview.stop corrId=\(msg.correlationId) reason=\(msg.reason)")
            routePreviewStop()
        ...
        case .companionName(let n):
            // Device naming (spec 2026-08-24): resolved display name pushed
    ...
    private func handleBinary(_ bytes: Data) {
        ...
        // strict D-05 order (reporter App.tsx handleSubmit). If no submit is
        ...
        if let shotBinding {
            NotificationCenter.default.post(
                name: .traceItXCompanionReportShotBinary,
                object: self,
                userInfo: [
        ...
        print("[traceitx-companion] WS recv binary bytes=\(bytes.count) corrId=\(corrId ?? "<none>")")
        guard let corrId = corrId else {
            print("[traceitx-companion] WS binary DROPPED — no pending submit corrId")
            return
        }
        NotificationCenter.default.post(
            name: .traceItXCompanionReportSubmitBinary,
            object: self,
            userInfo: [
    ...
    // MARK: - App lifecycle (Pitfall 4 — backgrounding kills WS)
    //
...
public extension Notification.Name {
    ...
    static let traceItXCompanionReportRequested =
        Notification.Name("com.scriptx.traceitx.companionReportRequested")

    /// Posted when the TV runtime receives a `report.submit` frame from the
    /// phone. userInfo: `["correlation_id": String, "submit": ReportSubmit]`.
    /// Subscribers apply `includes` + `description.redactions` to the
    /// in-memory draft and submit via the existing JSONLOutbox.
    static let traceItXCompanionReportSubmit =
        Notification.Name("com.scriptx.traceitx.companionReportSubmit")
    ...
    static let traceItXCompanionReportSubmitBinary =
        Notification.Name("com.scriptx.traceitx.companionReportSubmitBinary")
    ...
    static let traceItXCompanionReportShotBinary =
        Notification.Name("com.scriptx.traceitx.companionReportShotBinary")

    /// Posted when the phone leg ends and every half-assembled submit must be
    /// forgotten. No userInfo.
    static let traceItXCompanionResetSubmitFraming =
        Notification.Name("com.scriptx.traceitx.companionResetSubmitFraming")

    /// Posted on `report.completed`. userInfo: `["correlation_id": String, "event_id": String]`.
    static let traceItXCompanionReportCompleted =
        Notification.Name("com.scriptx.traceitx.companionReportCompleted")

    /// Posted on `report.failed`. userInfo: `["correlation_id": String, "reason": String]`.
    static let traceItXCompanionReportFailed =
        Notification.Name("com.scriptx.traceitx.companionReportFailed")
}
