...
// exist).
//
// Strict-concurrency (`TraceItXKit` builds with `StrictConcurrency=complete`):
// `CompanionBadge` is `@unchecked Sendable`, NOT `@MainActor` — matching
// `RelayWSClient`'s own idiom (see that file's header) rather than
// `CompanionPinPresenter`'s (`TraceItXReporterUI` has no strict-concurrency
...
/// a hop through `TraceItX.currentReplayConfig()` because that accessor is
...
/// resolves to an SDK overlay — leaking the badge into every capture path
...
final class TXCompanionBadgeWindow: UIWindow {}
...
public final class CompanionBadge: @unchecked Sendable {
    ...
    /// which reads the SDK's fetched-config snapshot — a sync read of
    ...
    private func defaultSurface() -> UIView? {
        ...
        let overlay = TXCompanionBadgeWindow(windowScene: scene)
