...
//   1. Combine `@Published` on `TraceItX.shared.companion.state` (preferred for
//      SwiftUI / Combine consumers).
//   2. NotificationCenter via `.traceItXCompanionStateChange` (UIKit-only hosts
//      or Objective-C bridges).
//
// Mirrors the dual-surface pattern locked for `ReportAPI.isPresenting` in
// `TraceItX.swift:236-279` (Phase 05.1).
//
...
///   connected" indicator (chrome is host-rendered, not SDK-shipped).
...
public extension Notification.Name {
    /// Posted on every flip of `TraceItX.shared.companion.state`. userInfo:
    ...
    static let traceItXCompanionStateChange =
        Notification.Name("com.scriptx.traceitx.companionStateChange")

    /// Posted on every flip of `TraceItX.shared.companion.pairUrl`. userInfo:
    /// `["pairUrl": String?]` (NSNull when nil to round-trip through userInfo).
    static let traceItXCompanionPairUrlChange =
        Notification.Name("com.scriptx.traceitx.companionPairUrlChange")

    /// Posted on every flip of `TraceItX.shared.companion.code` — the short
    /// display code from `/api/companion/announce` (spec 2026-08-07).
    /// userInfo: `["code": String?]` (NSNull when nil to round-trip through
    /// userInfo).
    static let traceItXCompanionCodeChange =
        Notification.Name("com.scriptx.traceitx.companionCodeChange")

    /// Posted on every flip of `TraceItX.shared.companion.attachedUserName` —
    ...
    static let traceItXCompanionAttachedUserNameChange =
        Notification.Name("com.scriptx.traceitx.companionAttachedUserNameChange")

    /// Posted on every flip of `TraceItX.shared.companion.resolvedName` — the
    ...
    static let traceItXCompanionResolvedNameChange =
        Notification.Name("com.scriptx.traceitx.companionResolvedNameChange")

    /// Posted on every flip of `TraceItX.shared.companion.attachChallenge`
    ...
    /// on set, `[:]` on clear.
    static let traceItXCompanionAttachChallengeChange =
        Notification.Name("com.scriptx.traceitx.companionAttachChallengeChange")
}
