...
//
// Exposed to hosts as `TraceItX.shared.companion`.
//
...
public struct CompanionAttachChallenge: Sendable, Equatable {
    ...
    /// Lives here (TraceItXKit), not `CompanionPinPresenter` (TraceItXReporterUI,
...
public final class CompanionAPI: ObservableObject, @unchecked Sendable {
    ...
    /// `.traceItXCompanionStateChange` only on a real value change.
    @Published public private(set) var state: CompanionState = .unpaired {
        didSet {
            guard oldValue != state else { return }
            NotificationCenter.default.post(
                name: .traceItXCompanionStateChange,
                object: self,
    ...
    /// `nil` until the relay issues a `pair.created` message. SDK ships zero
    ...
    @Published public private(set) var pairUrl: String? = nil {
        didSet {
            ...
            NotificationCenter.default.post(
                name: .traceItXCompanionPairUrlChange,
                object: self,
    ...
    /// list. `nil` when companion discovery wasn't attempted (no SDK key
    ...
    @Published public private(set) var code: String? = nil {
        didSet {
            ...
            NotificationCenter.default.post(
                name: .traceItXCompanionCodeChange,
                object: self,
    ...
    @Published public private(set) var attachedUserName: String? = nil {
        didSet {
            ...
            NotificationCenter.default.post(
                name: .traceItXCompanionAttachedUserNameChange,
                object: self,
    ...
    @Published public private(set) var resolvedName: String? = nil {
        didSet {
            ...
            NotificationCenter.default.post(
                name: .traceItXCompanionResolvedNameChange,
                object: self,
    ...
    @Published public private(set) var attachChallenge: CompanionAttachChallenge? = nil {
        didSet {
            ...
            NotificationCenter.default.post(
                name: .traceItXCompanionAttachChallengeChange,
                object: self,
                userInfo: userInfo
            )
        }
    ...
    /// Set true by `TraceItXReporterUI`'s `CompanionPinPresenter.install()`.
    ...
    /// (`CompanionPinPresenter.install()`) lives in the `TraceItXReporterUI`
    ...
    /// sdk-react-native/ios` is a separate CocoaPods pod (`TraceItXRN`)
    /// consuming `TraceItXKit` as a built product, not a package(set) peer.
    /// `TraceItXBridge.swift` (`companionAttachPinUi`/`startCompanion`) is
    ...
    // MARK: - Single-writer seams (SDK-internal; hosts must NEVER call)
    ...
    /// transition-barrier work already deferred elsewhere in this SDK (see
    /// `TraceItX.swift`'s `discardCapturedEvidenceForIdentityChange` doc for
