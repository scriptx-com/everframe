...
//
// Lifecycle:
//   1. Hosts wire it via `TraceItX.shared.start(config:)` when
//      `config.enableCompanion == true` (Task 2 wiring in TraceItX.swift).
//   2. Subscribes to `.traceItXCompanionReportRequested`. On firing:
//        a. Captures key-window screenshot (PNG + metadata).
//        b. Constructs `RelayMessage.reportAssembled(...)` with counts and
...
//   3. Subscribes to `.traceItXCompanionReportSubmit` (text) and
//      `.traceItXCompanionReportSubmitBinary` (PNG bytes). When both arrive
...
public final class CompanionCaptureBridge: @unchecked Sendable {
    ...
    /// may keep a replacement SDK session frozen.
    ...
    private struct CaptureStash {
        ...
        /// `TraceItX.shared.__consumePendingAttachments()` at the moment of
    ...
    public init(client: RelayWSClient) {
        ...
        requestToken = NotificationCenter.default.addObserver(
            forName: .traceItXCompanionReportRequested,
            object: nil,
            queue: nil
        ) { [weak self] note in
        ...
        submitToken = NotificationCenter.default.addObserver(
            forName: .traceItXCompanionReportSubmit,
            object: nil,
            queue: nil
        ) { [weak self] note in
            print("[traceitx-companion] notif: report.submit fired userInfoKeys=\(Array(note.userInfo?.keys.map { "\($0)" } ?? []))")
            guard let self else { print("[traceitx-companion] report.submit — self gone"); return }
            guard let corrId = note.userInfo?["correlation_id"] as? String,
                  let submit = note.userInfo?["submit"] as? ReportSubmit else {
                print("[traceitx-companion] report.submit — malformed userInfo, dropping")
                return
            }
        ...
        submitBinaryToken = NotificationCenter.default.addObserver(
            forName: .traceItXCompanionReportSubmitBinary,
            object: nil,
            queue: nil
        ) { [weak self] note in
            print("[traceitx-companion] notif: report.submit.binary fired userInfoKeys=\(Array(note.userInfo?.keys.map { "\($0)" } ?? []))")
            guard let self else { print("[traceitx-companion] report.submit.binary — self gone"); return }
            guard let corrId = note.userInfo?["correlation_id"] as? String,
                  let bytes = note.userInfo?["bytes"] as? Data else {
                print("[traceitx-companion] report.submit.binary — malformed userInfo, dropping")
                return
            }
            self.tryRunSubmit(correlationId: corrId, bakedBytes: bytes)
        }
        resetFramingToken = NotificationCenter.default.addObserver(
            forName: .traceItXCompanionResetSubmitFraming,
            object: nil,
            queue: nil
        ) { [weak self] note in
        ...
        shotBinaryToken = NotificationCenter.default.addObserver(
            forName: .traceItXCompanionReportShotBinary,
            object: nil,
            queue: nil
        ) { [weak self] note in
            guard let self else { return }
            guard let corrId = note.userInfo?["correlation_id"] as? String,
                  ...
                  let bytes = note.userInfo?["bytes"] as? Data else {
                print("[traceitx-companion] shot.binary — malformed userInfo, dropping")
                return
            }
            ...
            guard fits else {
                ...
                print("[traceitx-companion] shot.binary DROPPED — retained-bytes ceiling reached")
                self.sendFailed(correlationId: corrId, reason: "payload_too_large")
                self.dropEntries(correlationId: corrId)
                return
            }
            guard announced else {
                print("[traceitx-companion] shot.binary DROPPED — not announced by a pending submit")
                return
            }
            self.tryRunSubmit(correlationId: corrId, bakedBytes: nil)
        }
    ...
    //
    // The native TXReporterPresenter freezes crumbs+replay at reporter-open
    ...
    internal static func beginReportCaptureLifecycle() {
        captureOwner = nil
        #if canImport(UIKit)
        TraceItX.shared.__replayCancel()
        TraceItX.shared.__replayFreeze()
        #endif
    }
    ...
    @MainActor private func finishReportCapture(correlationId: String, cancelled: Bool) {
        ...
        if cancelled {
            ...
            #if canImport(UIKit)
            TraceItX.shared.__replayReporterDidClose()
            #endif
        }
    }
    ...
    private func handleReportRequest(correlationId: String,
        ...
        // `TXReporterPresenter.openAndAwait()`'s identical call. Placed
        ...
        //    modal (TXReporterPresenter.swift:72). If two report.requests fire
        //    back-to-back and the host called setExtra only before the first,
        //    the second envelope MUST see (nil, nil) here.
        let (hostExtraRaw, _) = await TraceItX.shared.__consumePendingAttachments()
    ...
    private func sendAssembled(correlationId: String,
        ...
        let toggles = ReportAssembledToggles(
    ...
    private func recordPendingSubmit(correlationId: String, submit: ReportSubmit) {
        // DEV-ONLY trace — REMOVE BEFORE PROD
        print("[traceitx-companion] recordPendingSubmit corrId=\(correlationId) title=\"\(submit.title)\" descLen=\(submit.description.text.count) includes.logs=\(submit.includes.logs) includes.network=\(submit.includes.network)")
        bufferLock.lock()
        defer { bufferLock.unlock() }
        let now = Date()
        let cutoff = now.addingTimeInterval(-Self.stashTTL)
    ...
    private func tryRunSubmit(correlationId: String, bakedBytes: Data?) {
        // DEV-ONLY trace — REMOVE BEFORE PROD
        print("[traceitx-companion] tryRunSubmit corrId=\(correlationId) bakedBytes=\(bakedBytes?.count ?? -1)")
        #if canImport(UIKit)
        ...
        guard let bytes = bufferedPrimary else {
            print("[traceitx-companion] tryRunSubmit corrId=\(correlationId) — waiting on binary frame")
            return
        }
        print("[traceitx-companion] tryRunSubmit corrId=\(correlationId) pending=\(pending != nil) stash=\(stash != nil)")
        ...
        guard let pending = pending else {
            ...
            print("[traceitx-companion] tryRunSubmit DROPPED — no pending submit for corrId=\(correlationId)")
            return
        }
        guard let stash = stash else {
            // No capture stash — phone shouldn't have submitted without an
            // assembled frame, but defend anyway.
            print("[traceitx-companion] tryRunSubmit FAILED — no_capture corrId=\(correlationId)")
        ...
        if announcedShotIds.contains(where: { arrivedShots[$0] == nil }) {
            print("[traceitx-companion] tryRunSubmit corrId=\(correlationId) — waiting on \(announcedShotIds.count - arrivedShots.count) shot binaries")
            return
        }
        ...
        // must never be re-read from the live client either. `TXUser` is a
        ...
        // which project's SDK key the upload goes out under. See
        // `TXCapturedUser.swift`.
        ...
        Task { @MainActor [weak self] in
            ...
            let extraShots = extraShotPngs.compactMap { data -> ReporterSubmission.Inputs.Shot? in
                guard extraUsed + data.count <= extraBudget else {
                    print("[traceitx-companion] extra shot dropped — request byte budget exhausted")
                    return nil
                }
            ...
            var inputs = ReporterSubmission.Inputs(
                ...
                // MUST NOT be re-read from `TraceItX.shared` here, for the
            ...
            print("[traceitx-companion] calling ReporterSubmission.submit corrId=\(correlationId) bakedImageSize=\(bakedImage.size) hostExtra=\(stash.hostExtra != nil)")
            do {
                ...
                NSLog("[TraceItX] companion submit threw (\(type(of: error)): \(error.localizedDescription))")
                self.sendFailed(correlationId: correlationId,
                                reason: "ingest_error")
            }
            self.dropEntries(correlationId: correlationId)
    ...
    @MainActor private func sendCompleted(correlationId: String, eventId: String) {
        client?.send(.reportCompleted(ReportCompleted(
            correlationId: correlationId,
            eventId: eventId,
    ...
    private func sendFailed(correlationId: String, reason: String) {
        let frame = RelayMessage.reportFailed(ReportFailed(
            correlationId: correlationId,
            reason: reason,
            type: "report.failed"))
        Task { @MainActor [weak self] in
    ...
    /// subscribes and forwards to the JS-side `traceitx.companion
    /// .reportRequested` event so the companion module can walk the React
    ...
    public static let shouldAttachReactTreeNotification =
        Notification.Name("traceItXCompanionShouldAttachReactTree")
