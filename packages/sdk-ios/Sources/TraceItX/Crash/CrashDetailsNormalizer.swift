...
func normalizeCrashDetails(_ options: CaptureExceptionOptions?,
    ...
    return withoutActuallyEscaping(redact) { redact in
        ...
        let severity: TraceItXProtocol.ErrorSeverity
...
private func fitCrashDetails(metadata initial: OwnedCrashValue?, context: String?,
                             severity: TraceItXProtocol.ErrorSeverity, truncated: Bool) -> CrashDetails {
    let encoder = EnvelopeBuilder.makeJSONEncoder()
    func generated(_ metadata: OwnedCrashValue?, loss: Bool) throws -> CrashDetails {
        // JSONAny intentionally has no value initializer. The bounded, owned
        // tree is the only input to this encode/decode boundary.
        let values = try metadata.map { try JSONDecoder().decode([String: JSONAny].self, from: encoder.encode($0)) }
        return CrashDetails(context: context, metadata: values, severity: severity, truncated: loss ? true : nil)
    }
    func fits(_ details: CrashDetails, metadata: OwnedCrashValue?, loss: Bool) throws -> Bool {
    ...
    return CrashDetails(context: context, metadata: nil, severity: severity, truncated: true)
}
