...
// (`xcodebuild test -scheme TraceItX-Package -destination 'platform=iOS
...
///
/// Reporter-UI's own controls (the bug-report composer, annotation toolbar,
...
enum TapBreadcrumbAdapter {
    ...
    /// Install-once. Wired from `TraceItX.start(config:)` (via
    ...
    @MainActor
    static func deriveLabelAndData(sender: Any?) -> (label: String, data: [String: JSONAny]) {
...
/// is `TraceItX.addBreadcrumb(kind: .navigation, ...)` called manually from
...
/// first-class escape hatch: `TraceItX.shared.recordScreen("Name")` from
...
enum NavigationBreadcrumbAdapter {
    ...
    /// transition core below is shared with `TraceItX.recordScreen`.
    @MainActor
...
/// analogue of Android's `TXWindowCallbackWrapper.dispatchTouchEvent`
...
///
/// Reporter-UI taps reach the LIVE buffer like every other tap — it's the
...
enum BreadcrumbTapNavAdapters {
    /// Wired from `TraceItX.start(config:)` immediately after
