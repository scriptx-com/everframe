...
public enum SensitiveRectRegistry {
    ...
    /// for every sensitive view: TXSensitiveView subclass, tx_isSensitive=true, or UITextField.isSecureTextEntry=true.
    ...
    /// Mark a view as sensitive. Used by `TraceItX.shared.markSensitive(_:)` — the public passthrough
    /// is wired in 04-06 atomically alongside other start()/kill() integration to avoid Wave-3 file
    /// collisions on TraceItX.swift.
    public static func mark(_ view: UIView) {
        view.tx_isSensitive = true
    }
    ...
    public static func isSensitive(_ view: UIView) -> Bool {
        return view is TXSensitiveView
            || view.tx_isSensitive
