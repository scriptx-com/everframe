...
public final class NetworkBodyCaptureGate: @unchecked Sendable {
    ...
    /// `networkCaptureConfiguration()` attaches `TXNetworkCaptureProtocol`
    ...
    public static func locallyDisabled(for config: TraceItXConfig?) -> Bool {
    ...
    /// comparing `TraceItX.shared.currentStartEpoch` against the epoch it
    /// captured at construction: `TraceItX.start()`/`kill()` are synchronous,
    ...
    /// Production reset — called from `TraceItX.kill()` (final-review Finding
    /// 3: process-lifetime sampling) AND, synchronously, from the START of
    /// `TraceItX.start()` (round-5 review Finding F23: `start(A) -> start(B)`
