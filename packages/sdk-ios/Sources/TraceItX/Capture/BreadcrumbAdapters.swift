...
//
// Lifecycle + error ARE installed (once) from `TraceItX.start(config:)`,
...
enum ConsoleBreadcrumbAdapter {
    ...
    /// the protocol `Level`. "debug" -> `.debug` (mirrors sdk-react's
    ...
    /// here.
    static func mapLevel(_ raw: String) -> Level {
        switch raw {
...
/// across repeated `TraceItX.start()` calls (mirrors `LogCapture.install()`'s
/// idempotency).
...
func traceItXUncaughtExceptionHandler(_ exception: NSException) {
...
enum ErrorBreadcrumbAdapter {
    ...
    /// Install-once. Wired from `TraceItX.start(config:)` after the
    /// kill-gate is set.
    static func install() {
        lock.lock(); defer { lock.unlock() }
        guard !installed else { return }
        ...
        NSSetUncaughtExceptionHandler(traceItXUncaughtExceptionHandler)
    }
...
enum BreadcrumbAdapters {
    /// Wired from `TraceItX.start(config:)` immediately after the kill-gate
