...
// violated. Breadcrumb identity (mustKeep/kept tracking) is therefore done
// by array INDEX into the sorted `entries` array, not by value/reference
// identity as the TS `Set<Breadcrumb>` does — index identity is equivalent
...
public enum BreadcrumbTrim {
    ...
    // Set<String> (not Set<BreadcrumbKind>) sidesteps a Sendable/concurrency-
    // safety warning on a static Set of a non-Sendable generated enum; rawValue
    // comparison is equivalent since BreadcrumbKind's raw values are unique.
    private static let structuralKinds: Set<String> = [
        BreadcrumbKind.navigation.rawValue, BreadcrumbKind.tap.rawValue,
        BreadcrumbKind.lifecycle.rawValue, BreadcrumbKind.error.rawValue,
        BreadcrumbKind.custom.rawValue,
    ]

    /// Structural kinds are cheap and always-keep-first; console/network are trimmable.
    static func isStructural(_ kind: BreadcrumbKind) -> Bool {
        structuralKinds.contains(kind.rawValue)
    }

    /// A synthetic per-kind trim marker ("+N <kind> hidden")? Spec §1 discriminator.
    public static func isTrimMarker(_ crumb: Breadcrumb) -> Bool {
        guard let value = crumb.data?["droppedCount"]?.value else { return false }
        // JSONAny.decode(...) only ever produces Int64 or Double for a JSON
    ...
    public static func trim(
        ...
        var seenKinds = Set<BreadcrumbKind>()
    ...
    private static func tSeqLess(_ a: Breadcrumb, _ b: Breadcrumb) -> Bool {
    ...
    private static func truncate(_ c: Breadcrumb, consoleCap: Int) -> Breadcrumb {
        if c.kind == .console {
    ...
    static func crumbCost(_ crumb: Breadcrumb) -> Int {
        entryOverhead + crumb.message.utf16.count + dataCost(crumb.data)
    }

    private static func dataCost(_ data: [String: JSONAny]?) -> Int {
    ...
    // value, then decode it back through JSONAny's own Decodable conformance.
    // Neither encoder/decoder needs any special date strategy here — the only
    // values ever passed through are String and Int.

    private static func jsonAny<T: Encodable>(_ value: T) -> JSONAny? {
        guard let data = try? JSONEncoder().encode(value) else { return nil }
        return try? JSONDecoder().decode(JSONAny.self, from: data)
    }

    private static func droppedCountData(_ n: Int) -> [String: JSONAny] {
