...
public struct OutboxEntry: Codable, Sendable, Equatable {
    ...
    /// `TXCapturedUser.identitySubject` at the moment `enqueueToOutbox` ran.
    ...
    /// so a `= nil` default lets a report queued by an SDK built before this
...
/// AES-GCM-encrypted JSONL outbox at `Library/Caches/com.traceitx/outbox.jsonl`.
///
...
public final class JSONLOutbox: @unchecked Sendable {
    ...
    private static let magic = Data("TXOBX001".utf8)
    ...
    private static let storageQueue = DispatchQueue(label: "com.traceitx.outbox", qos: .utility)
    ...
    /// Convenience: production path = `Library/Caches/com.traceitx/outbox.jsonl`.
    ...
    private static let testOutboxDirectory: URL = {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("com.traceitx-test-\(UUID().uuidString)", isDirectory: true)
    ...
    private static func outboxURL() -> URL {
        ...
        // path is `Library/Caches/com.traceitx`, which on macOS is
        ...
        let dir = caches.appendingPathComponent("com.traceitx", isDirectory: true)
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
