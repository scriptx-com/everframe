...
// TraceItXReporterUI converts once via UIColor(argb:) (ResolvedPalette).
//
// Everything here is `public` on purpose: TraceItXReporterUI is a separate
// SPM target and must read the box and run the resolver at presentation
// time. (The companion box stays internal because only TraceItXKit reads it.)
import Combine
import Foundation
...
/// at both TraceItX session boundaries (start()'s synchronous reset, kill()'s
...
public final class BrandingServerConfigBox: @unchecked Sendable {
    ...
    // Setter is internal(set) (#137 review follow-up): only TraceItXKit
    // writes the box (ReplaySession's config apply, the session-boundary
    // clears); TraceItXReporterUI is a reader. Tests keep writing through
...
/// per-field. Referenced by TraceItXConfig.theme; the RN bridge (stacked
