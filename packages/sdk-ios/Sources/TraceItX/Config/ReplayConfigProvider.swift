...
// only when the project has a signing secret AND this SDK declared the
// `identity` capability in `X-TX-SDK-Features` — see `sdkFeaturesHeaderValue`.
...
/// `{ enabled, windowSec }`. Present only when this SDK declares `resources`
/// in `X-TX-SDK-Features` — same capability-negotiation doctrine as
...
/// permanently off (the negotiation-gap this SDK closes).
///
...
/// Reporter branding block (iOS spec 2026-08-26). Present only when this SDK
/// declared `branding` in `X-TX-SDK-Features`.
///
...
private struct ReplayConfigWire: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // malformed companionBadge block degrades to nil (the SDK keeps its
...
public actor ReplayConfigProvider {
    ...
    /// value is re-asked for each time — the web SDK baked its identifier into
    ...
    public func refresh(force: Bool = false) async -> Bool {
        ...
        do {
            ...
            // than version.
            req.setValue(Self.sdkFeaturesHeaderValue, forHTTPHeaderField: "X-TX-SDK-Features")
