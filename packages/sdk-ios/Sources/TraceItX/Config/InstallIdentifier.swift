...
// copied.
//
// NOT SCOPED ON THE SDK KEY, unlike web. Web scopes its storage key on the
...
// install identity on SDK-key rotation, which is already a known inflation
...
// in Tests/TraceItXTests/Fixtures/install-id.v1.json is what proves it.
//
...
// this SDK assumes: one app, one `UserDefaults.standard`. That assumption is
...
enum InstallIdentifier {
    ...
    static let domainSeparator = "traceitx-install-id-v1"
    ...
    static let defaultsKey = "com.traceitx.installSeed"

    static let defaultsDayKey = "com.traceitx.installIdDay"
    ...
    /// config URL, and that read is the SDK's remote kill switch. An uncounted
