...
public struct MultipartUploader {
    ...
    ///   this POST as `X-Everframe-Companion-Attribution` so ingest can credit the
    ...
    ///   `X-Everframe-Identity-Token`. nil sends the report anonymously.
    public func upload(
        parts: [Part],
        ...
        req.setValue(idempotencyKey, forHTTPHeaderField: "X-Everframe-Idempotency-Key")
        if let companionAttribution = companionAttribution {
            ...
            req.setValue(companionAttribution, forHTTPHeaderField: "X-Everframe-Companion-Attribution")
        }
        ...
        #if DEBUG
        print("[Everframe] POST \(ingestURL.absoluteString) bytes=\(body.count) boundary=\(boundary)")
        #endif
        ...
        do {
            ...
            guard let http = response as? HTTPURLResponse else {
                #if DEBUG
                print("[Everframe] non-HTTP response: \(response)")
                #endif
            ...
            print("[Everframe] response status=\(http.statusCode) body=\(bodyPreview)")
            #endif
            return (http.statusCode, http.allHeaderFields)
        } catch {
            #if DEBUG
            let nsErr = error as NSError
            print("[Everframe] transport error url=\(endpoint.absoluteString) domain=\(nsErr.domain) code=\(nsErr.code) desc=\(nsErr.localizedDescription)")
            #endif
            throw error
        }
    }
...
/// `EverframeTransportError` cases owned by 04-01.
