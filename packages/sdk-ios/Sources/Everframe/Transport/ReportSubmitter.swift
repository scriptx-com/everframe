...
/// `ReportSubmitter` construction + `drainOutbox()` into Everframe.swift's
...
/// Everframe.swift with parallel plans 04-03 and 04-04.
///
/// Sibling type dependencies (resolved at wave-merge):
///   - `EverframeConfig`           — 04-01 (`Config/EverframeConfig.swift`)
///   - `EverframeTransportError`   — 04-01 (`Errors/...`)
public final class ReportSubmitter: Sendable {
    ...
    /// Throws `EverframeTransportError` on terminal HTTP errors (4xx other than
    ...
    ///   was CAPTURED under (`EFCapturedUser.identitySubject` on the live
    ...
    ///   `X-Everframe-Identity-Token`, already resolved by the caller via
    ...
    public func submit(
        ...
        do {
            ...
            case .terminal:
                ...
                throw EverframeTransportError.serverError(status: status)
            }
    ...
    ///   resolves each entry's `X-Everframe-Identity-Token`, via
    ...
    ///   a token. Every caller — `Everframe.start()`'s launch drain, the crash
    ...
    ///   `Everframe.shared.currentStartEpoch` read right before
    ...
    ///   `Everframe.shared.currentStartEpoch`, supplied as a closure so this
    ...
    public func drainOutbox(
        ...
        for e in entries {
            ...
            // the wrong host" — a dev-override `EVERFRAME_DEV_INGEST_URL`
    ...
    private static let sharedIsolatedSession: URLSession = {
        ...
        cfg.protocolClasses = (cfg.protocolClasses ?? []).filter { cls in
            ...
            return !name.contains("EFNetworkCaptureProtocol")
        }
...
/// attaches `X-Everframe-Identity-Token` to the ingest POST, but `URLSession`
...
/// already provides on Android); `X-Everframe-Identity-Token` — and, separately,
/// `X-Everframe-Companion-Attribution` — were NOT stripped and arrived at server
...
///
/// Deliberately does NOT touch `X-Everframe-Companion-Attribution` — established
