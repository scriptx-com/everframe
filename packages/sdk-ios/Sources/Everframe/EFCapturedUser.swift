...
// still be uploaded under a DIFFERENT project's Everframe SDK key.
//
...
//     finished report.
//
// But the Everframe SDK CONFIGURATION — which carries the API key and therefore the
...
// captured in, using the mechanism this Everframe SDK already has for exactly this class
// of bug: `Everframe._startEpoch`, a monotonically increasing generation counter
...
/// boundary, TOGETHER with the Everframe SDK session it was snapshotted in AND the
/// verified-identity subject (recognition spec 2026-08-06) active at that same
/// instant.
///
/// Construct only via `Everframe.shared.captureUserSnapshot()` — the memberwise
...
public struct EFCapturedUser {
    ...
    internal init(user: EFUser?, startEpoch: Int, identitySubject: String?) {
        self.user = user
    ...
    /// project's Everframe SDK key) or a `kill()` has run since the capture.
    ///
    ...
    public func resolve() -> EFUser? {
        Everframe.shared.resolveCapturedUser(self)
    }
}
...
/// critical section.
///
/// `EFCapturedUser` fixed half of this: the user could no longer be paired
...
///
/// Wraps `EFCapturedUser` rather than flattening it, so `resolve()` and its
...
/// second (see `Everframe.start(config:)` — the user clear had to move ahead of
...
public struct EFCapturedSession {
    /// The user snapshot, with its own `startEpoch` and `resolve()`.
    public let user: EFCapturedUser
    ...
    /// `Everframe._killGeneration` as of the same critical section. Monotonic
    /// and bumped ONLY by `kill()`, so a later `start()` cannot lower it back.
    public let killGeneration: UInt64

    /// `Everframe._configGeneration` as of the same critical section — the
    ...
    /// `Everframe.killGenerationChanged` — which is `internal`, and Swift
    ...
    public var isRevoked: Bool {
        Everframe.killGenerationChanged(since: killGeneration)
    }
    ...
    public var isSuperseded: Bool {
        ...
        // the epoch has moved.
        Everframe.configGenerationChanged(since: configGeneration)
            || user.startEpoch != Everframe.shared.currentStartEpoch
    }
