...
//
// Public config types for Everframe. Field set sourced from RESEARCH Finding 14
...
/// matters — "is the Everframe SDK already running exactly this?" — against the whole config rather
...
public struct EverframeConfig: Sendable, Equatable {
    ...
    /// `EverframeConfig.companionDeviceId`.
    ...
    /// `EverframeConfig.companionBadgeEnabled`.
    ...
    /// `EverframeBridge.parseCompanionBadgePosition`. Mirrors Android's
    ...
    /// Read by EFReporterPresenter via `Everframe.shared.currentConfig`.
    /// Mirrors Android's `EverframeConfig.theme`.
    public var theme: ReporterThemeOptions?

    /// Install-identifier client control (MAI meter spec 2026-08-27). ON by
    /// default: the Everframe SDK sends a value derived from a locally-minted,
...
public enum EverframeTransportError: Error {
