...
//
// EverframeReporter identity recognition (spec 2026-08-06), iOS side. Swift twin of
...
// leak.
//
// The `exp` read below is a DECODE, not a verification: this Everframe SDK holds no
...
//
// DELIBERATELY NOT ON `Everframe.swift`: that file is `canImport(UIKit)`-gated,
// so anything living there is unreachable from `swift test` on macOS — which
// is the only place this Everframe SDK's tests run in CI.
import Foundation

/// The header ingest reads the token from.
public let IDENTITY_TOKEN_HEADER = "X-Everframe-Identity-Token"
...
/// What a host can hand to `Everframe.shared.setIdentityToken(...)`.
public enum IdentityTokenSource: Sendable {
    ...
    /// the Everframe SDK presenting a stale token.
    case token(String)
...
public final class IdentityTokenHolder: @unchecked Sendable {
    ...
    /// that exists to make the Everframe SDK stop doing anything) could cancel the
    ...
    /// Cancel EVERY outstanding provider call — called from `Everframe.kill()`
    ...
    /// whole process lifetime by the `Everframe` singleton (never
    ...
    /// Everframe SDK stop doing anything) already closed everywhere else.
    ...
    /// `@testable import`, matching this file's/Everframe.swift's other
    /// double-underscore "Everframe SDK-internal, test seam" declarations
    ...
    /// a provider. Capture boundaries (`Everframe.captureUserSnapshot()`) are
...
/// thread or actor. See `Everframe._identityEnabledFlag`'s doc comment for
...
public final class IdentityEnabledFlag: @unchecked Sendable {
    ...
    /// is `@MainActor` but `Everframe.start()`/`kill()` are synchronous and
    ...
    /// `refreshConfigNow()` (`Everframe.shared.currentStartEpoch ==
    ...
    /// re-enabling identity for a project the Everframe SDK has already moved on
...
/// down via `everframeGuardSuspend`'s catch-all. Empirically verified this does
