...
// (CompanionCaptureBridge). Same Everframe SDK, same protocol → must produce
...
//
// EverframePlatform differences (parameterised, not branched):
...
//     `includeMetadata`). EFUser-typed `title` + `description` come from the
//     form fields.
...
// NOTE: Moved from EverframeReporterUI → Everframe in Plan 06.2-12 so that
// `CompanionCaptureBridge` (which lives in Everframe and cannot reach into the
...
public enum ReporterSubmission {
    ...
    public struct Inputs {
        ...
        public struct Shot {
            ...
            public init(
                image: UIImage,
                annotationsJSON: [EverframeJSONAny] = [],
                redactionsJSON: [EverframeJSONAny] = [],
                annotated: Bool
            ) {
                self.image = image
        ...
        /**
         ...
         defect, same remedy, same boundary. `EFUser` is a value type, so
         ...
         SESSION it was taken in, not a bare `EFUser?`. The
         user was snapshotted at the Send tap but `cfg` — which carries the Everframe SDK
         ...
         installed. See `EFCapturedUser.swift`'s file header.

         FOLLOW-UPS ITEM 9 — this is a `EFCapturedSession`, not a
         `EFCapturedUser`, and that difference is the whole fix. The epoch
         ...
         */
        public let capturedSession: EFCapturedSession
        ...
        /// the `X-Everframe-Companion-Attribution` header on the ingest POST; it
    ...
    /// annotated flag, pick its `EverframeAttachmentKind` and derive its part name.
    ...
    nonisolated static func buildAttachmentPlan(
        annotatedFlags: [Bool]
    ) -> [(partName: String, kind: EverframeAttachmentKind)] {
        annotatedFlags.enumerated().map { index, annotated in
            let kind: EverframeAttachmentKind = annotated ? .annotatedScreenshot : .screenshot
    ...
    /// `EFNetworkCaptureProtocol`, so a `URLProtocol` stub is not a dependable
    /// lever here either. The remaining alternative — pointing
    /// `EVERFRAME_DEV_INGEST_URL` at an unroutable host — needs CI env plumbing
    ...
    nonisolated(unsafe) internal static var __submitterFactoryForTesting: ((EverframeConfig) -> ReportSubmitter)?
    ...
    ///
    /// Throws `ReporterSubmissionError.notStarted` if `Everframe.shared.start(...)`
    /// was never called (no `currentConfig`). Throws transport errors
    /// (`EverframeTransportError`) on terminal failures; transient errors land
    ...
    public static func submit(_ inputs: Inputs) async throws -> ReportResult {
        ...
        // Send tap, NOT from a live `Everframe.shared.currentConfig` read here.
        //
        // `_config` carries the Everframe SDK KEY, i.e. the project every byte of this
        ...
        let frozenBodies: [EverframeNetworkBody]?
        ...
        // from `Everframe.shared.currentUser`. This function is `async` and
        ...
        // snapshot. `cfg` above carries the Everframe SDK key, i.e. the PROJECT this
        ...
        // (EFReporterPresenter.openAndAwait), shown in the modal so the user
        ...
        // envelope EverframeAttachment PER SHOT. `buildAttachmentPlan` derives each
        // shot's EverframeAttachmentKind (annotated iff that shot carries ≥1
        ...
        var envelopeAttachments: [EverframeAttachment] = []
        var multipartAttachments: [ReportSubmitter.Attachment] = []
        var allAnnotationsJSON: [EverframeJSONAny] = []
        var allRedactionsJSON: [EverframeJSONAny] = []
        ...
        for (index, shot) in inputs.shots.enumerated() {
            ...
            envelopeAttachments.append(EverframeAttachment(
        ...
        // reporter-open in EFReporterPresenter, or at report.request in
        ...
        func encodeEnvelope() throws -> Data {
            ...
            // Report EverframeResource Window (spec 2026-09-05, fix round 1 CRITICAL 1)
        ...
        // `X-Everframe-Identity-Token` value HERE, against the subject captured at
        // the Send tap (`inputs.capturedSession.user.identitySubject`, NOT a
        // live `Everframe.shared._identityHolder.cachedSubject(...)` read —
        ...
        // identity subject rides the SAME `EFCapturedSession.user` field the
        ...
        if inputs.capturedSession.user.startEpoch == Everframe.shared.currentStartEpoch {
            ...
            capturedEpochStillCurrent = inputs.capturedSession.user.startEpoch == Everframe.shared.currentStartEpoch
            ...
            // decision.
            let stillEnabled = isIdentityEnabled(await Everframe.shared.currentReplayConfig())
    ...
    /// One assembled session-replay attachment: the envelope `EverframeAttachment` ref +
    ...
    internal static func buildReplayAttachment(
        ...
        guard let (bytes, sha) = packed else { return nil }
            return ReplayPart(envelope: EverframeAttachment(
                ...
                durationMS: artifact.durationMs, format: .everframeVideoV1,
...
public enum ReporterSubmissionError: Error {
    /// `Everframe.shared.start(_:)` was never called. The caller (reporter VC)
    ...
    /// future metric: one means the Everframe SDK was never configured, the other means
