...
/// Live session-replay coordinator. One per Everframe SDK session (owned by `Everframe`).
@MainActor
public final class ReplaySession: NSObject {
    ...
    /// Report EverframeResource Window (spec 2026-09-05) — CPU/memory sampler feeding
    ...
    /// this is the ONE source `Everframe.currentReplayConfig()` reads to
    ...
    /// exactly once by `teardown()`. Mirrors `Everframe._startEpoch`
    /// (round-2 review Finding F9, commit 826e5f76) one level down: that
    /// epoch guards `Everframe.start()`'s async tail against installing a
    ...
    /// Round-6 review Finding F27 — the GLOBAL `Everframe._startEpoch` value
    /// current at the moment this session was constructed (see
    /// `Everframe.currentStartEpoch`'s doc comment for the full rationale).
    ...
    /// - Parameter startEpoch: the GLOBAL `Everframe._startEpoch` value this
    ...
    init(
        ...
        startEpoch: Int = Everframe.shared.currentStartEpoch
    ) {
        ...
        super.init()
        // A host can restart the Everframe SDK while a report is still being composed.
        // tvOS has no modal presenter: the companion owns that same freeze.
        if Everframe.shared.report.isPresenting || CompanionCaptureBridge.hasActiveReportCapture {
            video.freeze()
        }
    ...
    deinit {
        ...
        // `Everframe.kill()`'s MainActor hop — see its doc comment) is the
    ...
    /// `Everframe.kill()`'s MainActor hop immediately before nil'ing out its
    ...
    func teardown() {
        ...
        refreshLoopTask = nil
        // Report EverframeResource Window (spec 2026-09-05) — same "everything stops"
    ...
    @objc nonisolated private func thermalStateChanged() {
        DispatchQueue.main.async { [weak self] in
            guard let self, !self.isTornDown,
                  self.startEpochAtCreation == Everframe.shared.currentStartEpoch else { return }
    ...
    /// buffering — spin up the adaptive tick. Safe to call once at Everframe SDK start().
    ...
    func refreshConfigNow() async {
        ...
        //
        // `Everframe.shared.currentStartEpoch` closes that gap: it is bumped
        ...
        guard Everframe.shared.currentStartEpoch == startEpochAtCreation else { return }
        ...
        // the iOS Everframe SDK where a freshly-fetched `ReplayConfig` is read, so
        ...
        // Report EverframeResource Window (spec 2026-09-05) — negotiation-gap fix:
        ...
        // no Everframe SDK restart. Same last-good semantics as breadcrumbs above: a
        ...
        // Everframe SDK restart — this `applyConfig` call is reached by BOTH the
        ...
        // Everframe.shared.currentConfig snapshot) + one-shot sampling.
        ...
        NetworkBodyCaptureGate.shared.applyConfig(
            ...
            locallyDisabled: NetworkBodyCaptureGate.locallyDisabled(for: Everframe.shared.currentConfig)
                || NetworkBodyCaptureGate.breadcrumbsExcludeNetwork(latest.breadcrumbs)
        ) { [startEpochAtCreation] in
            Everframe.shared.currentStartEpoch == startEpochAtCreation
        }
        ...
        // currentReplayConfig().
        //
        // Codex round-2 fix — `kill()` now clears this same box (Everframe
        ...
        // hop (see `Everframe._identityEnabledFlag`'s doc comment).
        //
        ...
        // (`Everframe.__warmIdentityToken()`) correctly no-ops per round 6's
        ...
        // drift. Reuses the EXISTING `Everframe.shared.__warmIdentityToken()`
        ...
        let wasIdentityEnabled = Everframe.shared._identityEnabledFlag.get()
        let nowIdentityEnabled = isIdentityEnabled(latest)
        Everframe.shared._identityEnabledFlag.set(nowIdentityEnabled) { [startEpochAtCreation] in
            Everframe.shared.currentStartEpoch == startEpochAtCreation
        }
    ...
    public func freezeForReporter() {
        guard !isTornDown, Everframe.shared.currentStartEpoch == startEpochAtCreation else { return }
        video.freeze()
    }

    internal func completeVideoForSubmit() async -> NativeVideoClaim? {
        guard !isTornDown, Everframe.shared.currentStartEpoch == startEpochAtCreation else { return nil }
        ...
        return NativeVideoClaim(artifact: claim.artifact, omissionReason: claim.omissionReason, validate: { [weak self] in
            ...
            return !self.isTornDown && Everframe.shared.currentStartEpoch == self.startEpochAtCreation && claim.isValid
        })
    }
