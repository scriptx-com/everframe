...
public final class NetworkBodyRingBuffer: @unchecked Sendable {
    ...
    /// `append` consults `Everframe.shared.captureGate` — mirrors
    ...
    private var entries: [EverframeNetworkBody] = []
    private var frozen: [EverframeNetworkBody]?
    ...
    private static func cost(_ entry: EverframeNetworkBody) -> Int {
    ...
    public func append(_ entry: EverframeNetworkBody, `guard`: (() -> Bool)? = nil) {
    ...
    public func takeFrozen() -> [EverframeNetworkBody]? {
        lock.lock(); defer { lock.unlock() }
        let out = frozen
        frozen = nil
        return out
    ...
    /// lifecycle.
    public func snapshot() -> [EverframeNetworkBody] {
        lock.lock()
        defer { lock.unlock() }
        return entries
    }
