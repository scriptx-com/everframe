...
// "EverframeHandled" before being re-issued on a private inner session whose
...
// body-capture unit (the body path of `EFNetworkCaptureProtocol` on iOS;
...
public final class EFNetworkCaptureProtocol: URLProtocol, @unchecked Sendable {

    private static let HANDLED_KEY = "EverframeHandled"
    ...
    public override func startLoading() {
        ...
        let task = session.dataTask(with: req) { [weak self] data, response, error in
            ...
            // `Everframe.shared.captureGate` here, not just
            ...
            if Everframe.shared.captureGate, gateSnapshot.active, response != nil {
...
public extension Everframe {
    /// Returns a URLSessionConfiguration that captures network through Everframe.
    ...
    func networkCaptureConfiguration(base: URLSessionConfiguration = .default) -> URLSessionConfiguration {
        ...
        protos.insert(EFNetworkCaptureProtocol.self, at: 0)
