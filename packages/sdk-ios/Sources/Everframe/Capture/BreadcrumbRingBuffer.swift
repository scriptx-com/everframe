...
public final class BreadcrumbRingBuffer: @unchecked Sendable {
    ...
    /// Protocol EverframeBreadcrumb.message ceiling — enforced at add-time (mirrors
    ...
    private static func allKinds() -> Set<EverframeBreadcrumbKind> {
        [.console, .custom, .error, .lifecycle, .navigation, .network, .tap]
    }

    /// See NetworkRingBuffer.honorsKillGate — same rationale: `add` consults
    /// Everframe.shared.captureGate by default; the test-only init below
    ...
    private var entries: [EverframeBreadcrumb] = []
    private var frozen: [EverframeBreadcrumb]?
    ...
    private var configKinds: Set<EverframeBreadcrumbKind> = BreadcrumbRingBuffer.allKinds()
    ...
    public func add(
        kind: EverframeBreadcrumbKind, message: String, level: EverframeLevel? = nil, data: [String: EverframeJSONAny]? = nil
    ) {
        ...
        // THEN calls `clear()` — see `Everframe.kill()`), and only then resume
        ...
        // between the two.
        if honorsKillGate, !Everframe.shared.captureGate { return }
        preLockHook?()
    ...
    public func takeFrozen() -> [EverframeBreadcrumb]? {
        lock.lock(); defer { lock.unlock() }
        let out = frozen
        frozen = nil
        return out
    ...
    public func snapshot() -> [EverframeBreadcrumb] {
        lock.lock()
        defer { lock.unlock() }
        return entries
    }
    ...
    public func applyConfig(_ cfg: BreadcrumbsConfigWire?) {
        ...
        configKinds = Set(cfg.kinds.compactMap { EverframeBreadcrumbKind(rawValue: $0) })
    ...
    public func isKindEnabled(_ kind: EverframeBreadcrumbKind) -> Bool {
        lock.lock(); defer { lock.unlock() }
    ...
    // cannot hold a lone surrogate). Accepted: each Everframe SDK trims only its own
    ...
    private static func redactDataForStorage(_ data: [String: EverframeJSONAny]) -> [String: EverframeJSONAny] {
    ...
    // serialize, then decode back through EverframeJSONAny's own Decodable
    ...
    // EverframeJSONNull/[Any]/[String: Any]), not a single Encodable type. Fails soft
    // (never throws) — an unencodable/malformed graph yields [:] rather than
    // crashing the host.

    private static func mintJSONAny(_ dict: [String: Any]) -> [String: EverframeJSONAny] {
    ...
    private static func plainJSONValue(_ value: Any) -> Any {
        if value is EverframeJSONNull { return NSNull() }
    ...
    /// `Everframe.addBreadcrumb(data:)` coercion: host-supplied `[String: Any]`
    ...
    static func coerceHostData(_ data: [String: Any]) -> [String: EverframeJSONAny] {
