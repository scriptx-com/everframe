...
public final class CompanionPreviewSession: CompanionPreviewSessionApi {
    ...
    private let send: (EverframeRelayMessage) -> Void
    ...
    /// matching the web Everframe SDK's `capture-profile.ts` `livePreview` and
    ...
    public init(send: @escaping (EverframeRelayMessage) -> Void,
    ...
    public func start(correlationId: String) {
        guard !isTornDown else {
            print("[everframe-companion] preview.start after teardown — refused")
            return
        }
        ...
        if !livePreviewEnabled {
            ...
            send(.previewStop(EverframePreviewStop(correlationId: correlationId,
        ...
        task = Task { @MainActor [weak self] in
            ...
            while self.now().timeIntervalSince(startedAt) * 1000 < self.maxDurationMs {
                ...
                guard Everframe.captureGate, CompanionAuthEpoch.current == authAtStart else {
                ...
                // await-shot slot.
                self.send(.previewFrame(EverframePreviewFrame(correlationId: corrId,
                                                     height: frame.height,
                                                     mime: frame.mime,
                                                     seq: self.seq,
                                                     type: "preview.frame",
    ...
    public func stop(reason: PreviewStopReason) {
        ...
        if let corrId {
            send(.previewStop(EverframePreviewStop(correlationId: corrId,
    ...
    public func handleShotRequest(correlationId: String,
        ...
        guard !isTornDown else {
            send(.shotFailed(EverframeShotFailed(correlationId: correlationId,
        ...
        guard Everframe.captureGate else {
            send(.shotFailed(EverframeShotFailed(correlationId: correlationId,
                                        reason: "capture_unavailable",
                                        shotId: shotId,
                                        type: "shot.failed")))
        ...
        send(.shotAssembled(EverframeShotAssembled(correlationId: correlationId,
                                          height: out.height,
                                          mime: out.mime,
                                          shotId: shotId,
