...
//
// Loads packages/protocol/data/*.json (mirrored into Sources/Everframe/Resources/
// at scaffold time and CI-gated for drift) into the Everframe SDK at first-access. Eager
...
// truth. Each native Everframe SDK reimplements logic idiomatically but consumes the
// same JSON, so iOS / Android / web cannot drift on what counts as "sensitive".
import Foundation

// EverframeResource bundle resolver. The published artifact is the xcframework
...
#if SWIFT_PACKAGE
private let everframeResourceBundle: Bundle = .module
#else
private final class EverframeResourceMarker {}
private let everframeResourceBundle: Bundle = Bundle(for: EverframeResourceMarker.self)
#endif
...
public enum SharedData {
    public static let redactionPatterns: [RedactionPattern] = {
        guard let url = everframeResourceBundle.url(forResource: "redaction-patterns", withExtension: "json"),
    ...
    public static let sensitiveHeadersToRedact: Set<String> = {
        guard let url = everframeResourceBundle.url(forResource: "sensitive-headers", withExtension: "json"),
    ...
    public static let networkPathPatterns: NetworkPathPatterns? = {
        guard let url = everframeResourceBundle.url(forResource: "network-path-patterns", withExtension: "json"),
