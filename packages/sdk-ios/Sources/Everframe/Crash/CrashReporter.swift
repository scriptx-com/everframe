...
//
// EverframeCrash-envelope assembly for JS-error reports arriving over the RN bridge
...
//     ("everframe-react-native") distinctly from a native-iOS host
//     ("everframe-ios", the default) — mirrors Android's Task 12 handling of
//     the same wire shape.
import CryptoKit
import Foundation
...
public enum CrashReporter {
    ...
    private static let handledAdmission = HandledErrorAdmission {
        Everframe.shared.currentStartEpoch
    }
    ...
    /// Mirrors `Everframe.__startTailDelayHookForTesting`'s convention.
    ...
    private struct Facts: Decodable {
        ...
        init(from decoder: Decoder) throws {
            ...
            jsBundle = try? values.decode(EverframeJSBundle.self, forKey: .jsBundle)
    ...
    private static func validJsBundle(_ bundle: EverframeJSBundle?) -> EverframeJSBundle? {
        guard let bundle,
    ...
    /// the RN bridge passes `"everframe-react-native"`.
    ///
    /// `config` defaults to `nil` meaning "take it from the crash-entry
    /// session snapshot below", NOT to `Everframe.shared.currentConfig`
    ...
    private static func capture(
        ...
        //
        // Blocking: `captureSessionSnapshot()` takes `Everframe.stateLock` for
        ...
        // SEPARATE `Everframe.shared.captureConfigSnapshot()` call, its own
        // `stateLock` acquisition after the one that produced `captured`
        // above. `EFCapturedSession.config` (via `captureSessionSnapshot()`)
    ...
    private static func capture(
        ...
        // EverframeFrame-key derivation for fingerprinting: raw text with digit runs
        ...
        // `EverframeFrame(raw = ...)` branch too). The 5-frame cap lives INSIDE
        ...
        // `Everframe.shared.currentUser` read here. `resolve()` returns the
        ...
        do {
            ...
            let (bytes, idempotencyKey) = try EnvelopeBuilder(redactor: redactor, vitalsStartEpoch: capturedUser.startEpoch).buildEncoded(
                ...
                // Report EverframeResource Window (spec 2026-09-05) — same style as
            ...
            // lock-ordering hazard; capture deliberately releases Everframe SDK state
            ...
            // Everframe.shared.currentStartEpoch` comparison
            // `capturedUser.resolve()` already performs internally for the
            // user above. A raw `EFCapturedUser` field (unlike `.user`) has
            // no built-in gate of its own, so it is made explicit here — an
            // epoch mismatch means the Everframe SDK has already decided the WHOLE
            // captured snapshot is untrustworthy, not just the self-declared
            // user half of it. Without this, a LATER drain could attach a
            // header on the strength of a subject the Everframe SDK had already
            // concluded it should not rely on.
            let capturedEpochStillCurrent = capturedUser.startEpoch == Everframe.shared.currentStartEpoch
            ...
            if !fatal {
                ...
                // A's Everframe SDK key.
                //
                // Independent review, round 9, P1 — this used to be a
                // SEPARATE `Everframe.shared.currentStartEpoch` read at this
                ...
                // the old separate `EFCapturedConfig`/`captureConfigSnapshot()`
    ...
    /// `EverframeCrash.occurredAt` is a generated `Date` (ISO-8601 decode/encode). The
