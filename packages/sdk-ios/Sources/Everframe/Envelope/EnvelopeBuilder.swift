...
//
// Concrete envelope composition for the iOS Everframe SDK. Wraps the quicktype-generated
// `EverframeReportEnvelope` initializer with redaction + size-cap enforcement + SHA-256
...
public struct EnvelopeBuilder {
    ...
    /// (`EFCapturedSession.user.startEpoch`), not a live read. Vitals are stamped only while
    ...
    public func buildEncoded(
        ...
        extra: [String: String] = [:],
        /// Host-supplied opaque metadata string (`EverframePayload.extra`). Distinct
        ...
        /// pre-breadcrumbs builder — `EverframeCaptures.breadcrumbs` still comes back
        ...
        /// Report EverframeResource Window (spec 2026-09-05) — CPU/memory samples for
        ...
        let trimmedBreadcrumbs: [EverframeBreadcrumb]? = (breadcrumbs?.isEmpty == false)
        ...
        let bodiesForPayload: [EverframeNetworkBody]? = (filteredNetworkBodies?.isEmpty == false) ? filteredNetworkBodies : nil
        ...
        let vitalsGenerated: [EverframeVital]? = stamp.map { Array($0.entries.suffix(VitalsLimits.maxEnvelopeVitalsEntries)).toGeneratedVitals() }.flatMap { $0.isEmpty ? nil : $0 }
        ...
        guard bytes.count <= Self.SIZE_CAP_BYTES else {
            throw EverframeTransportError.payloadTooLarge(bytes: bytes.count, limit: Self.SIZE_CAP_BYTES)
        }
    ...
    ///
    /// Field-name source: `swift/Sources/EverframeProtocol/Generated.swift`
    ...
    /// * `Generated.EverframeSDK.name` enum currently lacks `everframe-ios`. Plan 04-02
    ///   extends the protocol package's `name` enum and updates this site to
    ///   use `EverframeName.everframeIOS`. For now the placeholder is `.everframeReact`
    ...
    private static func composeEnvelope(
        ...
        attachments: [EverframeAttachment],
        annotations: [EverframeJSONAny]?,
        redactions: [EverframeJSONAny]?,
        extra: [String: String],
        userExtra: String?,
        trimmedBreadcrumbs: [EverframeBreadcrumb]?,
        source: EverframeReportEnvelopeSource?,
        crash: EverframeCrash?,
        networkBodies: [EverframeNetworkBody]?,
        resources: [ResourceSample]?,
        vitalsGenerated: [EverframeVital]?,
        sessionId: String?
    ) throws -> EverframeReportEnvelope {
        let now = Date()

        // Encode logs/network as `[EverframeJSONAny]?` via JSON round-trip — Generated.swift
        ...
        let logsJSON: [EverframeJSONAny]? = try redactedLogs.isEmpty ? nil : Self.toJSONAnyArray(
        ...
        let networkJSON: [EverframeJSONAny]? = try redactedNetwork.isEmpty ? nil : Self.toJSONAnyArray(
        ...
        // WHOLE report. `EverframePayload.resources` is now a generated, typed
        // `[EverframeResource]?` (envelope.v1.schema.json regenerated to include the
        // resources block) — `ResourceSample` still lives in the
        // higher-level Capture module (EverframeKit) and can't be referenced
        // directly from `EverframePayload` (EverframeProtocol), so this still goes
        // through a JSON round trip, just decoding into the generated
        // `EverframeResource` type instead of an opaque `EverframeJSONAny`.
        let resourcesJSON: [EverframeResource]? = try (resources?.isEmpty ?? true) ? nil : {
            let data = try JSONEncoder().encode(resources!)
            return try JSONDecoder().decode([EverframeResource].self, from: data)
        }()

        // Map sdkName argument to the protocol enum. As of 04-02 the protocol
        // enum has been extended to include `everframe-ios` (and `everframe-android`
        // for Phase-5 readiness) so the placeholder branch from 04-01 is gone:
        // every supported Everframe SDK name has a direct enum case. Unknown names fall
        // back to `.everframeIos` and the literal value is preserved in
        // `reporter.description` for traceability.
        let nameEnum: EverframeName = EverframeName(rawValue: sdkName) ?? .everframeIos
        ...
        let user: EverframeUser? = {
            guard let id = extra["user.id"] ?? extra["user.email"] ?? extra["user.displayName"] else { return nil }
            _ = id
            return EverframeUser(displayName: extra["user.displayName"], email: extra["user.email"], id: extra["user.id"])
        }()

        let reporter = EverframeReporter(description: descriptionText, title: title, user: user)
        ...
        let captures = EverframeCaptures(
        ...
        let captureControl = EverframeCaptureControl(
            degradedReason: extra["captureControl.degradedReason"],
            excluded: [],
            included: includedCaptures
        )

        let payload = EverframePayload(
            annotations: annotations,
            breadcrumbs: trimmedBreadcrumbs,
            // EverframeCrash/error reporting (spec 2026-07-18): nil for manual reporter
        ...
        // EverframeApp + device come from extras at this scaffold layer. Plan 04-02 wires
        ...
        let device = EverframeDevice(
            ...
            screenSize: EverframeScreenSize(
        ...
        let context = EverframeContext(app: app, device: device, route: extra["route"])

        #if os(tvOS)
        let defaultPlatform: EverframePlatform = .tvos
        let defaultFormFactor: EverframeFormFactor = .tv
        #else
        let defaultPlatform: EverframePlatform = .ios
        let defaultFormFactor: EverframeFormFactor = .phone
        #endif
        ...
        return EverframeReportEnvelope(
            ...
            context: context,
            payload: payload,
            protocolVersion: EverframeProtocolVersion.the10,
            reporter: reporter,
            reportID: reportId.uuidString,
            sdk: EverframeSDK(
                formFactor: EverframeFormFactor(rawValue: extra["sdk.formFactor"] ?? "") ?? defaultFormFactor,
                name: nameEnum,
                platform: EverframePlatform(rawValue: extra["sdk.platform"] ?? "") ?? defaultPlatform,
                version: sdkVersion
            ),
    ...
    /// `EverframeJSONAny`'s decode order (integral values decode as `Int64`, not
    ...
    /// `.value as? Int64`). `EverframeNetworkBody.ref` is generated as `Double` (minted
    ...
    private static func reqId(from crumbData: [String: EverframeJSONAny]?) -> Double? {
    ...
    /// `[EverframeJSONAny]` decodable by quicktype's generated wrapper. Used for
    /// payload.logs/network where the schema declares `array<unknown>`.
    private static func toJSONAnyArray(_ rows: [[String: Any]]) throws -> [EverframeJSONAny] {
