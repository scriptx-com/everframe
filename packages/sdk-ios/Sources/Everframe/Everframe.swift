...
//
// Public singleton entry point for the Everframe iOS Everframe SDK.
//
...
//             the SINGLE Wave-4 writer to Everframe.swift; plans 04-03/04/05 do
...
public final class Everframe: @unchecked Sendable {
    public static let shared = Everframe()

    /// Semver of the iOS Everframe SDK, embedded into every envelope's `reporter.version`.
    /// Sourced from `Everframe.podspec`'s `spec.version` via the auto-generated
    ...
    private var _config: EverframeConfig?
    ...
    /// `EFCapturedSession`'s header documents as harmless for the crash path
    ...
    /// on `EFCapturedSession.isSuperseded` on both platforms.
    private var _configGeneration: UInt64 = 0
    private var _user: EFUser?
    ...
    ///
    /// `internal`, not `private` — `@testable import EverframeKit` needs to
    ...
    /// but the closure it was handed (`{ Everframe.shared.currentStartEpoch
    ...
    public var currentConfig: EverframeConfig? {
        stateLock.lock(); defer { stateLock.unlock() }
        return _config
    }

    /// Read-only snapshot of the current user.
    public var currentUser: EFUser? {
        stateLock.lock(); defer { stateLock.unlock() }
        return _user
    }
    ...
    ///
    /// External review, finding 1 (Serious) — see `EFCapturedUser.swift`'s
    /// file header for the full defect. In short: the submit paths snapshot
    /// the user at the Send tap, but read the CONFIG (which carries the Everframe SDK
    ...
    public func captureUserSnapshot() -> EFCapturedUser {
        stateLock.lock(); defer { stateLock.unlock() }
    ...
    /// critical section — see `EFCapturedSession`. The crash path's entry
    ...
    /// would reopen the exact non-atomicity `EFCapturedUser` exists to
    ...
    public func captureSessionSnapshot() -> EFCapturedSession {
        ...
        return EFCapturedSession(
            user: EFCapturedUser(user: _user, startEpoch: _startEpoch, identitySubject: identitySubject),
            config: _config,
    ...
    /// captured? Backs `EFCapturedSession.isSuperseded` — see
    ...
    ///
    /// Backs `EFCapturedUser.resolve()`; kept here because `_startEpoch` and
    /// its lock live here. `internal` — hosts resolve through the snapshot.
    internal func resolveCapturedUser(_ captured: EFCapturedUser) -> EFUser? {
        stateLock.lock(); defer { stateLock.unlock() }
    ...
    /// `Everframe.__replayConfigOverrideForTesting` one-for-one, including
    ...
    /// mirrors Android `Everframe.__replayConfigOverrideForTesting`, added
    ...
    /// `Everframe.__setConfigForTesting` (added alongside this for the same
    /// finding — see `packages/sdk-android/.../Everframe.kt`).
    ///
    ...
    /// a real `start()` / `kill()`.
    internal static func __setConfigForTesting(_ config: EverframeConfig?) {
        Everframe.shared.stateLock.lock()
        Everframe.shared._config = config
        Everframe.shared._configGeneration &+= 1
        Everframe.shared.stateLock.unlock()
    }
    ...
    internal static var __hasReplaySessionForTesting: Bool {
        Everframe.shared._replaySession != nil
    }
    #endif
    ...
    /// normally.
    nonisolated(unsafe) internal static var __replaySessionFactoryForTesting: ((EverframeConfig) -> ReplaySession)?
    ...
    public func start(config: EverframeConfig) throws {
        ...
        // what overturns it. `_config` carries the Everframe SDK KEY, so
        ...
        // isolated call) could read `Everframe.shared.currentStartEpoch`
        ...
        // id/email/display name under B's Everframe SDK key: exactly the cross-project
        ...
        let vitalsStillNewest: () -> Bool = { Everframe.shared.currentStartEpoch == epoch }
        ...
        // made (`Everframe.swift`'s kill(), just below its own gate flip) and
        ...
        // configuration, so no observer can ever see B's Everframe SDK key paired with
        // A's user. `kill()` already did this (see its own comment below);
        // `start()` did not, and it is the more dangerous half: `_config`
        // carries the Everframe SDK KEY, i.e. the project every subsequent report is
        ...
        // than a config-equality predicate would be (EverframeConfig carries
        ...
        // first.
        Task { @MainActor in
            Everframe.shared._replaySession?.teardown()
        }
        #endif
        ...
        // executor. `config` is now Sendable (EverframeConfig: Sendable), so
        ...
        let startTask = Task { [config, epoch] in
            ...
            // Finding F31) pins byte-for-byte: `Everframe
            ...
            // in `EFReporterPresenter.swift`); a kill()/start() race
            ...
            await submitter.drainOutbox(
                identityHolder: Everframe.shared._identityHolder,
                currentReplayConfig: { await Everframe.shared.currentReplayConfig() },
                epochAtInitiation: epoch,
                currentEpoch: { Everframe.shared.currentStartEpoch }
            )
            ...
            let vitalsDims = VitalsDims.current(sdkVersion: Everframe.SDK_VERSION)
            ...
            VitalsRuntime.shared.install(vitalsController, startEpoch: epoch) { Everframe.shared.currentStartEpoch == epoch }
            ...
            await MainActor.run {
                ...
                Everframe.shared.stateLock.lock()
                let stillCurrent = Everframe.shared._startEpoch == epoch
                Everframe.shared.stateLock.unlock()
                ...
                Everframe.shared._replaySession = session
    ...
    /// `_config`, which carries the Everframe SDK KEY — the project every subsequent
    ...
    /// (`Everframe.kt`, gated the same way) and web's (`client.ts`, a no-op
    /// before `init`).
    ///
    ...
    /// the caller is telling the truth. `setIdentityToken` hands the Everframe SDK a
    ...
    /// `EverframeReporterUI`'s `EFReporterPresenter` calls this from a
    /// SEPARATE module and `_identityHolder` is not part of `EverframeKit`'s
    ...
    ///   2. `EFReporterPresenter.openAndAwait()` (`EverframeReporterUI`), at
    ...
    ///      silently.
    ///
    /// EverframeReporter-open is real time BEFORE the Send tap and off the capture's
    ...
    /// with no "opening" moment to warm ahead of, and this Everframe SDK deliberately
    ...
    /// down ASYNCHRONOUSLY (see the `Task { @MainActor in Everframe.shared
    ...
    public func __warmIdentityToken() {
        ...
        Task.detached(priority: .utility) {
            ...
            // immediately.
            await Everframe.__warmIdentityTokenPreReadHookForTesting?()
    ...
    /// @everframe/web already have). `nil` for every pure-native host, and
    ...
    /// (`EverframeBridge.configure`) installs a non-nil implementation once;
    ...
    /// (`EFReporterPresenter.openAndAwait`, `CompanionCaptureBridge
    ...
    /// above: a plain global, written once by `EverframeBridge.configure`
    ...
    private actor ExtraResolveSerializer {
        ...
        /// callers OUTSIDE the actor (e.g. `Everframe.__consumePendingAttachments`)
    ...
    public func addBreadcrumb(
        ...
        let resolvedKind = kind.flatMap(EverframeBreadcrumbKind.init(rawValue:)) ?? .custom
        let resolvedLevel = level.flatMap(EverframeLevel.init(rawValue:))
    ...
    /// can call it from `.onAppear` (a `.everframeScreen()` modifier is a planned
    ...
    /// Android twin (`Everframe.recordScreen`). `from`/`to` win over `data`
    /// keys on collision.
    ...
    /// product (`EverframeReporterUI`) and can't see internal symbols. Treat
    /// as private — the `__` prefix marks it as an Everframe SDK seam; production
    ...
    //
    // The reporter presenter (EverframeReporterUI module) and the submit path
    // (this module) drive the replay lifecycle through these `__`-prefixed
    // seams on the singleton — same indirection as `__resolver` /
    // `__setPresenting`, avoiding a Everframe → ReporterUI dependency cycle.
    ...
    /// EverframeReporter cancel/close: discard the frozen window, resume a fresh buffer.
    ...
    public func kill() {
        ...
        // (Everframe.kt), and with web (sdk-core `client.ts` kill(), which
        ...
        // Everframe.kt kill() -> `_identityHolder.cancelOutstandingWork()`.
        ...
        // identical Android fix (Everframe.kt kill()).
        ...
        let stillThisKill: () -> Bool = { Everframe.shared.currentStartEpoch == killEpoch }
        ...
        // Host-owned triggers manage their own lifecycle. The Everframe SDK-owned mobile
    ...
    private static func clearCapturedEvidenceBuffers() {
        ...
        // Report EverframeResource Window (spec 2026-09-05, fix round 1 IMPORTANT 2)
    ...
    /// and `pairUrl` to render a QR / connection indicator. Everframe SDK ships zero QR
...
public final class ReportAPI: ObservableObject, @unchecked Sendable {
    ...
    /// Wired in plan 04-06 to EFReporterPresenter.openAndAwait() via the
    /// runtime-injected resolver below — this keeps Everframe.swift independent
    /// of the EverframeReporterUI module (which depends on Everframe).
    ...
    /// Set by EverframeReporterUI at module-load (or by tests) to provide the
    /// async/await reporter implementation. Avoids a hard dependency from
    /// Everframe → EverframeReporterUI (which would create a dependency cycle).
    ...
    //      use `@ObservedObject private var report = Everframe.shared.report`
    //      and read `report.isPresenting`, or subscribe to `report.$isPresenting`.
    //   2. `Notification.Name.everframeReporterPresentingChange` — non-Combine
    //      consumers subscribe via NotificationCenter; userInfo carries
    //      `["isPresenting": Bool]` for the new value.
    //
    // Single-writer invariant: the only writer is `EFReporterPresenter` (in
    // the EverframeReporterUI module), via the `__setPresenting` slot below.
    // The slot indirection mirrors `__resolver` and prevents a Everframe →
    // EverframeReporterUI dependency cycle (per RESEARCH Pattern 1 / Pitfall 1).

    /// Read-only public surface; flipped by EFReporterPresenter via
    ...
    @Published public private(set) var isPresenting: Bool = false {
        didSet {
            ...
            NotificationCenter.default.post(
                name: .everframeReporterPresentingChange,
                object: self,
    ...
    /// Lambda slot populated by EFReporterPresenter.installResolver() (mirrors
    /// the existing `__resolver` indirection). Avoids the Everframe →
    /// EverframeReporterUI dependency cycle. Single-writer-from-presenter
    ...
    public static func __performSetPresenting(_ value: Bool) {
        Everframe.shared.report.isPresenting = value
    }
}

public extension Notification.Name {
    /// Posted on every flip of `Everframe.shared.report.isPresenting`.
    ...
    static let everframeReporterPresentingChange =
        Notification.Name("dev.everframe.reporterPresentingChange")
}
