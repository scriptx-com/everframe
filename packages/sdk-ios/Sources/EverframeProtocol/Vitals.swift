...
// carries the `Vitals` prefix because Generated.swift already owns `EverframeVital`,
// `EverframeVitalKind` and `EverframeType` (the flattened envelope rendering of the union).
import Foundation

// MARK: - JSON value

/// A JSON value for player-event and custom `data`. Codable, Equatable and
/// Sendable, unlike the generated `EverframeJSONAny` (a class holding `Any`), which is
/// why the collector works in this type and converts to `EverframeJSONAny` only at the
...
public enum VitalsWireCodec {
    ...
    /// Decode is used by the parity tests only (the Everframe SDK never receives vitals). Note for
    /// `VitalsJSON.init(from:)`: on the macOS 14+ host Foundation's JSONDecoder tells `true`
    /// from `1`; older NSNumber-backed decoders did not, which is one more reason the Everframe SDK
