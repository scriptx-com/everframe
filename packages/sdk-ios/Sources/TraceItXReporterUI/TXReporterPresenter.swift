...
// is observed by TraceItXKit's existing UIWindow event interception. Here we wire
// `ReportAPI.__resolver` so `await TraceItX.shared.report.open()` resolves
// here, AND wires `ReportAPI.__setPresenting` so the SDK's `report.isPresenting`
...
public enum TXReporterPresenter {
    ...
    /// this once at app launch (typical placement: alongside `TraceItX.shared.start(...)`).
    ...
    public static func installResolver() {
        ...
        // or the `traceItXReporterPresentingChange` Notification.
    ...
    public static func openAndAwait() async throws -> ReportResult {
        ...
        // `TraceItX.__warmIdentityToken()`'s doc comment for the full chain.
        ...
        // Reporter window not yet constructed, so the reporter chrome is not
        ...
        TraceItX.shared.__replayFreeze()   // lifecycle freeze at reporter-open
        ...
        // consumes `payload.reactTree` any more. `TraceItX.attachReactTree`
        ...
        // contract honest.
        let (hostExtra, _) = await TraceItX.shared.__consumePendingAttachments()
        ...
        return try await withCheckedThrowingContinuation { (cont: CheckedContinuation<ReportResult, Error>) in
            ...
            let palette = ResolvedPalette(
                ThemeResolver.resolve(server: brandingServer, inline: TraceItX.shared.currentConfig?.theme)
            )
            let showWatermark = shouldShowWatermark(brandingServer)

            let wc = ReporterWindowController()
            self.controller = wc
            let vc = TXReporterViewController(
    ...
    /// Internal — invoked by TXReporterViewController on Send / Cancel.
