...
// `TXReporterPresenter.installResolver()` on iOS and callable directly on
...
// build and run there, unlike `TXReporterPresenter` (which is iOS-only: TV
...
public enum CompanionPinPresenter {
    ...
    private static func present(_ challenge: CompanionAttachChallenge) {
        ...
        // linger.
        guard !TraceItX.shared.companion.__builtinPinUiSuppressed else {
            dismiss()
            return
        }
        dismiss()
...
final class CompanionPinViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        instructionLabel.text = "Enter this code in the TraceItX dashboard"
