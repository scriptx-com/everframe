...
@MainActor
public final class EFReporterViewController: UIViewController, UITextFieldDelegate, UITextViewDelegate {

    // Inputs
    private let captureResult: ScreenshotCapture.Result?
    /// Host-attached opaque extra string. Set by Everframe.shared.setExtra(...)
    ...
    /// Resolved ONCE per presentation by EFReporterPresenter (Approach A —
    ...
    private func layoutFooter() {
        ...
        if showWatermark {
            ...
            label.text = "Powered by Everframe"
    ...
    @objc private func watermarkTapped() {
        guard let url = URL(string: "https://everframe.dev/?ref=powered-by") else { return }
    ...
    /// same invariants — see EFReporterPresenter.openAndAwait), crop to the
    ...
    @objc private func sendTapped() {
        ...
        // (`EFCapturedUser`), but both wired it in where `Inputs` is
        ...
        // Metadata always ship (the Everframe SDK's `includeLogs`/`includeNetwork`
        ...
        let inputs = ReporterSubmission.Inputs(
            ...
            // `EFUser` is a value type, so this is a snapshot by construction.
            //
            ...
            // session (and therefore which project's Everframe SDK key) it belongs to.
            // `submit(_:)` reads the config asynchronously, after this hop; a
            // `start(projectB)` in that window would otherwise have uploaded
            // this user under B's key. See `EFCapturedUser.swift`.
            //
        ...
        Task { [weak self] in
            ...
            do {
                ...
                // of a report the Everframe SDK has just been told not to send.
                self.onComplete(.success(.cancelled))
            } catch {
