...
struct CompanionPreviewSessionTests {
    ...
    /// `Everframe.start()`. These tests drive the session directly, so they
    /// model a started Everframe SDK — otherwise every capture is (correctly) refused by
    /// the kill-switch check.
    init() { Everframe.captureGate = true }

    /// `EverframeRelayMessage` is an ENUM wrapping the per-message structs, so these
    /// accessors pattern-match rather than cast — `headers.contains { $0 is
    /// EverframePreviewFrame }` does not compile, there is no such subtype relationship.
    final class Spy {
        enum Event { case header(EverframeRelayMessage); case binary(Data) }
        var events: [Event] = []
        func send(_ m: EverframeRelayMessage) { events.append(.header(m)) }
        func sendBinary(_ d: Data) { events.append(.binary(d)) }

        var binaryCount: Int {
            events.filter { if case .binary = $0 { return true } else { return false } }.count
        }
        var headers: [EverframeRelayMessage] {
