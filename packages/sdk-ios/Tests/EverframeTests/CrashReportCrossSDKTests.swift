...
//
// EverframeCrash/error-reporting cross-SDK fixture parity — Swift side of the
...
// `EverframeReportEnvelope` (Task 1 added `source` + `payload.crash`), re-encoded
...
struct CrashReportCrossSDKTests {
    @Test func hermesIdentityRoundTripsExactly() throws {
        ...
        let decoded = try Self.canonicalDecoder().decode(EverframeReportEnvelope.self, from: encoded)
    ...
    /// regenerated `EverframeReportEnvelope`, re-encode canonically, decode the
    ...
    @Test func jvmCrashFixture_preservesOrderedCausesAndMappingIdentity() throws {
        let env = try Self.canonicalDecoder().decode(EverframeReportEnvelope.self, from: Self.jvmFixtureData())
        let crash = try #require(env.payload.crash)
    ...
    @Test func preJvmCrashInitializerRemainsSourceCompatible() {
        let crash = EverframeCrash(
            ...
            frames: [EverframeFrame(col: 2, file: "index.js", function: "run", line: 1, raw: "at run")],
            handled: true,
            jsBundle: nil,
    ...
    @Test func preDetailsCrashInitializerPreservesJvmMetadata() {
        let jvm = EverframeJVMCrashMetadata(causes: [], causesTruncated: false, mappingID: "release-7")
        let crash = EverframeCrash(
            exceptionType: "TypeError",
            fatal: false,
            fingerprint: "0123456789abcdef",
    ...
    @Test func genericCauseFixtureRoundTripsAndWithPreservesCause() throws {
        let crash = try Self.canonicalDecoder().decode(EverframeCrash.self, from: Self.causeFixtureData())
        ...
        let decoded = try Self.canonicalDecoder().decode(EverframeCrash.self, from: encoded)
