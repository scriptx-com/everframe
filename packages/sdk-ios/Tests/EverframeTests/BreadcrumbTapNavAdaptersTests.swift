...
// `xcodebuild test -scheme Everframe-Package -destination
...
struct BreadcrumbTapNavAdaptersTests {
    ...
    private func crumbs(kind: EverframeBreadcrumbKind) -> [EverframeBreadcrumb] {
        BreadcrumbRingBuffer.shared.freeze()
    ...
    @Test func tap_maskedAncestor_labelIsMaskedAndLeaksNoText() throws {
        ...
        do {
            ...
            let sensitiveContainer = EFSensitiveView(frame: .zero)
            let button = UIButton(type: .system)
            button.setTitle(secretTitle, for: .normal)
    ...
    @Test func recordScreen_chainsFromTo_suppressesSelfTransition_andMergesHostData() throws {
        ...
        do {
            ...
            Everframe.shared.recordScreen(a)                          // first screen — no crumb
            Everframe.shared.recordScreen(a)                          // A → A — suppressed
            Everframe.shared.recordScreen(b, data: ["stack": "root", "from": "spoofed"])
            Everframe.shared.recordScreen("")                         // blank — dropped
    ...
    @Test func recordScreen_preStartOrKilled_isNoOpAndDoesNotSeedChain() throws {
        resetBreadcrumbState()
        NavigationBreadcrumbAdapter.__resetForTesting()
        Everframe.shared.kill() // force captureGate closed regardless of prior test order
        do {
            BreadcrumbSharedStateTestLock.lock.lock()
            defer { BreadcrumbSharedStateTestLock.lock.unlock() }
            Everframe.shared.recordScreen("PreStartA")
            Everframe.shared.recordScreen("PreStartB")
        ...
        do {
            BreadcrumbSharedStateTestLock.lock.lock()
            defer { BreadcrumbSharedStateTestLock.lock.unlock() }
            Everframe.shared.recordScreen("Home")
