...
// path (Notification.Name.everframeReporterPresentingChange) per CONTEXT D-02.
//
// Design notes:
//  • The test exercises ReportAPI directly via the public `__setPresenting`
//    slot indirection used by EFReporterPresenter at runtime — the slot is
//    populated in installResolver(); these tests populate it themselves to
//    stay independent of EverframeReporterUI (which lives in a separate target).
...
final class ReportAPIIsPresentingTests {
    ...
    @Test func test1_initialIsFalse() {
        let api = Everframe.shared.report
