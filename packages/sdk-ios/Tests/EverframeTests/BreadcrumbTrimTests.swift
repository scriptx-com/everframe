...
final class BreadcrumbTrimTests: XCTestCase {
    ...
    private struct Case: Decodable {
        let name: String
        let options: Options
        let input: [EverframeBreadcrumb]
        let expected: [EverframeBreadcrumb]
    }
    ...
    /// Canonical JSON (sorted keys) for a `[EverframeBreadcrumb]` array — used as the
    /// deep-equality check since `EverframeBreadcrumb` isn't `Equatable` (its `data`
    /// field is `[String: EverframeJSONAny]`, and `EverframeJSONAny` doesn't conform either).
    private func canonicalJSON(_ crumbs: [EverframeBreadcrumb]) throws -> String {
    ...
    func test_parity_fixture_all_cases() throws {
        ...
        // Tests/EverframeTests/BreadcrumbTrimTests.swift
        //   -> Tests/EverframeTests
        //   -> Tests
        //   -> sdk-ios
        //   -> packages
    ...
    func test_countCap_enforcesProtocolCeilingEvenUnderByteBudget() {
        ...
        let taps: [EverframeBreadcrumb] = (0..<200).map { i in
            EverframeBreadcrumb(
    ...
    func test_combinedByteEvictionAndCountCap_locksTsOutcomeNumbers() {
        let taps: [EverframeBreadcrumb] = (0..<130).map { i in
            EverframeBreadcrumb(
                data: nil, kind: .tap, level: nil, message: "tap-btn",
                seq: i, t: Double(i + 1), truncated: nil
            )
        }
        let consoles: [EverframeBreadcrumb] = (0..<5).map { i in
            EverframeBreadcrumb(
    ...
    func test_byteEvictionPass_sanityRespondsToByteBudgetWhenCountCapCannotEngage() {
        ...
        let taps: [EverframeBreadcrumb] = (0..<3).map { i in
            EverframeBreadcrumb(
                data: nil, kind: .tap, level: nil, message: "tap-btn",
                seq: i, t: Double(i + 1), truncated: nil
            )
        }
        let consoles: [EverframeBreadcrumb] = (0..<3).map { i in
            EverframeBreadcrumb(
                data: nil, kind: .console, level: nil, message: String(repeating: "c", count: 1500),
                seq: 200 + i, t: Double(200 + i), truncated: nil
            )
    ...
    func test_isTrimMarker_discriminatesOnNumericDroppedCount() throws {
        let numeric = try JSONDecoder().decode(
            [String: EverframeJSONAny].self, from: "{\"droppedCount\":3}".data(using: .utf8)!
        )
        let markerCrumb = EverframeBreadcrumb(
        ...
        let nonNumeric = try JSONDecoder().decode(
            [String: EverframeJSONAny].self, from: "{\"droppedCount\":\"x\"}".data(using: .utf8)!
        )
        let stringCrumb = EverframeBreadcrumb(
        ...
        let noDataCrumb = EverframeBreadcrumb(
