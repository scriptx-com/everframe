...
// call sites, `Everframe.start()`'s launch drain runs too early to EVER attach
...
// submitter.drainOutbox(identityHolder: Everframe.shared._identityHolder,
...
// `Everframe.shared.setIdentityToken(...)` the way a host would post-`start()`,
...
// .drainOutbox(identityHolder: Everframe.shared._identityHolder,
// currentReplayConfig: { await Everframe.shared.currentReplayConfig() })` — against a
...
struct CrashDrainIdentityHeaderTests {
    ...
    @Test func nonFatalCrashDrainAttachesTheHeaderOnceIdentityAndConfigHaveGenuinelySettled() async throws {
        try await withGlobalCaptureStateLock {
            ...
            let liveConfig = await Everframe.shared.currentReplayConfig()
            ...
            let tempDir = FileManager.default.temporaryDirectory
                .appendingPathComponent("everframe-crash-drain-identity-\(UUID().uuidString)", isDirectory: true)
