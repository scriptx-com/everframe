...
// touches `BreadcrumbRingBuffer.shared` / `Everframe.shared`).
//
// Cross-suite pollution note: `BreadcrumbRingBuffer.shared` is process-wide,
// and `StderrIntercept` (installed by ANY test's `Everframe.start(config:)` —
...
/// production `everframeUncaughtExceptionHandler` documents — so it can be
...
struct BreadcrumbAdaptersTests {
    ...
    // STDERR_FILENO for the WHOLE TEST PROCESS, and `Everframe.start(config:)`
    ...
    // build the Everframe SDK into the example iOS app, call `everframeLogger.log(...)`, and
    ...
    @Test func networkMap_nilStatusIsFailedAndError() {
        ...
        #expect(m.level == .error)
        #expect(m.data["status"]?.value is EverframeJSONNull)
        #expect(m.data["durationMs"]?.value is EverframeJSONNull)
    }
    ...
    @Test func errorHandler_chainsToPreviousHandlerAndLandsCrumb() throws {
        ...
        everframeUncaughtExceptionHandler(exception)
