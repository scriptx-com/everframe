...
final class VitalsSessionBoundaryTests: XCTestCase {
    ...
    func testKillTearsTheRuntimeDownAndClearsTheServerSignal() throws {
        ...
        let i = FakeIntegration(); _ = Everframe.shared.trackPlayer(i, name: "main")
    ...
    func testATrackPlayerBeforeStartIsHonouredByTheFirstStart() throws {
        ...
        let h = Everframe.shared.trackPlayer(i, name: "main")
        XCTAssertEqual(h.id, "")
        try Everframe.shared.start(config: config(appA))
    ...
    func testTrackVitalsIsDroppedWithNoRunningCollectorAndRecordedWithOne() throws {
        Everframe.shared.trackVitals("nothing")                       // no-op, no crash
        try Everframe.shared.start(config: config(appA))
        let c = try XCTUnwrap(waitForController()); enable()
        Everframe.shared.trackVitals("ad_break", data: ["position": "midroll"])
    ...
    func testAnEnvelopeCarriesOnlyTheVitalsOfTheSessionItWasCapturedUnder() throws {
        try Everframe.shared.start(config: config(appA))
        let epochA = Everframe.startEpochLockFree()
        _ = waitForController(); enable()
        Everframe.shared.trackVitals("under_a")
        ...
        try Everframe.shared.start(config: config(appB))
        _ = waitForController(); enable()
        Everframe.shared.trackVitals("under_b")
    ...
    func testTheTransportIsSilencedByKillAndByASupersedingStartEpoch() throws {
        ...
        try Everframe.shared.start(config: config(appA))
        let a = transport(boundTo: Everframe.startEpochLockFree())
        ...
        try Everframe.shared.start(config: config(appB))       // a superseding start, no kill
        a.send(Data(#"{"payload":{"kind":"chunk"}}"#.utf8))
        let b = transport(boundTo: Everframe.startEpochLockFree())
