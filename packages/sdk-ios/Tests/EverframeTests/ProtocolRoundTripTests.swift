...
struct ProtocolRoundTripTests {
    ...
    /// quicktype's generated `EverframeReportEnvelope`, re-encodes (sorted keys), and
    ...
    /// `[EverframeJSONAny]?` fields (EverframePayload, etc.), and Equatable was therefore
    ...
    @Test func canonicalMinimalEnvelope_decodesAndReencodes() throws {
        ...
        let legacy = EverframeReportEnvelope(attachments: env.attachments,
        ...
        let restored = try decoder.decode(EverframeReportEnvelope.self, from: encoder.encode(anchored))
