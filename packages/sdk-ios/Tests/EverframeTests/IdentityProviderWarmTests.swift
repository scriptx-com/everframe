...
//   2. `Everframe.captureUserSnapshot()` stamps `identitySubject` from the
...
// `Everframe.swift` nor `IdentityTokenHolder.swift` nor `IdentityGate.swift` is
...
/// `Everframe.__warmIdentityTokenPreReadHookForTesting` — mirrors
...
final class IdentityProviderWarmTests: XCTestCase {
    ...
    /// this target.
    private func pollForCapturedSubject() async -> EFCapturedUser {
        var captured = Everframe.shared.captureUserSnapshot()
        var attempts = 0
        while captured.identitySubject == nil && attempts < 100 {
            try? await Task.sleep(nanoseconds: 20_000_000)  // 20ms
            captured = Everframe.shared.captureUserSnapshot()
            attempts += 1
        }
        return captured
    }
    ...
    /// capture path (`Everframe.shared.captureUserSnapshot()`, via
    ...
    func testInstallingAOneShotTokenStillWarmsTheCacheImmediately() async throws {
        ...
        // project (see `Everframe._identityEnabledFlag`'s doc comment); this
    ...
    func testClearingIdentityStampsNoSubject() async throws {
        Everframe.shared.setIdentityToken(.token(jwt(sub: "eve", exp: Date().addingTimeInterval(300))))
    ...
    func testReporterOpenRewarmsAnAgedTokenBeforeTheNextCapture() async throws {
        ...
        // verbatim.
        let staleCaptured = Everframe.shared.captureUserSnapshot()
        XCTAssertNil(
        ...
        // THE FIX under test: this is exactly what `EFReporterPresenter
    ...
    func testReporterOpenWarmDoesNotInvokeTheProviderWhenIdentityIsDisabled() async throws {
        ...
        // Exactly what `EFReporterPresenter.openAndAwait()` /
    ...
    // { @MainActor in ... }`, `Everframe.swift`'s `start()`), so a
    ...
    /// Drives the REAL `start()`, via `Everframe.__warmIdentityTokenPreReadHookForTesting`
    ...
    /// 17's own citation, `Everframe.swift:741`, is real, iOS-only, UIKit-
    ...
    func testAProviderWarmRacingAProjectSwitchNeverObservesTheOldProjectsEnablement() async throws {
        let gate = AsyncGate()
        Everframe.__warmIdentityTokenPreReadHookForTesting = { await gate.waitUntilOpen() }
        defer { Everframe.__warmIdentityTokenPreReadHookForTesting = nil }
...
//
// `EFReporterPresenter.swift` (EverframeReporterUI module) and
...
final class ReporterOpenIdentityWarmSourceGate: XCTestCase {
    ...
    func testEFReporterPresenterWarmsIdentityAtReporterOpen() throws {
        let code = Self.strippingLineComments(
            try Self.source("Sources/EverframeReporterUI/EFReporterPresenter.swift"))

        let warmCall = try XCTUnwrap(
            code.range(of: "Everframe.shared.__warmIdentityToken()"),
            """
            EFReporterPresenter.swift no longer calls Everframe.shared.__warmIdentityToken() at \
        ...
        let captureCall = try XCTUnwrap(
            ...
            "capture-before-reporter ordering marker not found — has EFReporterPresenter.swift been restructured?"
        )
        XCTAssertTrue(
    ...
    func testCompanionCaptureBridgeWarmsIdentityAtReportRequest() throws {
        let code = Self.strippingLineComments(
            try Self.source("Sources/Everframe/Companion/CompanionCaptureBridge.swift"))

        XCTAssertNotNil(
            code.range(of: "Everframe.shared.__warmIdentityToken()"),
            """
            CompanionCaptureBridge.swift no longer calls Everframe.shared.__warmIdentityToken() at \
