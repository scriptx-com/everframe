...
//     `git show 4011fad0~1:packages/sdk-ios/Tests/EverframeTests/
...
struct CompanionBadgeServerConfigBoxTests {
    ...
    // setter). `Everframe.start()`'s session-boundary reset section
    // (Everframe.swift, alongside `_user = nil`/`_identityHolder.set(nil)`)
    ...
    // `Everframe.start()` from this test target is heavy (full config
    ...
    // above: `Everframe.start()`'s session-boundary reset clears
...
struct CompanionBadgeTests {
    ...
    func activeKeyWindowExcludesTheBadgeWindowWhenItIsTheOnlyOne() {
        ...
        let badgeWindow = EFCompanionBadgeWindow(frame: CGRect(x: 0, y: 0, width: 100, height: 100))
