...
// attaches `X-Everframe-Identity-Token` (a real person's bearer credential) to the
...
// headers — including `X-Everframe-Identity-Token` AND, separately,
// `X-Everframe-Companion-Attribution` — straight through to the new host.
//
...
final class IdentityHeaderRedirectTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("everframe-identity-redirect-\(UUID().uuidString)", isDirectory: true)
