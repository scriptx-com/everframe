...
//
// Self-declared identity (spec 2026-08-12), iOS half. `Everframe.shared.setUser`
// has always stored `_user` (Everframe.swift) and nothing ever read it back:
...
// `EverframeReporter(description:title:user:)` call and its `let user: EverframeUser? = { ... }`
...
final class EnvelopeUserTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("everframe-envelope-user-tests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        // Everframe.shared is a process-wide singleton whose state persists
        ...
        Everframe.__resetBodyStateResetHookForTesting()
    }
    ...
    func testSetUserReachesEnvelope() throws {
        try startSession()
        Everframe.shared.setUser(EFUser(id: "u_1", email: "a@b.com", displayName: "A"))
    ...
    /// populated `EFUser` produces a partially populated `user` object
    ...
    /// `Everframe.shared.currentUser`). `capture: CaptureConfig(logs: false)`
    ...
    func testKillClearsUserSoTheNextSessionIsAnonymous() throws {
        ...
        XCTAssertNil(Everframe.shared.currentUser, "kill() must not leave the self-declared user set")
    ...
    /// with it the Everframe SDK key, i.e. the PROJECT every subsequent report is
    ...
    /// need an equality over `EverframeConfig` (which carries closures and is
    ...
    func testStartClearsTheUserSoASecondProjectIsNotAttributedToTheFirst() throws {
        ...
        XCTAssertNil(
            Everframe.shared.currentUser,
            "start() must not carry the previous configuration's user into the new one"
        )
        let reporter = try captureAndDecodeReporter()
        XCTAssertNil(
            reporter["user"],
            "project A's user must never be uploaded under project B's Everframe SDK key"
        )
    }
    ...
    func testAnIdenticalRestartAlsoClearsTheUser() throws {
        ...
        try Everframe.shared.start(config: config)
        Everframe.shared.setUser(EFUser(id: "u_1"))

        try Everframe.shared.start(config: config)
        XCTAssertNil(Everframe.shared.currentUser, "a same-key restart clears the user too")
    }
    ...
    /// CONFIG — which carries the Everframe SDK key, i.e. the project the envelope is
    ...
    func testACapturedUserSurvivesWhenNoRestartIntervenes() throws {
        ...
        Everframe.shared.setUser(EFUser(id: "u_2", email: "b@b.com"))
    ...
    /// handed back A's person to be uploaded under B's Everframe SDK key. The guard
    ...
    func testASnapshotTakenInsideStartsResetWindowCannotResolve() throws {
        ...
        try startSession()
        Everframe.shared.setUser(EFUser(id: "u_1", email: "a@b.com", displayName: "A"))
        let epochUnderProjectA = Everframe.shared.currentStartEpoch

        var capturedInWindow: EFCapturedUser?
        Everframe.__bodyStateResetHookForTesting = {
            capturedInWindow = Everframe.shared.captureUserSnapshot()
        }
        ...
        XCTAssertEqual(
            captured.startEpoch, Everframe.shared.currentStartEpoch,
        ...
        XCTAssertNil(
            captured.resolve(),
            "a user snapshotted inside start()'s reset window must never resolve — it would upload project A's person under project B's Everframe SDK key"
        )
    }

    // MARK: - EverframeCrash entry boundary (round-5 external review, finding 2)

    /// `CrashReporter.captureFacts` used to read `Everframe.shared.currentUser`
    ...
    func testACrashCarriesTheUserActiveAtCrashEntryNotAtEncodeTime() throws {
        ...
        CrashReporter.__afterUserSnapshotHookForTesting = {
            Everframe.shared.setUser(EFUser(id: "u_2", email: "b@b.com", displayName: "B"))
        }
        ...
        XCTAssertEqual(
            Everframe.shared.currentUser?.id, "u_2",
    ...
    /// strength of a subject the Everframe SDK had already concluded, via the SAME
    ...
    func testACrashOutboxEntryCarriesNoIdentitySubjectWhenAnotherProjectStartsDuringProcessing() throws {
        ...
        XCTAssertNil(
            ...
            An epoch mismatch means the Everframe SDK already decided the whole captured snapshot is \
    ...
    /// a header on the strength of a subject captured while the Everframe SDK had
    ...
    /// Everframe SDK already decided, at capture time, not to attribute — even though
    ...
    func testACrashEntryCapturedWhileIdentityIsDisabledDrainsWithNoHeaderEvenAfterIdentityIsEnabled() async throws {
        ...
        await drainSubmitter.drainOutbox(
            ...
            epochAtInitiation: Everframe.shared.currentStartEpoch,
    ...
    /// authorized with project A's Everframe SDK key, uploaded to project A's
    ...
    func testADelayedDrainWithholdsTheHeaderWhenAnotherProjectStartsBeforeItRuns() async throws {
        ...
        let epochAtInitiation = Everframe.shared.currentStartEpoch
        ...
        XCTAssertNil(
            RecordingURLProtocol.recorded.first?.identityToken,
            """
            project B's live token must never attach to a request still authorized with project A's Everframe SDK \
    ...
    func testKillResetsTheIdentityEnabledFlag() throws {
        ...
        XCTAssertFalse(Everframe.shared._identityEnabledFlag.get())
    }

    func testPartialUserOmitsMissingFields() throws {
        try startSession()
        Everframe.shared.setUser(EFUser(email: "a@b.com"))
        let reporter = try captureAndDecodeReporter()
        let user = try XCTUnwrap(reporter["user"] as? [String: Any])
    ...
    /// carries the Everframe SDK key, so preserving the user re-opens the
    ...
    func testSetUserBeforeStartIsDroppedNotStored() throws {
        ...
        try startSession()
        XCTAssertNil(Everframe.shared.currentUser)
        let reporter = try captureAndDecodeReporter()
        XCTAssertNil(
            reporter["user"],
    ...
    func testSetUserAfterStartTakesEffect() throws {
        ...
        Everframe.shared.setUser(EFUser(id: "u_1", email: "a@b.com", displayName: "A"))
        XCTAssertEqual(Everframe.shared.currentUser?.id, "u_1")
        let reporter = try captureAndDecodeReporter()
    ...
    // `EFCapturedConfig`/`captureConfigSnapshot()`, superseded by
    // `EFCapturedSession`/`captureSessionSnapshot()`, which reads config,
    ...
    func testCaptureSessionSnapshotReflectsThePostKillEpochEvenThoughConfigPersists() throws {
        ...
        try Everframe.shared.start(config: projectA)
        let epochBeforeKill = Everframe.shared.captureSessionSnapshot().user.startEpoch

        Everframe.shared.kill()
        let snapshotAfterKill = Everframe.shared.captureSessionSnapshot()
    ...
    func testCaptureSessionSnapshotMovesConfigAndEpochTogetherAcrossARestart() throws {
        ...
        try Everframe.shared.start(config: projectB)
        let snapshotAtB = Everframe.shared.captureSessionSnapshot()
    ...
    func testASnapshotOfTheMixedStateReportsItselfSuperseded() throws {
        ...
        var capturedInWindow: EFCapturedSession?
        Everframe.__bodyStateResetHookForTesting = {
            capturedInWindow = Everframe.shared.captureSessionSnapshot()
        }
        defer { Everframe.__resetBodyStateResetHookForTesting() }
        ...
        XCTAssertEqual(
            captured.user.startEpoch, Everframe.shared.currentStartEpoch,
    ...
    // MARK: - EverframeCrash-entry session snapshot (2026-08-13 follow-ups item 6)
    ...
    private static let crumbMarkerA = "EVERFRAMEFOLLOWUP10-PROJECT-A-ONLY-CRUMB"
...
/// (see `EFCapturedSession`'s doc comment), so there is no longer any window
...
/// `Everframe.shared.currentStartEpoch` read at the drain-kickoff line
...
/// drain-kickoff line to a bare `Everframe.shared.currentStartEpoch` read
/// makes this fail.
///
/// Merge note (native-identity x captured-session, 2026-08-16): this
/// originally pinned the now-removed `EFCapturedConfig`/
...
/// `EFCapturedSession` subsumes that job (it already carries `config`
...
final class CrashReporterConfigSnapshotSourceGate: XCTestCase {
    private static func crashReporterSource() throws -> String {
        ...
        // …/Tests/EverframeTests/EnvelopeUserTests.swift → up 3 → package root.
        let packageRoot = thisFile
            .deletingLastPathComponent()   // EverframeTests
            .deletingLastPathComponent()   // Tests
            .deletingLastPathComponent()   // package root
        let file = packageRoot
            .appendingPathComponent("Sources/Everframe/Crash/CrashReporter.swift")
    ...
    func testConfigAndEpochAreReadFromOneAtomicSnapshotNotTwoSeparateReads() throws {
        ...
        let snapshotCall = try XCTUnwrap(
            code.range(of: "let captured = Everframe.shared.captureSessionSnapshot()"),
            """
            CrashReporter.swift no longer captures the user, config and generation \
            counters together via captureSessionSnapshot(). Reading Everframe.shared \
            .currentConfig and Everframe.shared.currentStartEpoch as two SEPARATE \
        ...
        let drainEpochArg = try XCTUnwrap(
            ...
            `Everframe.shared.currentStartEpoch` read here would reopen the round-9 P1 hazard: \
...
final class ReporterSubmissionUserExtrasSourceGate: XCTestCase {
    ...
    /// `extra`. Reading `Everframe.shared.currentUser` there meant a `setUser`
    ...
    func testSubmitReadsTheCapturedSnapshotAndNeverTheLiveSingleton() throws {
        ...
        XCTAssertNil(
            code.range(of: "Everframe.shared.currentUser"),
            """
            ReporterSubmission.swift reads Everframe.shared.currentUser again. \
        ...
        XCTAssertNotNil(
            code.range(of: "public let capturedSession: EFCapturedSession"),
    ...
    /// `Everframe.shared.currentConfig` live and required the captured user to
    ...
    func testSubmitRoutesByTheCapturedConfigAndNeverTheLiveOne() throws {
        ...
        XCTAssertNil(
            code.range(of: "Everframe.shared.currentConfig"),
            """
            ReporterSubmission.swift reads the LIVE config again. That is \
            follow-ups item 9 exactly: the Everframe SDK key decides which project \
    ...
    func testIdentityHeaderIsResolvedFromTheRealSingletonHolderAndThreadedIntoSubmit() throws {
        ...
        XCTAssertNotNil(
            ...
            capturedSubject. A live Everframe.shared._identityHolder.cachedSubject(...) read there \
        ...
        XCTAssertNotNil(
            code.range(of: "holder: Everframe.shared._identityHolder,"),
            "resolveIdentityHeader(...) no longer passes Everframe.shared._identityHolder (the real singleton) as its holder — identity wiring reverted to inert."
        )
        XCTAssertNil(
    ...
    func testIdentityHeaderResolutionChecksTheEpochBeforeResolvingAndWithholdsOnMismatch() throws {
        let code = Self.strippingLineComments(try Self.submissionSource())
        let epochProbe = "inputs.capturedSession.user.startEpoch == Everframe.shared.currentStartEpoch"
        ...
        let preCheck = try XCTUnwrap(
            ...
            Everframe.shared.currentStartEpoch before resolving the identity header. Without it, a \
    ...
    func testBothSubmitPathsSnapshotTheUserAtTheirOwnBoundary() throws {
        ...
        let vc = Self.strippingLineComments(
            try String(
                contentsOf: packageRoot.appendingPathComponent(
                    "Sources/EverframeReporterUI/ReporterViewController.swift"),
                encoding: .utf8)
        )
        XCTAssertNotNil(
            vc.range(
                of: #"let capturedSession\s*=\s*Everframe\.shared\.captureSessionSnapshot\(\)"#,
            ...
            would ship it under B's Everframe SDK key.
            """
        )
        XCTAssertNotNil(
        ...
        let bridge = Self.strippingLineComments(
            try String(
                contentsOf: packageRoot.appendingPathComponent(
                    "Sources/Everframe/Companion/CompanionCaptureBridge.swift"),
                encoding: .utf8)
        )
    ...
    ///
    /// The `EFCapturedUser` epoch guard cannot catch this: user and epoch are
    ...
    func testTheInAppReporterSnapshotsTheUserBeforeItBakesAnything() throws {
        ...
        XCTAssertTrue(
            ...
            Send-tap user, and the EFCapturedUser epoch guard cannot see it \
    ...
    /// of a subject the Everframe SDK had already concluded it should not rely on.
    ///
    ...
    func testPersistedIdentitySubjectIsGatedByTheSameEpochDecisionAsTheLiveToken() throws {
        ...
        let subjectArgRange = try XCTUnwrap(
            ...
            could attach a header on the strength of a subject the Everframe SDK had already concluded it \
            should not rely on.
            """
        )
        XCTAssertTrue(
...
final class ReporterSubmissionIdentityEnablementSourceGate: XCTestCase {
    ...
    func testIdentityEnablementIsReReadAfterResolutionAndGatesTheToken() throws {
        ...
        let stillEnabledRange = try XCTUnwrap(
            code.range(of: "let stillEnabled = isIdentityEnabled(await Everframe.shared.currentReplayConfig())",
        ...
        let assignRange = try XCTUnwrap(
            code.range(of: "capturedEpochStillCurrent = inputs.capturedSession.user.startEpoch == Everframe.shared.currentStartEpoch"),
...
/// Report EverframeResource Window (spec 2026-09-05) — same source-gate technique as
