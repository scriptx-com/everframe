...
// so tests are isolation-safe and don't leak state through `Everframe.shared.companion`.
import Testing
import Foundation
import Combine
...
final class CompanionStateTests {
    ...
    @Test func test2_stateChangePostsNotification() async throws {
        ...
        let token = NotificationCenter.default.addObserver(
            forName: .everframeCompanionStateChange,
            object: api,
            queue: nil
        ) { note in
    ...
    @Test func test4_pairUrlChangePostsNotification() async throws {
        ...
        let token = NotificationCenter.default.addObserver(
            forName: .everframeCompanionPairUrlChange,
            object: api,
            queue: nil
        ) { note in
    ...
    @Test func test7_everframeSharedCompanionExists() {
        // The public property must be reachable from the singleton.
        #expect(Everframe.shared.companion is CompanionAPI)
    }
