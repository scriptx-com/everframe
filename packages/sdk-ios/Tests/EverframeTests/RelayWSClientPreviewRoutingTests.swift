...
struct RelayWSClientPreviewRoutingTests {
    ...
    func terminalReportFramesClearStash() async throws {
        ...
        for (label, message) in [
            ("completed", EverframeRelayMessage.reportCompleted(
                EverframeReportCompleted(correlationId: "c1", eventId: "e1", type: "report.completed"))),
            ("failed", EverframeRelayMessage.reportFailed(
                EverframeReportFailed(correlationId: "c1", reason: "nope", type: "report.failed"))),
        ] {
            let (client, session) = makeClient()
    ...
    func shotBinaryIsIngested() async throws {
        ...
        let token = NotificationCenter.default.addObserver(
            forName: .everframeCompanionReportShotBinary, object: nil, queue: nil
