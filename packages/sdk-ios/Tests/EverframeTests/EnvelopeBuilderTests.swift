...
// 04-01-shipped builder end-to-end through Generated.swift's `EverframeReportEnvelope`,
...
struct EnvelopeBuilderTests {
    ...
    @Test func tvReportsDefaultToTVPlatformAndFormFactor() throws {
        for extra: [String: String] in [[:], ["sdk.platform": "invalid", "sdk.formFactor": "invalid"]] {
            let (bytes, _) = try makeBuilder().buildEncoded(reportId: UUID(), sdkName: "everframe-ios",
    ...
    @Test func buildEncoded_roundTripsThroughGeneratedSwift() throws {
        ...
        #expect(envelope.sdk.name == .everframeIos)
    ...
    @Test func sha256IsDeterministicAcrossEncodings() throws {
        ...
        let (bytes1, key1) = try builder.buildEncoded(
            reportId: id, sdkName: "everframe-ios", sdkVersion: "1.0.0",
    ...
    @Test func sizeCap_throwsOnOversize() throws {
        ...
        let (accepted, _) = try builder.buildEncoded(
            reportId: UUID(), sdkName: "everframe-ios", sdkVersion: "1.0.0",
        ...
        do {
        ...
        } catch let err as EverframeTransportError {
            switch err {
            ...
            Issue.record("expected EverframeTransportError, got \(error)")
        }
    }
    ...
    private func makeCrumb(
        kind: EverframeBreadcrumbKind, message: String, seq: Int, t: Double = 1_700_000_000_000
    ) -> EverframeBreadcrumb {
        EverframeBreadcrumb(data: nil, kind: kind, level: .info, message: message, seq: seq, t: t, truncated: nil)
    }
