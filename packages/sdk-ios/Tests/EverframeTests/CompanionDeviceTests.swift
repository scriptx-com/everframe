...
struct CompanionDeviceTests {
    ...
    @Test func hashToUuid_knownVector() {
        ...
        // rule, computed once and pinned here): SHA-256("everframe-test-vector"),
        // first 16 bytes, byte 6 → `(b & 0x0f) | 0x40`, byte 8 →
        // `(b & 0x3f) | 0x80`, hex-formatted 8-4-4-4-12.
        let id = CompanionDeviceId.hashToUuid("everframe-test-vector")
        #expect(id == "abb64861-9a56-4d83-9384-9c6559a5599e")
    }
