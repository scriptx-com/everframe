...
/// other suites that also drive `Everframe.shared`/`NetworkBodyCaptureGate
...
struct ReplaySessionRefreshLoopTests {
    ...
    // now fails closed on a nil `Everframe.shared.currentConfig` (spec §3 —
    ...
    // directly (not via `Everframe.start()`), so `currentConfig` would
    ...
    // mirroring the `@Before`/`@After` `Everframe.__setConfigForTesting`
    ...
    // Report EverframeResource Window (spec 2026-09-05) — negotiation-gap fix. Proves
    ...
    // apply without an Everframe SDK restart, and a malformed `windowSec` alone must
    ...
    // Report EverframeResource Window (spec 2026-09-05) — lifecycle wiring follow-up.
    ...
    // `teardown()` — the same call `Everframe.kill()` and a superseding
    ...
    // past the provider's TTL) must start sampling with no Everframe SDK restart —
    ...
    // EXISTING `Everframe.shared.__warmIdentityToken()` entry point rather
    ...
    // anonymous for the exact integration this Everframe SDK recommends.
    //
    // This drives `ReplaySession.refreshConfigNow()` directly, matching the
    // rest of this suite, plus `Everframe.shared.setIdentityToken(...)` /
    ...
    /// installed Xcode/Everframe SDK is ahead of CI's pinned version, which hits an
    ...
    @Test func aConfigApplyThatEnablesIdentityRetriesTheWarmAndCachesTheSubject() async {
      await withGlobalCaptureStateLock {
        ...
        defer { Everframe.shared.setIdentityToken(nil) }
        defer { Everframe.shared.__replayConfigOverrideForTesting = nil }
        ...
        //    `Everframe.shared.currentReplayConfig()` (what
        ...
        await poll { Everframe.shared._identityHolder.cachedSubject(now: Date()) != nil }
        #expect(
            Everframe.shared._identityHolder.cachedSubject(now: Date()) == "alice",
