...
final class CompanionAttachChallengeTests {
    ...
    /// A plain client with no Everframe SDK key — same shape as
    ...
    @Test func attachChallenge_setsPropertyAndPostsNotification() {
        ...
        let observer = NotificationCenter.default.addObserver(
            forName: .everframeCompanionAttachChallengeChange, object: api, queue: nil
        ) { note in posted = note }
    ...
    //
    // Round-2 review finding 3: `CompanionPinPresenter` (EverframeReporterUI)
    ...
    // lookup) and out of reach from this target (EverframeTests only depends
    // on EverframeKit/EverframeProtocol — see Package.swift), so the pure
    // deadline math lives on `CompanionAttachChallenge` in EverframeKit
    ...
    //
    // EverframeDevice naming (spec 2026-08-24): server -> TV push after a dashboard
    ...
    @Test func companionName_setsResolvedNameAndPostsNotification() {
        ...
        var posted: Notification?
        let observer = NotificationCenter.default.addObserver(
            forName: .everframeCompanionResolvedNameChange, object: api, queue: nil
        ) { note in posted = note }
        defer { NotificationCenter.default.removeObserver(observer) }
