...
//   • no Everframe SDK key means no HTTP call at all.
import Testing
import Foundation
...
final class RelayWSClientCompanionTests {
    ...
    // MARK: - EverframeDevice identity + resolvedName wiring (naming spec 2026-08-24,
    ...
    //
    // Android has enforced this since its `is EverframeReportRequest` branch was
    ...
    private func recordSends(_ client: RelayWSClient) -> UnsafeBox<[EverframeRelayMessage]> {
        let sent = UnsafeBox<[EverframeRelayMessage]>([])
    ...
    ///
    /// Scoped to ONE client, because `.everframeCompanionReportRequested` is a
    ...
    private func observeReportRequested(
        ...
        let token = NotificationCenter.default.addObserver(
            forName: .everframeCompanionReportRequested, object: client, queue: nil
        ) { note in
    ...
    @Test func aStaleCompletionFromASupersededBond_doesNotEndTheLiveReport() {
        ...
        // EFUser A attaches and starts a report.
        client.handleControl(#"""
    ...
    @Test func codeAndAttachedUserName_postNotificationsOnEveryFlip() async {
        ...
        let codeToken = NotificationCenter.default.addObserver(
            forName: .everframeCompanionCodeChange, object: api, queue: nil
        ) { note in codes.append(note.userInfo?["code"] as? String) }
        let nameToken = NotificationCenter.default.addObserver(
            forName: .everframeCompanionAttachedUserNameChange, object: api, queue: nil
...
private extension UnsafeBox where T == [EverframeRelayMessage] {
    func appendMessage(_ element: EverframeRelayMessage) {
        lock.lock(); _value.append(element); lock.unlock()
    }
}
