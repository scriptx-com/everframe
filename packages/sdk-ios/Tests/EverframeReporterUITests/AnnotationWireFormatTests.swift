...
// avoids UIKit (Foundation + CoreGraphics + EverframeProtocol only).
//
// EverframeJSONAny (EverframeProtocol/Generated.swift) decodes numbers by trying
...
// `Double`, inside `EverframeJSONAny.value`. A literal `as? [Double]` / `as? Double`
...
// are the numbers we expect" regardless of which numeric type EverframeJSONAny
// picked.
import Testing
import CoreGraphics
import Foundation
