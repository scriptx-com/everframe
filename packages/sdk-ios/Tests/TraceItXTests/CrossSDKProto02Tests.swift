...
// the regenerated quicktype `ReportEnvelope` (Plan 06-06 Task 2 added the
...
//   The quicktype-generated structs contain `[JSONAny]?` fields (annotations,
...
struct CrossSDKProto02Tests {
    ...
    /// `ReportEnvelope`, re-encode canonically, decode the canonical bytes,
    ...
    @Test func crossSDKFixture_omitsRetiredPayloadFields() throws {
        let env = try Self.canonicalDecoder().decode(ReportEnvelope.self, from: Self.fixtureData())
    ...
    /// `data.droppedCount` MUST decode as a NUMBER (JSONAny `.value as? Int64`),
