...
//
// RESEARCH Pitfall 5 — TraceItX.shared.start(_:) MUST return synchronously in
...
struct StartPerfTests {
    ...
    @Test func heavyInitRunsDetached() async throws {
        ...
        TraceItX.__resetHeavyInitFlagForTesting()
        try TraceItX.shared.start(config: config)
        // Heavy init runs on Task.detached — wait up to 1s for it to complete.
        let ran = await AsyncTestHelpers.waitFor({ TraceItX.__heavyInitDidRun }, timeout: 1.0)
