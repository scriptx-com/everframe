...
// round trip. `TraceItX.__pendingExtraResolveHook` is the seam
// `__consumePendingAttachments()` awaits right before it drains pending
// attachments; the RN bridge (`TraceItXBridge.configure`) installs a real
...
final class ExtraResolverHookTests {
    ...
    @Test func noHookInstalled_drainsPendingExtraExactlyAsBeforeThisFeature() async {
        TraceItX.__pendingExtraResolveHook = nil
        TraceItX.shared.setExtra("pre-existing value")

        let (extra, _) = await TraceItX.shared.__consumePendingAttachments()

        #expect(extra == "pre-existing value")
    }

    @Test func hookThatAnswersInTime_updatesTheDrainedValue() async {
        TraceItX.shared.setExtra("stale")
        TraceItX.__pendingExtraResolveHook = {
            ...
            TraceItX.shared.setExtra("fresh from resolver")
        }
    ...
    @Test func nonThrowingHookType_makesFailOpenHoldByConstruction() async {
        ...
        // to propagate here.
        TraceItX.shared.setExtra("still ships")
        TraceItX.__pendingExtraResolveHook = {
    ...
    @Test func hookThatNeverResumesButBoundsItself_stillLetsConsumeReturnPromptly() async {
        TraceItX.shared.setExtra("unchanged — JS never answered")
        TraceItX.__pendingExtraResolveHook = {
    ...
    @Test func consumeStillClearsPendingStateExactlyOnceRegardlessOfTheHook() async {
        TraceItX.shared.setExtra("only once")
        TraceItX.__pendingExtraResolveHook = nil

        let (first, _) = await TraceItX.shared.__consumePendingAttachments()
        let (second, _) = await TraceItX.shared.__consumePendingAttachments()
    ...
    // the other gets nil. `TraceItX.__consumePendingAttachments()`'s
    ...
    func twoConcurrentConsumeCallsAreSerialized_BsHookDoesNotStartUntilAsWholeAskAndWaitAndDrainFinish() async {
        ...
        TraceItX.__pendingExtraResolveHook = {
            ...
            if index == 1 {
                await gateA.wait()
                TraceItX.shared.setExtra("from A")
            } else {
                await gateB.wait()
                TraceItX.shared.setExtra("from B")
            }
            await recorder.record("exit-\(index)")
        }

        let taskA = Task { await TraceItX.shared.__consumePendingAttachments() }
        let taskB = Task { await TraceItX.shared.__consumePendingAttachments() }
    ...
    func aSlowHookDoesNotBlockReportBFromEventuallyShipping_itJustSerializesAfterA() async {
        ...
        TraceItX.__pendingExtraResolveHook = {
            ...
            if index == 1 {
                await gateA.wait()
                TraceItX.shared.setExtra("A's value")
            } else {
                TraceItX.shared.setExtra("B's value")
            }
        }
