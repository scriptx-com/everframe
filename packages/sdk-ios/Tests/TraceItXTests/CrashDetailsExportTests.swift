...
private enum CrashDetailsNativeExport {
    ...
    static func writeIfRequested(_ document: Document) throws {
        guard let output = ProcessInfo.processInfo.environment["TRACEITX_CRASH_DETAILS_OUTPUT"] else {
            return
        }
        guard NSString(string: output).isAbsolutePath else {
            throw NSError(domain: "CrashDetailsNativeExport", code: 1,
                          userInfo: [NSLocalizedDescriptionKey: "TRACEITX_CRASH_DETAILS_OUTPUT must be absolute"])
        }
        ...
        guard FileManager.default.fileExists(
            ...
            throw NSError(domain: "CrashDetailsNativeExport", code: 2,
                          userInfo: [NSLocalizedDescriptionKey: "TRACEITX_CRASH_DETAILS_OUTPUT parent must exist"])
        }
        guard !FileManager.default.fileExists(atPath: url.path) else {
            throw NSError(domain: "CrashDetailsNativeExport", code: 3,
                          userInfo: [NSLocalizedDescriptionKey: "TRACEITX_CRASH_DETAILS_OUTPUT must be absent"])
        }
