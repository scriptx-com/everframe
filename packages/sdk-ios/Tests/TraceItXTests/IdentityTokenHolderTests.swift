...
// NOT on the TraceItX singleton: TraceItX.swift is canImport(UIKit)-gated, so a
// holder living there would be unreachable from `swift test` on macOS — which
// is every test run this SDK has in CI.
//
...
final class IdentityTokenHolderTests: XCTestCase {
    ...
    func testAMalformedTokenStillLetsTheReportSubmit() async throws {
        ...
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("traceitx-malformed-token-\(UUID().uuidString)", isDirectory: true)
        ...
        let submitter = ReportSubmitter(
            config: TraceItXConfig(appId: "app"), outbox: outbox, session: session)
    ...
    // report. `ttlSeconds` up to 600 is a supported `@traceitx/identity`
    ...
    // finding. `TraceItX.captureUserSnapshot()` never calls `get(now:)` at
    ...
    func testKillClearsTheIdentityToken() async {
        ...
        let got = await TraceItX.shared._identityHolder.get(now: now)
        XCTAssertNil(got, "a killed SDK must present nothing")
    }
    ...
    /// `_identityHolder.cancelOutstandingWork()` call from `TraceItX.swift`'s
    ...
    // the closure `ReplaySession` handed it read `TraceItX.shared
    ...
    // (`_startEpochMirrorLock`, TraceItX.swift) instead of `stateLock` — the
    ...
    /// unreliable to hit on purpose). `TraceItX.__lockStateLockForTesting()`
    ...
    func testFlagSetGuardNeverDeadlocksAgainstAConcurrentStateLockHolderCallingIntoTheFlag() throws {
        ...
        let epochAtCapture = TraceItX.shared.currentStartEpoch
        ...
        let threadR = Thread {
            TraceItX.shared._identityEnabledFlag.set(true) {
                guardEntered.signal()
                releaseGuard.wait()
                return TraceItX.shared.currentStartEpoch == epochAtCapture
            }
        }
        threadR.start()
        ...
        TraceItX.__lockStateLockForTesting()
        defer { TraceItX.__unlockStateLockForTesting() }
    ...
    // `Task`, so `cancelOutstandingWork()` (wired into `TraceItX.kill()`,
    ...
    func testProviderClosureTypeStaysNonThrowingByConstruction() throws {
        ...
        // …/Tests/TraceItXTests/IdentityTokenHolderTests.swift → up 3 → package root.
        ...
        let source = try String(
            contentsOf: packageRoot.appendingPathComponent(
                "Sources/TraceItX/Identity/IdentityTokenHolder.swift"),
            encoding: .utf8
        )
        XCTAssertNotNil(
