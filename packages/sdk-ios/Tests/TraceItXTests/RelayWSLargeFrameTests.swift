...
@MainActor final class RelayWSLargeFrameTests: XCTestCase {
    func testMultiMegabyteScreenshotReachesSubmitBridge() async throws {
        let queue = DispatchQueue(label: "traceitx.test.websocket")
        ...
        let submit = try JSONEncoder().encode(ReportSubmit(annotations: [], correlationId: "large-tv-shot",
        ...
        let token = NotificationCenter.default.addObserver(forName: .traceItXCompanionReportSubmitBinary,
