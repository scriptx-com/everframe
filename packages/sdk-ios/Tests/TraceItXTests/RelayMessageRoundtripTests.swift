...
//
// Phase 06.2-07 Task 1 — RelayMessage codegen round-trip.
//
// Loads `packages/protocol/__tests__/fixtures/v1-relay-fixture.json` (copied
// into `Tests/TraceItXTests/Fixtures/`) and round-trips each entry through
// the codegenned `RelayMessage` enum produced by Plan 06.2-03. Catches:
...
struct RelayMessageRoundtripTests {
    ...
    private struct FixtureFile: Decodable {
        ...
        let messages: [JSONValue]  // raw JSON nodes — each decoded as a RelayMessage
    }
    ...
    @Test func test1_fixtureSchemaVersion() throws {
        ...
        #expect(f.messages.count == 10) // one per RelayMessage case
    }
    ...
    @Test func test2_eachMessageDecodesAndReEncodesEquivalently() throws {
        ...
        for (idx, raw) in f.messages.enumerated() {
            ...
            do {
                msg = try dec.decode(RelayMessage.self, from: originalData)
            } catch {
            ...
            // same `RelayMessage` enum value (Equatable conformance is the
            // ground truth).
            let reencoded: Data
            do { reencoded = try enc.encode(msg) }
            catch { Issue.record("message[\(idx)] failed re-encode: \(error)"); continue }

            let msg2: RelayMessage
            do { msg2 = try dec.decode(RelayMessage.self, from: reencoded) }
    ...
    @Test func test3_allCasesCoveredByFixture() throws {
        ...
        for raw in f.messages {
            ...
            let msg = try dec.decode(RelayMessage.self, from: data)
    ...
    private func discriminator(of msg: RelayMessage) -> String {
        switch msg {
...
/// re-encode them, and let `RelayMessage` parse from the bytes. Avoids
