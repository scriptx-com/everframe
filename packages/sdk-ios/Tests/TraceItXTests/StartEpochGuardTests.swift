...
// even though the SDK believes itself killed.
//
...
//
// `TraceItX.__startTailDelayHookForTesting` parks the async tail right after
...
struct StartEpochGuardTests {
    ...
    // `TraceItX.shared` (start/kill, the async start-tail hook) for real —
    ...
    @Test func killDuringStartsAsyncTailDiscardsTheStaleSessionInstall() async throws {
        try await withGlobalCaptureStateLock {
            defer { TraceItX.__resetStartTailDelayHookForTesting() }
            ...
            #expect(
                !TraceItX.__hasReplaySessionForTesting,
    ...
    @Test func startAfterTheRacedKillStillInstallsItsOwnSessionNormally() async throws {
        try await withGlobalCaptureStateLock {
            ...
            await gate.open()
            try? await Task.sleep(nanoseconds: 200_000_000)
            #expect(!TraceItX.__hasReplaySessionForTesting)
