...
// cross-suite parallel tests that toggle TraceItX.shared's kill-gate cannot
...
struct NetworkBodyRingBufferTests {
    ...
    // Round-6 review Finding F31: drives `TraceItX.shared` for real —
    ...
    /// can force `TraceItX.shared.kill()` (which, on `.shared`, synchronously
    ...
    // `TraceItX.shared` for real — wrapped in `withGlobalCaptureStateLock`
    ...
    @Test func killGateRaceDoesNotInsertAfterConcurrentClear() async throws {
        try await withGlobalCaptureStateLock {
            ...
            // production (TraceItX.swift's kill(), ~line 448-477).
            TraceItX.shared.kill()
    ...
    @Test func f34_remoteConfigDisablingBodiesBeforeAppendDropsTheAlreadyBuiltEntry() {
        ...
        // Decision point: gate ON, exactly as `TXNetworkCaptureProtocol`
