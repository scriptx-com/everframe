...
final class RNDetailsDeliveryTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("traceitx-rn-details-\(UUID().uuidString)", isDirectory: true)
    ...
    func testPublicFactsPreserveAbsenceAndIsolateInvalidOptionalDetails() throws {
        let config = TraceItXConfig(appId: "rn-details-app")
        let legacy = outbox("legacy.jsonl")
        XCTAssertTrue(CrashReporter.captureHandledFacts(
            json: #"{"exceptionType":"Legacy"}"#,
            sdkName: "traceitx-react-native", outbox: legacy, config: config
        ))
        ...
        for (index, value) in ["null", "42", "[]"].enumerated() {
            ...
            XCTAssertTrue(CrashReporter.captureHandledFacts(
                ...
                sdkName: "traceitx-react-native", outbox: invalid, config: config
            ))
    ...
    func testHandledAndAutomaticFactsPersistOwnedDetailsAndEntryIdentity() throws {
        TraceItX.__setConfigForTesting(TraceItXConfig(appId: "entry-session-app"))
        TraceItX.captureGate = true
        TraceItX.shared.setUser(TXUser(id: "session-user"))
        ...
        XCTAssertTrue(CrashReporter.captureHandledFacts(
            json: json, sdkName: "traceitx-react-native", outbox: handled
        ))
        XCTAssertTrue(CrashReporter.captureFacts(
            json: json, sdkName: "traceitx-react-native", outbox: automatic
        ))
        ...
        for (entry, isHandled) in [
            ...
            XCTAssertEqual(String(decoding: stored.prefix(8), as: UTF8.self), "TXOBX001")
    ...
    func testLifecycleAndConfigurationRefusalsDoNotPersistDetails() throws {
        ...
        TraceItX.__setConfigForTesting(TraceItXConfig(appId: "app"))
        TraceItX.captureGate = true
    ...
    func testPublicFactsRetryByteIdenticalDetailsAndOriginalHeadersAfter503() async throws {
        ...
        TraceItX.__setConfigForTesting(TraceItXConfig(appId: appId))
        TraceItX.shared.__replayConfigOverrideForTesting = identityConfig
        ...
        XCTAssertTrue(CrashReporter.captureHandledFacts(
            ...
            """,
            sdkName: "traceitx-react-native", outbox: persisted
        ))
        ...
        let submitter = ReportSubmitter(
            config: TraceItXConfig(appId: appId), outbox: reopened, session: session
        )
        let epoch = TraceItX.shared.currentStartEpoch
        let identityHolder = TraceItX.shared._identityHolder
        let firstDrain = Task {
