...
// `ReplaySession`/`TraceItX` elsewhere was left open at the network-body
// gate's own apply boundary.
//
// Window (a): `refreshConfigNow()`'s `guard TraceItX.shared.currentStartEpoch
...
// `TraceItX.start()`/`kill()` are synchronous, NON-actor-isolated public
...
// see `TraceItX.start()`'s doc comment for why this is two short, un-nested
...
// gate's own lock).
//
// Belongs to the same `TraceItX.shared` / `NetworkBodyCaptureGate.shared` /
...
struct StartEpochApplyRaceTests {
    ...
    @Test func aParkedBetweenItsEpochCheckAndTheApplyCannotArmTheGateAfterStartBOnABackgroundThread() async throws {
      try await withGlobalCaptureStateLock {
        ...
        try TraceItX.shared.start(config: TraceItXConfig(appId: appA, capture: captureConfig))
        let epochAtACreation = TraceItX.shared.currentStartEpoch
        ...
        // actor-isolated.
        let bThread = Thread {
            try? TraceItX.shared.start(config: TraceItXConfig(appId: appB, capture: captureConfig))
        }
        bThread.start()

        let bEpochBumped = await AsyncTestHelpers.waitFor({ TraceItX.shared.currentStartEpoch != epochAtACreation })
    ...
    @Test func aNonSupersededApplyPublishesBothTheCompanionBadgeAndBrandingBoxes() async throws {
      try await withGlobalCaptureStateLock {
        let captureConfig = CaptureConfig(logs: false, network: true)
        try TraceItX.shared.start(config: TraceItXConfig(appId: appA, capture: captureConfig))
        let epochAtCreation = TraceItX.shared.currentStartEpoch
        CompanionBadgeServerConfigBox.shared.value = nil
        defer { CompanionBadgeServerConfigBox.shared.value = nil }
        BrandingServerConfigBox.shared.value = nil
        defer { BrandingServerConfigBox.shared.value = nil }
    ...
    @Test func startBumpsTheGlobalEpochNoLaterThanItResetsTheBodyCaptureGate() async throws {
      try await withGlobalCaptureStateLock {
        let epochBeforeStart = TraceItX.shared.currentStartEpoch

        var epochObservedAtResetHook: Int?
        TraceItX.__bodyStateResetHookForTesting = {
            epochObservedAtResetHook = TraceItX.shared.currentStartEpoch
        }
        defer { TraceItX.__resetBodyStateResetHookForTesting() }

        try TraceItX.shared.start(config: TraceItXConfig(appId: appA, capture: CaptureConfig(logs: false)))
