...
// See `TraceItX.swift`'s `start()` for the full rationale.
//
...
struct SessionBoundaryResetTests {
    ...
    private func bodyEntry(ref: Int) -> NetworkBody {
        NetworkBody(
            ref: Double(ref),
    ...
    // `TraceItX.shared` (start/kill) and `NetworkBodyCaptureGate.shared` /
    ...
    @Test func startBSynchronouslyResetsSessionAsNetworkBodyState() async throws {
      try await withGlobalCaptureStateLock {
        ...
        let configA = TraceItXConfig(appId: appA, capture: CaptureConfig(logs: false))
        try TraceItX.shared.start(config: configA)
        ...
        let configB = TraceItXConfig(appId: appB, capture: CaptureConfig(logs: false))
        try TraceItX.shared.start(config: configB)
    ...
    @Test func startBTearsDownSessionAsRefreshLoopSoItCannotReArmTheGateLater() async throws {
      try await withGlobalCaptureStateLock {
        ...
        // `TraceItX.shared.currentConfig` snapshot installed by `start()`
        ...
        // TraceItX.swift's start() doc comment for why this can't be made
    ...
    // pattern) and is NEVER installed as `TraceItX.shared._replaySession` and
