...
// longer exists.
//
// Fixed two ways (mirroring `TraceItX._startEpoch`, round-2 review Finding
// F9, commit 826e5f76):
...
//      `TraceItX.kill()`'s MainActor hop, NOT just relied upon via `deinit`
...
struct ReplaySessionTeardownRaceTests {
    ...
    // network-body gate fails closed on a nil `TraceItX.shared.currentConfig`
    // (spec §3). This suite drives `ReplaySession` directly (not via
    // `TraceItX.start()`), so `currentConfig` would otherwise stay nil
    ...
    // `NetworkBodyCaptureGate.shared` and `TraceItX.shared`'s test-only
    ...
    @Test func staleInitialFetchCompletingAfterTeardownCannotArmTheGlobalBodyGate() async {
      await withGlobalCaptureStateLock {
        ...
        // TraceItX.kill() calls) landing mid-await in production.
        session.teardown()
