...
// `TraceItX._replaySession` with a fresh session WITHOUT tearing down the
...
// `TraceItX.__replaySessionFactoryForTesting` — real network is
...
struct ReplaySessionSupersessionTests {
    ...
    // Round-6 review Finding F31: drives `TraceItX.shared` (start) and
    ...
    @Test func supersedingStartTearsDownPriorSessionSoItsStaleOnResponseCannotArmTheGate() async throws {
      try await withGlobalCaptureStateLock {
        ...
        // reads that flag off the REAL `TraceItX.shared.currentConfig`
        ...
        let config = TraceItXConfig(appId: testAppId, capture: CaptureConfig(logs: false, network: true))
