...
final class CompanionSubmitBridgeTests {
    ...
    @Test func test2b_multiShotSubmit_waitsForEveryShotThenProceeds() async throws {
        ...
        submit = ReportSubmit(
            ...
            shots: [ReportSubmitShot(annotations: [], shotId: "s1")],
            title: submit.title,
            type: submit.type)
        NotificationCenter.default.post(
            name: .traceItXCompanionReportSubmit, object: nil,
            userInfo: ["correlation_id": corrId, "submit": submit])
        NotificationCenter.default.post(
            name: .traceItXCompanionReportSubmitBinary, object: nil,
        ...
        NotificationCenter.default.post(
            name: .traceItXCompanionReportShotBinary, object: nil,
    ...
    // `.traceItXCompanionReportSubmitBinary` notification when a
    ...
    //
    // Contract: the companion bridge consumes `TraceItX.shared.setExtra` +
    // `TraceItX.shared.attachReactTree` exactly ONCE at the moment of
    // `report.request` — not at submit time. After consumption,
    // `__consumePendingAttachments()` MUST return (nil, nil). This mirrors
    // the in-process iOS/tvOS reporter modal (TXReporterPresenter.swift:72)
    ...
    // public API: TraceItX.shared.__consumePendingAttachments()) and verify
    ...
    @Test func test4_consumePendingAttachments_drainsOnce() async throws {
        ...
        _ = await TraceItX.shared.__consumePendingAttachments()

        // Two values set.
        TraceItX.shared.setExtra("companion-extra")
        let treeJSON = Self.makeReactTreeJSON()
        TraceItX.shared.attachReactTree(treeJSON)

        // First drain (what the bridge does inside handleReportRequest).
        let (extra1, tree1) = await TraceItX.shared.__consumePendingAttachments()
        #expect(extra1 == "companion-extra")
        #expect(tree1 != nil)

        // Second drain — must be nil (consume-once invariant).
        let (extra2, tree2) = await TraceItX.shared.__consumePendingAttachments()
        #expect(extra2 == nil)
        #expect(tree2 == nil)
    }
    ...
    /// (TXReporterPresenter.swift custom .dateDecodingStrategy).
    ...
    static func makeSubmit(correlationId: String) -> ReportSubmit {
        return ReportSubmit(
            annotations: [],
            correlationId: correlationId,
            description: ReportSubmitDescription(redactions: [], text: "test desc"),
            includes: ReportSubmitIncludes(
