...
final class ReplayConfigProviderTests: XCTestCase {
    ...
    private let configUrl = URL(string: "https://traceitx.com/api/config")!
    private let apiKey = "tx_test_key"
    ...
    // MARK: - resources block (Report Resource Window, spec 2026-09-05)
    //
    // The server emits this block ONLY to a caller that declares `resources`
    // in X-TX-SDK-Features — without the token below the feature is
    // silently, permanently off (the negotiation gap this SDK closes).
    ...
    // must still come through, so the SDK knows the feature is on and falls
    ...
    // Session Vitals never shipped on iOS — this SDK must not advertise a
    ...
    func testRefreshDeclaresNetworkBodiesCapability() async {
        ...
        // `resources` (Report Resource Window, spec 2026-09-05),
        ...
        XCTAssertEqual(
            req?.value(forHTTPHeaderField: "X-TX-SDK-Features"),
