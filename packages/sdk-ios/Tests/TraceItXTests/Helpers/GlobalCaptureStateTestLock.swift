...
// NetworkBodyRingBufferTests, NetworkRingBufferTests) drives the SAME process-global `TraceItX.shared`
...
// does, before touching `TraceItX.shared`, `NetworkBodyCaptureGate.shared`,
...
// well — every symbol it touches (`TraceItX.shared.kill()`,
...
/// touches `TraceItX.shared` / `NetworkBodyCaptureGate.shared` /
...
/// `TraceItX.shared` itself (`kill()` — nils the replay session, flips the
...
/// including when nothing was ever `start()`-ed: `TraceItX.kill()` is
...
func resetGlobalCaptureStateForTesting() async {
    ...
    #if canImport(UIKit)
    _ = await AsyncTestHelpers.waitFor({ !TraceItX.__hasReplaySessionForTesting })
    #endif
