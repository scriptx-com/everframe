...
/// no `TraceItX.shared`, no stderr intercept, no UIKit — the criteria for
...
final class OutboxKeyBindingTests: XCTestCase {
    func test_drain_keeps_report_on_disk_until_server_accepts_it() async throws {
        ...
        let submitter = ReportSubmitter(config: TraceItXConfig(appId: "key-A"), outbox: box, session: session)
    ...
    override func setUpWithError() throws {
        ...
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("traceitx-keybinding-\(UUID().uuidString)", isDirectory: true)
    ...
    func test_drain_submits_each_entry_with_its_own_key_and_endpoint() async throws {
        ...
        let submitter = ReportSubmitter(
            config: TraceItXConfig(appId: "key-B"),
            outbox: box,
    ...
    func test_default_outbox_path_is_isolated_under_xctest() throws {
        ...
        XCTAssertTrue(
            path.contains("com.traceitx-test-"),
