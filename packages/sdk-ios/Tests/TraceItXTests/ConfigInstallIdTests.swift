...
// endpoint is this SDK's remote kill switch: an uncounted install is
...
final class ConfigInstallIdTests: XCTestCase {
    ...
    func test_the_supplier_is_evaluated_on_every_fetch_not_once() async {
        ...
        // effect — which is exactly how the web SDK shipped in 2a, and why it
        // needs a retrofit.
        let fetcher = RecordingFetcher()
        let counter = Counter()
    ...
    // behavioural test, for `TraceItX.swift`'s `start()` call site. No
    // behavioural test can reach `TraceItX.swift:1070`'s
    // `session = ReplaySession(...)` construction: every suite that drives
    // `start()` (`KillSwitchTests`, `ReplaySessionSupersessionTests`, etc.)
    // does so through `TraceItX.__replaySessionFactoryForTesting`, which
    ...
    // / `TXReporterPresenterBrandingSourceGate`): it fails if a future edit
    ...
    func test_start_constructs_the_replay_session_with_the_install_id_provider_argument() throws {
        let code = Self.strippingLineComments(
            try Self.source("Sources/TraceItX/TraceItX.swift"))

        let callSite = try XCTUnwrap(
            code.range(of: "session = ReplaySession("),
            """
            TraceItX.swift no longer constructs ReplaySession at its documented start() \
        ...
        let argument = try XCTUnwrap(
            ...
            """
            TraceItX.swift's start() no longer passes installIdProvider: installIdProvider to \
            the ReplaySession it constructs. Without this argument the MAI install \
            identifier is silently dropped at the SDK's own start() call site — the \
    ...
    // test on this host, so there is no way to actually start() the SDK with
    ...
    // (`TraceItXConfig.installIdentifierEnabled`) — a host that has
    ...
    func test_start_feeds_the_config_flag_into_the_install_id_supplier() throws {
        ...
        let callSite = try XCTUnwrap(
            ...
            """
            TraceItX.swift no longer builds the install-id supplier via \
        ...
        let argument = try XCTUnwrap(
            ...
            """
            TraceItX.swift's start() no longer passes \
...
extension ConfigInstallIdTests {
    ...
    // one call site in `TraceItX.swift` — so a convenience init that
