...
# @traceitx/babel-plugin-displayname

> Preserves React component `displayName` through production minification — required for AI-readable TraceItX reports.

**License:** MIT

## Why

When Babel + terser/uglify minifies your bundle, component identifiers like `Foo` become `v3` or `t`. TraceItX resolves the focused element back to its component name by reading whatever names exist on the DOM at report time — without `displayName` baked in, every report points at `<v3 />` gibberish, useless to an AI agent or human reviewer.
...
module.exports = {
  plugins: [
    ['@traceitx/babel-plugin-displayname', { factories: ['withProfiler'] }],
  ],
};
```
...
If your project uses SWC (Next.js 12+, Metro for RN), use [`@traceitx/swc-plugin-displayname`](../swc-plugin-displayname) instead. Both plugins implement identical case behavior — see `__tests__/fixtures/displayname-cases.md`.
