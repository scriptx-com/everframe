...
//
// The machinery itself lives in `@traceitx/web` (src/companion/), which is
// framework-free by construction. This file re-composes that namespace with
// the one React-coupled member — `useCompanion` — so `@traceitx/react`'s
...
// `export type { … } from '@traceitx/web'`: tsup's dts step (rollup-plugin-dts)
...
import type * as web from '@traceitx/web';

export { createCompanion } from '@traceitx/web';
...
export { handleReportRequest, handleReportSubmit } from '@traceitx/web';
...
export { start, stop } from '@traceitx/web';
