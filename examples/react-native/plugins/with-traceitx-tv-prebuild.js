...
// a small set of TraceItX-specific TV manifest deltas the upstream plugin
...
//        revisions that might clobber it on tvOS. TraceItX explicitly keeps
...
//
// Use one explicit per-platform branch. TraceItX has a single sample, so this
...
const EXCLUDE_MARKER =
  '# TRACEITX_TV_AUTOLINK_EXCLUDE — injected by ./plugins/with-traceitx-tv-prebuild.js';
  ...
  return withDangerousMod(config, [
    ...
    async (cfg) => {
      ...
      if (!fs.existsSync(podfilePath)) {
        throw new Error(
          `[with-traceitx-tv-prebuild] expected Podfile at ${podfilePath}`,
        );
      }
      ...
      if (!match) {
        throw new Error(
          '[with-traceitx-tv-prebuild] could not locate the autolinking ' +
      ...
      console.log(
        `[with-traceitx-tv-prebuild] tvOS autolinking excludes: ${TV_AUTOLINK_EXCLUDE.join(', ')}`,
      );
      return cfg;
    },
  ]);
...
function withTraceItXTVPrebuild(config) {
...
module.exports = withTraceItXTVPrebuild;
