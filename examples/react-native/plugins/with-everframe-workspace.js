...
// Android build to resolve the workspace Everframe SDK from the local
...
// `:everframe_sdk-react-native`, which declares
// `implementation("dev.everframe:everframe-core:1.2.0-SNAPSHOT")` and
...
//   `pnpm --filter @everframe/sdk-android run publish:maven-local` before
...
// in app.json) plus pnpm's symlinked `node_modules/@everframe/...` lets
// the SDK podspec's `spm_dependency` resolve to packages/sdk-ios
// without further plugin help.

const { withProjectBuildGradle } = require('@expo/config-plugins');

const PROBE = 'EVERFRAME_MAVEN_LOCAL';
const COMMENT = '// EVERFRAME_MAVEN_LOCAL — Injected by ./plugins/with-everframe-workspace.js';
...
function withEverframeWorkspace(config) {
...
module.exports = withEverframeWorkspace;
