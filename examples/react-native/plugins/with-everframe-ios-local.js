...
// workspace's local Everframe iOS SDK (`packages/sdk-ios`) instead of the
// published `Everframe` pod on CocoaPods Trunk.
//
// What it does:
...
//      `pod install` failure ("missing dist/EverframeKit.xcframework")
//      when the dev forgets the build step.
//   2. Injects a `pod 'Everframe', :path => ..., :podspec => ...`
//      override into the generated `ios/Podfile`. CocoaPods honors the
//      override regardless of any transitive `s.dependency 'Everframe/Core'`
//      declarations (the EverframeRN bridge's podspec has two such lines).
//   2b. Injects `ENV['EVERFRAME_NATIVE_POD_VERSION'] = '<canonical version>'`
//      alongside that override, read out of `packages/sdk-ios/Everframe.podspec`
//      the same way `dev-podspec/Everframe.podspec` tracks it. This is the iOS
//      counterpart of Android's `everframeNativeVersion`: EverframeRN.podspec's
...
//        - Info.plist `EverframeDevIngestURL` (withDevIngestInfoPlist) —
//          works under `expo run:ios`, which deep-links the app via
//          `simctl openurl` and drops host env.
//        - Xcode scheme env EVERFRAME_DEV_INGEST_URL (injectSchemeIngestEnv)
...
//
// Companion to `with-everframe-workspace.js` (Android `mavenLocal()`
...
//
// Opt-out for release / smoke-test runs against the *published* Everframe
// pod from CocoaPods Trunk:
//
//   EVERFRAME_LOCAL_IOS=0 npx expo prebuild --platform ios --clean
//
// When that env var is set to `0`, the plugin strips any prior injection
// from the Podfile and leaves `EverframeRN.podspec`'s standard
// `s.dependency 'Everframe/Core', '~> 0.1.0'` path intact. Default (env
...
// canonical one at packages/sdk-ios/Everframe.podspec (which is `:http =>`-
// sourced from GitHub Releases). Two podspecs named Everframe.podspec
// can't live in the same dir.
const DEV_PODSPEC_DIR_FROM_IOS = '../../../packages/sdk-ios/dev-podspec';
const PROBE = 'EVERFRAME_DEV_PODSPEC';
const MARKER = '# EVERFRAME_DEV_PODSPEC — injected by ./plugins/with-everframe-ios-local.js';

// Env var EverframeRN.podspec reads to override its npm-derived
// `Everframe/Core` version range — see the comment on `native_version_override`
// there. Injected into the Podfile (not just the caller's shell) alongside
// the `pod 'Everframe', :path =>` override so a bare `pod install` resolves.
const NATIVE_VERSION_ENV_VAR = 'EVERFRAME_NATIVE_POD_VERSION';
 ...
 * `packages/sdk-ios/Everframe.podspec` — the same regex
 * `dev-podspec/Everframe.podspec` already uses to track it
 * (`/spec\.version\s*=\s*"([^"]+)"/`). This is the version injected as
 * `EVERFRAME_NATIVE_POD_VERSION` so EverframeRN.podspec's dependency range
...
function readCanonicalSdkVersion(sdkRoot) {
  const podspecPath = path.join(sdkRoot, 'Everframe.podspec');
  const contents = fs.readFileSync(podspecPath, 'utf8');
  const match = contents.match(/spec\.version\s*=\s*"([^"]+)"/);
  if (!match) {
    throw new Error(
      `[with-everframe-ios-local] could not parse spec.version from ${podspecPath}`,
    );
  }
  return match[1];
}
 ...
 * matching the canonical Everframe.podspec layout. Earlier attempts at
 ...
 * (manifested at build time as `no such module 'EverframeKit'`).
 */
const VENDORED_XCFRAMEWORKS = [
  'EverframeProtocol.xcframework',
  'EverframeKit.xcframework',
  'EverframeReporterUI.xcframework',
];
...
/**
 * Rebuild local xcframeworks if `dist/EverframeKit.xcframework` is missing,
 ...
 * honors the EVERFRAME_DEV_INGEST_URL runtime env inside `#if DEBUG`; a
 * Release build is hardwired to https://everframe.dev, so local example-app
...
function ensureXcframeworks(sdkRoot) {
  const flagFramework = path.join(sdkRoot, 'dist', 'EverframeKit.xcframework');
  ...
  const wantConfig =
    process.env.EVERFRAME_XCFRAMEWORK_CONFIGURATION || 'Debug';

  if (!fs.existsSync(buildScript)) {
    throw new Error(
      `[with-everframe-ios-local] expected build script at ${buildScript}`,
    );
  }
  ...
  if (!buildReason) {
    ...
    console.log(
      `[with-everframe-ios-local] dist/ xcframeworks up-to-date (${wantConfig}), skipping rebuild`,
    );
    return;
  }
  ...
  // eslint-disable-next-line no-console
  console.log(
    `[with-everframe-ios-local] building ${wantConfig} dist/ xcframeworks (${buildReason}) — this can take a few minutes`,
  );
  const result = spawnSync('bash', [buildScript], {
    cwd: sdkRoot,
    stdio: 'inherit',
    env: { ...process.env, EVERFRAME_XCFRAMEWORK_CONFIGURATION: wantConfig },
  });
  if (result.status !== 0) {
    throw new Error(
      `[with-everframe-ios-local] build-xcframework.sh failed with exit ${result.status}`,
    );
  }
}
...
// `ENV['EVERFRAME_NATIVE_POD_VERSION'] = …` line (present since the iOS
// escape-hatch was added; absent in Podfiles from older plugin versions —
// hence `?`), and the `pod 'Everframe', …` line with any `:`-prefixed
...
/**
 * Inject the `pod 'Everframe'` override, plus the
 * `ENV['EVERFRAME_NATIVE_POD_VERSION']` assignment EverframeRN.podspec reads
 * to override its npm-derived dependency range, into the Podfile inside the
 * first `target 'EverframeRNExample' do … end` block.
 *
 ...
 * @param {string} nativeVersion canonical `packages/sdk-ios/Everframe.podspec` version
 */
function injectOverride(contents, nativeVersion) {
  ...
  if (!match) {
    throw new Error(
      `[with-everframe-ios-local] could not locate target block in Podfile`,
    );
  }
  ...
  const overrideBlock =
    ...
    `  pod 'Everframe', :path => '${DEV_PODSPEC_DIR_FROM_IOS}'\n`;

  return cleaned.replace(re, `$1${overrideBlock}`);
}

/**
 * Strip any prior `# EVERFRAME_DEV_PODSPEC` injection (pod override AND the
 * `ENV['EVERFRAME_NATIVE_POD_VERSION']` assignment alongside it) without
 ...
 * `s.dependency 'Everframe/Core'` path from EverframeRN.podspec — an
...
// Step 0c originally pre-declared `EverframeRN` with `:testspecs => ['Tests']`
// here, so CocoaPods would build a `EverframeRN-Unit-Tests` target (autolinking
...
// `ios/Tests/*.swift` files was fixed (`@testable import Everframe_ReactNative`
// → `EverframeRN`), the test target still failed to compile — a SEPARATE,
// pre-existing defect: `EverframeRN`'s compiled module records a minimum
...
// is a compile-only gate on the `EverframeRN` scheme instead, and a test
// target that can't compile is worse than none. See
// `.superpowers/sdd/2026-08-13-rn-native-ci-host/task-3-report.md` for the
// full trace.

/**
 * Inject `EVERFRAME_DEV_INGEST_URL` into the app scheme's LaunchAction
 ...
 * scripts/with-free-port.mjs) set SIMCTL_CHILD_EVERFRAME_DEV_INGEST_URL
 ...
 * EVERFRAME_DEV_INGEST_URL when prebuilding for a physical device (LAN IP).
 *
...
function injectSchemeIngestEnv(schemePath, url) {
  if (!fs.existsSync(schemePath)) {
    // eslint-disable-next-line no-console
    console.warn(
      `[with-everframe-ios-local] no scheme at ${schemePath} — skipping scheme env injection (expo-run path still covered via SIMCTL_CHILD_)`,
    );
    return;
  }
  ...
  let next;
  if (original.includes('key = "EVERFRAME_DEV_INGEST_URL"')) {
    // Already injected — just refresh the value.
    next = original.replace(
      /(key = "EVERFRAME_DEV_INGEST_URL"\s*\n\s*value = ")[^"]*(")/,
      `$1${url}$2`,
    );
  } else {
    ...
    if (!closeTag.test(original)) {
      // eslint-disable-next-line no-console
      console.warn(
        `[with-everframe-ios-local] no <LaunchAction> in ${schemePath} — skipping scheme env injection`,
      );
      return;
    }
  ...
  if (next !== original) {
    ...
    // eslint-disable-next-line no-console
    console.log(
      `[with-everframe-ios-local] scheme env EVERFRAME_DEV_INGEST_URL=${url} → ${path.basename(schemePath)}`,
    );
  }
}

/**
 * Bake `EverframeDevIngestURL` into the app's Info.plist. This is the
...
function withDevIngestInfoPlist(config) {
  return withInfoPlist(config, (cfg) => {
    if (process.env.EVERFRAME_LOCAL_IOS === '0') {
      delete cfg.modResults.EverframeDevIngestURL;
      return cfg;
    }
    const url = process.env.EVERFRAME_DEV_INGEST_URL || 'http://localhost:8787';
    cfg.modResults.EverframeDevIngestURL = url;
    // eslint-disable-next-line no-console
    console.log(
      `[with-everframe-ios-local] Info.plist EverframeDevIngestURL=${url}`,
    );
    return cfg;
  });
}

function withEverframeIosLocal(config) {
  ...
  return withDangerousMod(config, [
    ...
    async (cfg) => {
      ...
      // Everframe pod on CocoaPods Trunk. Set `EVERFRAME_LOCAL_IOS=0` in
      // the environment and prebuild will leave (or revert) the Podfile
      // to the unpatched, EverframeRN.podspec-driven dependency path.
      // Anything else (including unset) keeps the local-link behavior so
      // the daily dev loop doesn't require an env var.
      const usePublished = process.env.EVERFRAME_LOCAL_IOS === '0';

      if (!fs.existsSync(podfilePath)) {
        throw new Error(
          `[with-everframe-ios-local] expected Podfile at ${podfilePath}`,
        );
      }
      ...
      if (usePublished) {
        ...
        if (stripped !== original) {
          ...
          // eslint-disable-next-line no-console
          console.log(
            `[with-everframe-ios-local] EVERFRAME_LOCAL_IOS=0 — reverted Podfile to published-pod dependency`,
          );
        } else {
          // eslint-disable-next-line no-console
          console.log(
            `[with-everframe-ios-local] EVERFRAME_LOCAL_IOS=0 — using published Everframe pod (no override present)`,
          );
        }
        return cfg;
      }
      ...
      if (next !== original) {
        ...
        // eslint-disable-next-line no-console
        console.log(
          `[with-everframe-ios-local] injected Everframe dev podspec override into ${podfilePath}`,
        );
      }
      ...
      if (projectName) {
        ...
        injectSchemeIngestEnv(
          schemePath,
          process.env.EVERFRAME_DEV_INGEST_URL || 'http://localhost:8787',
        );
      }
      return cfg;
    },
...
module.exports = withEverframeIosLocal;
