...
//
// SpecimenPlate — react-native-svg port of the web example's generative
// "pinned specimen" illustration (examples/react-web/app/components/
// SpecimenPlate.tsx; keep the geometry in sync). Each insect is drawn from
// its catalog data, so the example needs no binary image assets and every
// plate is still visually distinct. The web version pulses the firefly glow
// with CSS; here the glow is static — the plate must render identically on
// phone and TV without an animation loop.
...
function BeetleBody({ plate }: { plate: Specimen['plate'] }): React.JSX.Element {
  ...
  return (
    <G>
      ...
      {glow ? (
        <>
          <Circle cx={100} cy={158} r={18} fill="url(#txx-glow)" opacity={0.8} />
...
function ButterflyBody({ plate }: { plate: Specimen['plate'] }): React.JSX.Element {
...
function DamselflyBody({ plate }: { plate: Specimen['plate'] }): React.JSX.Element {
...
function MantisBody({ plate }: { plate: Specimen['plate'] }): React.JSX.Element {
...
function BeeBody({ plate }: { plate: Specimen['plate'] }): React.JSX.Element {
...
const BODIES: Record<Specimen['plate']['kind'], (props: { plate: Specimen['plate'] }) => React.JSX.Element> = {
  beetle: BeetleBody,
...
export interface SpecimenPlateProps {
  specimen: Specimen;
  /** Rendered width/height in pt; the plate is square. */
  size: number;
  /** Hide the plate frame + catalog tag (e.g. tiny list thumbnails). */
  bare?: boolean;
}

export function SpecimenPlate({ specimen, size, bare }: SpecimenPlateProps): React.JSX.Element {
  ...
  return (
    <Svg
      ...
      height={size}
      accessible
      accessibilityLabel={`Illustration of ${specimen.commonName} (${specimen.latinName})`}>
      {specimen.plate.glow ? (
        <Defs>
          <RadialGradient id="txx-glow" cx="50%" cy="50%" r="50%">
